/// Otomatik üretilen çeviri haritası (fr).
const Map<String, String> cevirilerFr = {
  'dizi.jpg uygulamasını indir': 'Télécharger l\'application dizi.jpg',
  'Daha hızlı gezinme, bildirimler ve tam ekran deneyim uygulamada.':
      'Navigation plus rapide, notifications et expérience plein écran sont dans l\'application.',
  'Uygulamayı indir': 'Télécharger l\'application',
  'Tarayıcıda devam et': 'Continuer dans le navigateur',
  'Bir bağlantı yapıştır ya da video yükle, arkadaşlarını davet et, aynı anda izleyin.':
      'Collez un lien ou importez une vidéo, invitez vos amis et regardez ensemble.',
  'URL yapıştır': 'Coller le lien',
  'Yapıştır': 'Coller',
  'Panoda görsel yok': 'Aucune image dans le presse-papiers',
  'Video bağlantısı': 'Lien de la vidéo',
  'Odada aç': 'Ouvrir dans le salon',
  'Doğrudan video adresi': 'Adresse vidéo directe',
  '{} videosu': 'Vidéo {}',
  'Başka video seç': 'Choisir une autre vidéo',
  'Bu video adresi açılamadı': 'Impossible d\'ouvrir cette adresse vidéo',
  'Oda sahibi henüz video seçmedi': 'L\'hôte n\'a pas encore choisi de vidéo',
  'Bir video bağlantısı yapıştır ya da dosya yükle':
      'Collez un lien vidéo ou importez un fichier',
  '{} bir video bağlantısı ekledi': '{} a ajouté un lien vidéo',
  'Desteklenen siteler: {} · doğrudan video adresi (.mp4, .webm)':
      'Sites pris en charge : {} · adresse vidéo directe (.mp4, .webm)',
  'Bu adres desteklenmiyor. Desteklenen siteler: {} · doğrudan video adresi (.mp4, .webm)':
      'Cette adresse n\'est pas prise en charge. Sites pris en charge : {} · adresse vidéo directe (.mp4, .webm)',
  'Bu adres desteklenmiyor. {} bağlantısı ya da doğrudan bir video adresi (.mp4) yapıştır.':
      'Cette adresse n\'est pas prise en charge. Collez un lien {} ou une adresse vidéo directe (.mp4).',
  '{} · doğrudan video adresi (.mp4) · ya da en fazla {} GB dosya':
      '{} · adresse vidéo directe (.mp4) · ou un fichier jusqu\'à {} Go',
  '{} · doğrudan video adresi': '{} · Adresse vidéo directe',
  'Bu canlı yayın biçimini (m3u8) tarayıcı oynatamaz; telefon uygulamasında açılır.':
      'Le navigateur ne peut pas lire ce format en direct (m3u8) ; il s\'ouvre dans l\'application mobile.',
  'Yorumun bu sayfada görünecek': 'Ton commentaire apparaîtra sur cette page',
  'Hepsini gördün, baştan gösteriyoruz':
      'Vous avez tout vu, nous vous montrons depuis le début',
  'Aktarılacak tanınan veri bulunamadı.': 'Aucune donnée reconnue à importer.',
  'Arama': 'Recherche',
  'Aramaya başla': 'Lance une recherche',
  'Son aramalar': 'Recherches récentes',
  'Ayarlar': 'Paramètres',
  'Bağla': 'Associer',
  'Bio': 'Bio',
  'Bitirdim': 'Terminé',
  'Bunları da Beğenebilirsin': 'Tu pourrais aussi aimer',
  'Bölüm': 'Épisode',
  'Bıraktım': 'Abandonné',
  'Dil': 'Langue',
  'Dil ara...': 'Rechercher une langue...',
  'Dizi': 'Série',
  'Dizi ve filmlerini takip et': 'Suis tes séries et tes films',
  'Dizi yorumu': 'Commentaire de série',
  'Dizi, film veya kişi ara...':
      'Cherche une série, un film ou une personne...',
  'Dosya en fazla 30MB olabilir': 'Le fichier ne peut pas dépasser 30 Mo',
  'Dosya en fazla {}MB olabilir': 'Le fichier ne peut pas dépasser {} Mo',
  'E-posta': 'E-mail',
  'E-posta veya kullanıcı adı': "E-mail ou nom d'utilisateur",
  'En az 2 harf yaz': 'Écris au moins 2 lettres',
  'Film': 'Film',
  'Film yorumu': 'Commentaire de film',
  'Fotoğraf / video ekle': 'Ajouter une photo / vidéo',
  'Fotoğraf veya GIF — profil ve kapak resmi':
      'Photo ou GIF — photo de profil et de couverture',
  'Giriş Yap': 'Se connecter',
  'Gönder': 'Envoyer',
  'Gönderildi': 'Envoyé',
  'Haftanın Dizileri': 'Séries de la semaine',
  'Haftanın Filmleri': 'Films de la semaine',
  'Henüz izleme kaydın yok': "Tu n'as pas encore d'historique",
  'Henüz listen yok — Profil sekmesinden oluştur.':
      "Tu n'as pas encore de liste — crée-en une depuis l'onglet Profil.",
  'Henüz yorum yok.': 'Pas encore de commentaires.',
  'Hesabın bağlandı!': 'Ton compte est associé !',
  'Hesabın yok mu? Kayıt ol': 'Pas de compte ? Inscris-toi',
  'Hesabını Bağla': 'Associe ton compte',
  'Hesap Oluştur': 'Créer un compte',
  'Kapak resmi ekle': 'Ajouter une couverture',
  'Kaydet': 'Enregistrer',
  'Kendinden bahset...': 'Parle-nous de toi...',
  'Keşfet': 'Découvrir',
  'Kimseyi takip etmiyor': 'Ne suit personne',
  'Kişi ara': 'Rechercher des personnes',
  'Kişi yorumu': 'Commentaire de personne',
  'Konuk Oyuncular': 'Acteurs invités',
  'Kullanıcı adı (küçük harf)': "Nom d'utilisateur (minuscules)",
  'Kullanıcı adı ara...': "Rechercher un nom d'utilisateur...",
  'Liste adı': 'Nom de la liste',
  'Listeye Ekle': 'Ajouter à une liste',
  'Misafir hesabındasın — e-postanla bağla, verilerini kaybetme!':
      'Tu es sur un compte invité — associe ton e-mail pour ne pas perdre tes données !',
  'Misafir olarak devam et': 'Continuer en invité',
  'Oluştur': 'Créer',
  'Oyuncular': 'Casting',
  'Profil': 'Profil',
  'Profil kaydedildi': 'Profil enregistré',
  'Puanla': 'Noter',
  'Puanı Sil': 'Supprimer la note',
  'Puanın': 'Ta note',
  'Sezonlar': 'Saisons',
  'Sunucu hatası ({})': 'Erreur serveur ({})',
  'S{} · {}. Bölüm': 'S{} · E{}',
  'S{}B{} yorumu': 'Commentaire S{}E{}',
  'Takibi Bırak': 'Ne plus suivre',
  'Takip': 'Abonnements',
  'Takip Edilenler': 'Abonnements',
  'Takip Et': 'Suivre',
  'Takipçi': 'Abonnés',
  'Takipçi yok': 'Aucun abonné',
  'Takipçiler': 'Abonnés',
  'Takvim': 'Calendrier',
  'Tamam': 'OK',
  'Tekrar Dene': 'Réessayer',
  'Tekrar dene': 'Réessaie',
  'Toplam İzleme Süresi': "Temps d'écran total",
  'Tüm Zamanların En İyileri': 'Les meilleurs de tous les temps',
  "Tüm verini (izleme, puan, yorum, liste) TV Time uyumlu ZIP olarak al ya da TV Time / Letterboxd ZIP'ini içe aktar.":
      'Exporte toutes tes données (visionnages, notes, commentaires, listes) en ZIP compatible TV Time, ou importe un ZIP TV Time / Letterboxd.',
  'Tümünü Kaldır': 'Tout retirer',
  'Tümünü İzledim': "J'ai tout vu",
  'Türk Dizileri': 'Séries turques',
  'Veri içe aktar (.zip / .csv)': 'Importer des données (.zip / .csv)',
  'Verilerim': 'Mes données',
  'Verilerimi dışa aktar (e-posta)': 'Exporter mes données (e-mail)',
  'Verilerini dışa aktar': 'Exporte tes données',
  'Yaklaşan bölüm yok.\nDizi detayından "İzliyorum" durumuna al, yeni bölümleri burada takip et.':
      'Aucun épisode à venir.\nPasse une série en "Je regarde" depuis sa fiche et suis les nouveaux épisodes ici.',
  'Yapımları': 'Filmographie',
  'Yeni Diziler': 'Nouvelles séries',
  'Yeni Liste': 'Nouvelle liste',
  'Yeni kullanıcı adı (isteğe bağlı)': "Nouveau nom d'utilisateur (facultatif)",
  'Yorum': 'Commentaire',
  'Yorumunu yaz...': 'Écris ton commentaire...',
  'Zaten hesabın var mı? Giriş yap': 'Déjà un compte ? Connecte-toi',
  'ort. {}': 'moy. {}',
  'takip': 'abonnements',
  'takipçi': 'abonnés',
  '{} / {} bölüm': '{} / {} épisodes',
  '{} TMDB': '{} TMDB',
  '{} ay': '{} mois',
  '{} ay~tekil': '{} mois',
  '{} bölüm izlendi': '{} épisodes vus',
  '{} dizi.jpg': '{} dizi.jpg',
  '{} dk': '{} min',
  '{} dk~tekil': '{} min',
  '{} gün': '{} jours',
  '{} gün~tekil': '{} jour',
  '{} içerik': '{} titres',
  '{} içerik · otomatik': '{} titres · automatique',
  '{} kayıt atlandı (eşleşmedi).':
      '{} entrées ignorées (aucune correspondance).',
  '{} saat': '{} heures',
  '{} saat~tekil': '{} heure',
  '{} sezon': '{} saisons',
  '{} yıl': '{} ans',
  '{} yıl~tekil': '{} an',
  '{}. Bölüm': 'Épisode {}',
  '{}. Sezon': 'Saison {}',
  'Çıkış Yap': 'Se déconnecter',
  'Ülke': 'Pays',
  'Ülke ara...': 'Rechercher un pays...',
  'Ülke seç': 'Choisis un pays',
  'İlk yorumu sen yaz!': 'Écris le premier commentaire !',
  'İncelemeler': 'Critiques',
  'İncelemen (isteğe bağlı)': 'Ta critique (facultatif)',
  'İptal': 'Annuler',
  'İzlediklerim': 'Mon historique',
  'İzledim': 'Vu',
  'İzledin': 'Tu as vu',
  'İzleme geçmişin ve listelerin korunur; artık her cihazdan girebilirsin.':
      "Ton historique et tes listes sont conservés ; tu peux maintenant te connecter depuis n'importe quel appareil.",
  'İzleme geçmişin, puanların, yorumların ve listelerin ZIP olarak kayıtlı e-posta adresine gönderilecektir.':
      'Ton historique, tes notes, tes commentaires et tes listes seront envoyés en ZIP à ton adresse e-mail enregistrée.',
  'İzleyeceğim': 'À voir',
  'İzliyorum': 'Je regarde',
  'İçe aktarım tamamlandı': 'Import terminé',
  'Şifre': 'Mot de passe',
  '• profil bilgisi': '• infos de profil',
  '• {} kitaplık kaydı': '• {} entrées de bibliothèque',
  '• {} favori': '• {} favoris',
  '• {} izleme kaydı': '• {} visionnages',
  '• {} liste': '• {} listes',
  '• {} puan': '• {} notes',
  '• {} yorum': '• {} commentaires',
  '{}+ yapım': '{}+ titres',
  'Yorumlar': 'Commentaires',
  'Yorumlar ({})': 'Commentaires ({})',
  'Yorumları ({})': 'Commentaires ({})',
  'Listelerim': 'Mes listes',
  'Nereden izledin?': "Où l'as-tu regardé ?",
  'Diziye git': 'Voir la série',
  'TV / Kanal': 'TV / Chaîne',
  'Sinema': 'Cinéma',
  'DVD / Blu-ray': 'DVD / Blu-ray',
  'Diğer': 'Autre',
  'Akış': 'Fil',
  'Akışın boş.\nİzlediğin dizi ve filmlere yorum yapılınca burada görünecek.':
      'Ton fil est vide.\nQuand on commentera les séries et films que tu regardes, ça apparaîtra ici.',
  'Listeleri ({})': 'Ses listes ({})',
  'Liste boş.': 'Cette liste est vide.',
  'Yanıtla': 'Répondre',
  '@{} kullanıcısına yanıt veriyorsun': 'Tu réponds à @{}',
  'İzlediğim Diziler ({})': 'Séries vues ({})',
  'İzlediğim Filmler ({})': 'Films vus ({})',
  'Tümünü gör': 'Tout voir',
  'Bildirimler': 'Notifications',
  'Henüz bildirim yok': 'Pas encore de notifications',
  '@{} yorumuna yanıt verdi': '@{} a répondu à ton commentaire',
  '@{} yorumunu beğendi': '@{} a aimé ton commentaire',
  '{} ve {} yorumunu beğendi': '{} et {} ont aimé ton commentaire',
  '{} ve {} kişi yorumunu beğendi': '{} et {} autres ont aimé ton commentaire',
  '@{} seni takip etti': '@{} te suit maintenant',
  '@{} sana mesaj gönderdi': "@{} t'a envoyé un message",
  'Mesajlar': 'Messages',
  'Mesaj': 'Message',
  'Mesaja eklenecek': 'Sera ajoute au message',
  'Mesajını yaz...': 'Écris un message...',
  'Henüz sohbetin yok.\nBir profile girip mesaj gönderebilirsin.':
      'Pas encore de discussions.\nOuvre un profil et envoie un message.',
  'Şifreni mi unuttun?': 'Mot de passe oublié ?',
  'Şifreyi Sıfırla': 'Réinitialiser le mot de passe',
  'E-postadaki kod': 'Code reçu par e-mail',
  'Yeni şifre': 'Nouveau mot de passe',
  'Sana Özel': 'Pour toi',
  '{} özetin': 'Ton bilan {}',
  'Yıllık izleme istatistiklerin': 'Tes stats annuelles',
  'Rozetler': 'Badges',
  'Verdiğin puan': 'Notes données',
  'Ortalama puanın': 'Ta note moyenne',
  'En çok izlediğin diziler': 'Tes séries les plus vues',
  'İlk Bölüm': 'Premier épisode',
  '100 Bölüm': '100 épisodes',
  '1000 Bölüm': '1000 épisodes',
  'İlk Film': 'Premier film',
  '50 Film': '50 films',
  'İlk Yorum': 'Premier commentaire',
  '25 Yorum': '25 commentaires',
  '10 Puan': '10 notes',
  'İlk Takipçi': 'Premier abonné',
  '10 Dizi Bitirdin': '10 séries terminées',
  'İzlemeyi Bıraktım': "J'ai arrêté de regarder",
  'İzlediği Diziler ({})': 'Séries vues ({})',
  'İzlediği Filmler ({})': 'Films vus ({})',
  '500 Bölüm': '500 épisodes',
  '5000 Bölüm': '5000 épisodes',
  '10 Film': '10 films',
  '100 Film': '100 films',
  '100 Yorum': '100 commentaires',
  '50 Puan': '50 notes',
  '100 Puan': '100 notes',
  '10 Takipçi': '10 abonnés',
  '50 Takipçi': '50 abonnés',
  '25 Dizi Bitirdin': '25 séries terminées',
  '50 Dizi Bitirdin': '50 séries terminées',
  '10 Beğeni': "10 j'aime",
  '100 Beğeni': "100 j'aime",
  'yazıyor...': 'écrit...',
  'ses kaydediyor...': 'enregistre un vocal...',
  'Profil düzeni': 'Disposition du profil',
  'Konumla ve kırp': 'Positionner et rogner',
  'Listeye eklendi': 'Ajouté à la liste',
  'Listeyi sil?': 'Supprimer la liste ?',
  'Sil': 'Supprimer',
  'Bu içerik hiç izlenmemiş olarak işaretlenecek ve listelerinden kaldırılacak.':
      'Ce titre sera marqué comme jamais vu et retiré de tes listes.',
  'Gelecek bölüm bugün': "Prochain épisode aujourd'hui",
  'Gelecek bölüm {} gün sonra': 'Prochain épisode dans {} jours',
  'Tema': 'Thème',
  'Sistem': 'Système',
  'Koyu': 'Sombre',
  'Açık': 'Clair',
  'İzleyenler': 'Spectateurs',
  'Yorumunu yaz... (@ ile etiketle)':
      'Écris ton commentaire... (@ pour identifier)',
  '@{} bir yorumda seni etiketledi': 'Mention de @{} dans un commentaire',
  'Mesajı sil': 'Supprimer le message',
  'Mesaj silinemedi': 'Impossible de supprimer le message',
  'Bağlantı geçersiz veya sayfa bulunamadı':
      'Lien invalide ou page introuvable',
  "Keşfet'e dön": 'Retour à Découvrir',
  'Düzenle': 'Modifier',
  'Mesajı düzenle': 'Modifier le message',
  'Yanıtlanıyor': 'Réponse',
  'düzenlendi': 'modifié',
  'Fotoğraf': 'Photo',
  'Video': 'Vidéo',
  'İçerik': 'Contenu',
  'çevrimiçi': 'en ligne',
  'son görülme {} dk önce': 'en ligne il y a {} min',
  'son görülme {} saat önce': 'en ligne il y a {} h',
  'son görülme {} gün önce': 'en ligne il y a {} j',
  'Yorumlar yüklenemedi': 'Impossible de charger les commentaires',
  'Favori': 'Favori',
  'Listeye ekle': 'Ajouter à la liste',
  'İçerik paylaş': 'Partager le contenu',
  'Kapat': 'Fermer',
  'İzlediğin dizi ve filmleri işaretledikçe burada toplanır.':
      'Tout ce que tu marques comme vu se retrouve ici.',
  'Henüz sohbetin yok': "Aucune conversation pour l'instant",
  'Bir profile girip mesaj gönderebilirsin.':
      'Ouvre un profil et envoie un message.',
  'Listeyi sil': 'Supprimer la liste',
  'Puan kaydedilemedi': "Impossible d'enregistrer la note",
  'Nerede İzlenir': 'Où regarder',
  'Abonelik': 'Abonnement',
  'Kirala': 'Louer',
  'Satın al': 'Acheter',
  'Devam et': 'Continuer',
  'Şikayet et': 'Signaler',
  'Engelle': 'Bloquer',
  'Engeli kaldır': 'Débloquer',
  'Kullanıcı engellendi': 'Utilisateur bloqué',
  'Engel kaldırıldı': 'Blocage retiré',
  'Şikayet sebebi': 'Motif du signalement',
  'Spam veya yanıltıcı': 'Spam ou trompeur',
  'Taciz veya nefret söylemi': 'Harcèlement ou incitation à la haine',
  'Uygunsuz / cinsel içerik': 'Contenu inapproprié / sexuel',
  'Şiddet veya tehlikeli içerik': 'Violence ou contenu dangereux',
  'Telif hakkı ihlali': 'Violation de droits d’auteur',
  'Şikayetin alındı, teşekkürler': 'Ton signalement a été reçu, merci',
  'Hesabımı Sil': 'Supprimer mon compte',
  'Hesabın ve tüm verin (izleme, puan, yorum, liste, mesaj) kalıcı olarak silinecek. Bu işlem geri alınamaz.':
      'Ton compte et toutes tes données (historique, notes, commentaires, listes, messages) seront définitivement supprimés. Cette action est irréversible.',
  'Şifreni gir': 'Saisis ton mot de passe',
  'Hesabımı kalıcı sil': 'Supprimer définitivement mon compte',
  'Sesli mesaj': 'Message vocal',
  'Görüldü': 'Vu',
  'Gönderi': 'Publication',
  'Ses gönderilemedi': 'Impossible d’envoyer le message vocal',
  'Kaydediliyor…': 'Enregistrement…',
  'Ses oynatılamadı': 'Impossible de lire l’audio',
  'Oynat': 'Lire',
  'Fragmanı oynat': 'Lire la bande-annonce',
  'Altyazıyı aç': 'Activer les sous-titres',
  'Altyazıyı kapat': 'Désactiver les sous-titres',
  '10 saniye ileri': 'Avancer de 10 secondes',
  '10 saniye geri': 'Reculer de 10 secondes',
  'Duraklat': 'Pause',
  'Liste görünümü': 'Vue liste',
  'Takvim görünümü': 'Vue calendrier',
  'Önceki ay': 'Mois précédent',
  'Sonraki ay': 'Mois suivant',
  'Bu gün bölüm yok': 'Aucun épisode ce jour',
  'Gizlilik Politikası': 'Politique de Confidentialité',
  'Son güncelleme: {}': 'Dernière mise à jour : {}',
  'Bu politika, dizi.jpg uygulamasını ve dizijpg.com sitesini kullandığında hangi verilerin toplandığını, nasıl kullanıldığını ve haklarının neler olduğunu açıklar.':
      "Cette politique explique quelles données sont collectées, comment elles sont utilisées et quels sont tes droits lorsque tu utilises l'application dizi.jpg et le site dizijpg.com.",
  'Topladığımız Veriler': 'Données que nous collectons',
  'Hesap: e-posta adresi, kullanıcı adı ve şifre. Şifreler geri döndürülemez şekilde özetlenerek saklanır; misafir hesaplar e-postasız kullanılabilir.':
      "Compte : adresse e-mail, nom d'utilisateur et mot de passe. Les mots de passe sont stockés sous forme de hachage irréversible ; les comptes invités peuvent être utilisés sans e-mail.",
  'Profil: avatar, kapak görseli, bio ve ülke gibi eklemeyi seçtiğin bilgiler.':
      "Profil : les informations que tu choisis d'ajouter, comme l'avatar, l'image de couverture, la bio et le pays.",
  'Kullanım: izleme geçmişin, puanların, yorumların, listelerin, tepkilerin ve favorilerin.':
      'Utilisation : ton historique de visionnage, tes notes, commentaires, listes, réactions et favoris.',
  'Mesajlar: yazılı, görselli ve sesli mesajların sunucularımızda saklanır. Mesajlar uçtan uca şifreli değildir; yalnızca şikayet edilirse moderasyon amacıyla incelenir.':
      "Messages : tes messages texte, image et vocaux sont stockés sur nos serveurs. Les messages ne sont pas chiffrés de bout en bout ; ils ne sont consultés qu'en cas de signalement, à des fins de modération.",
  'Yüklenen medya: profiline, yorumlarına ve mesajlarına eklediğin fotoğraf, GIF, video ve ses kayıtları.':
      'Médias envoyés : photos, GIF, vidéos et enregistrements vocaux que tu ajoutes à ton profil, tes commentaires et tes messages.',
  'Teknik: IP adresi, yaklaşık konum (ülke/şehir düzeyi), cihaz platformu, uygulama sürümü ve hata kayıtları. Bunlar güvenlik ve hata ayıklama için tutulur.':
      "Données techniques : adresse IP, localisation approximative (pays/ville), plateforme de l'appareil, version de l'application et journaux d'erreurs. Elles sont conservées pour la sécurité et le débogage.",
  'Kullanım istatistikleri: hangi cihaz türü, işletim sistemi ve tarayıcıyla girildiği kaba sınıflar hâlinde günlük toplam sayaçlara eklenir; tarayıcı kimliğinin kendisi saklanmaz ve bu sayılar kişilere bağlanamaz.':
      'Statistiques d’usage : le type d’appareil, le système d’exploitation et le navigateur utilisés pour se connecter sont ajoutés à des compteurs quotidiens globaux sous forme de catégories approximatives ; l’identifiant du navigateur lui-même n’est pas conservé et ces chiffres ne peuvent être rattachés à personne.',
  "Bildirimler: push bildirimleri için cihaz token'ı ve dil tercihin saklanır. Bildirimleri cihazının ayarlarından istediğin zaman kapatabilirsin.":
      "Notifications : pour les notifications push, nous conservons le jeton de ton appareil et ta langue. Tu peux les désactiver à tout moment dans les réglages de l'appareil.",
  'Verileri Nasıl Kullanırız': 'Comment nous utilisons tes données',
  'Verilerini yalnızca hizmeti sunmak, hesabını korumak, bildirim göndermek, hataları gidermek ve kötüye kullanımı önlemek için kullanırız. Verilerini satmayız, reklam amacıyla kimseyle paylaşmayız.':
      'Nous utilisons tes données uniquement pour fournir le service, protéger ton compte, envoyer des notifications, corriger les erreurs et prévenir les abus. Nous ne vendons pas tes données et ne les partageons avec personne à des fins publicitaires.',
  'Çerezler ve Yerel Depolama': 'Cookies et stockage local',
  'Oturumunu açık tutmak ve dil/tema gibi tercihlerini hatırlamak için yerel depolama kullanırız. Sitenin hangi bölümlerinin ne kadar kullanıldığını toplu olarak görmek için web tarafında Google Analytics çalışır; bu araç tarayıcına kendi ölçüm çerezini yazar. Reklam veya reklam hedefleme çerezi yoktur.':
      'Nous utilisons le stockage local pour te garder connecté et mémoriser tes préférences comme la langue et le thème. Sur le Web, Google Analytics fonctionne afin que nous puissions voir de façon agrégée quelles parties du site sont utilisées ; cet outil dépose son propre cookie de mesure dans ton navigateur. Il n’y a aucun cookie publicitaire ou de ciblage.',
  'Üçüncü Taraf Hizmetler': 'Services tiers',
  "Dizi ve film bilgileri TMDB'den, izleme sağlayıcı bilgisi JustWatch'tan alınır. Push bildirimleri Google Firebase üzerinden iletilir, web trafiği ölçümü Google Analytics ile yapılır, site trafiği Cloudflare tarafından korunur. Bu hizmetler kendi gizlilik politikalarına tabidir.":
      'Les informations sur les séries et les films proviennent de TMDB, et celles sur les fournisseurs de streaming de JustWatch. Les notifications push sont transmises via Google Firebase, le trafic web est mesuré avec Google Analytics et le trafic du site est protégé par Cloudflare. Ces services sont soumis à leurs propres politiques de confidentialité.',
  'Saklama ve Silme': 'Conservation et suppression',
  'Verilerin hesabın açık olduğu sürece saklanır. Ayarlar\'daki "Hesabımı Sil" ile hesabını kalıcı olarak silebilirsin; verilerin anında, yedeklerdeki kopyaları en geç 14 gün içinde silinir. Hata kayıtları 30 gün sonra otomatik silinir.':
      "Tes données sont conservées tant que ton compte existe. Avec « Supprimer mon compte » dans les Réglages, tu peux supprimer ton compte définitivement ; tes données sont effacées immédiatement et leurs copies dans les sauvegardes sous 14 jours au plus tard. Les journaux d'erreurs sont supprimés automatiquement après 30 jours.",
  "Verilerini Ayarlar'dan ZIP olarak dışa aktarabilirsin; arşiv e-posta adresine gönderilir.":
      "Tu peux exporter tes données au format ZIP depuis les Réglages ; l'archive est envoyée à ton adresse e-mail.",
  'Güvenlik': 'Sécurité',
  'Veriler şifreli bağlantı (HTTPS) üzerinden taşınır ve erişimi sınırlı sunucularda saklanır.':
      'Les données transitent par des connexions chiffrées (HTTPS) et sont stockées sur des serveurs à accès restreint.',
  'Çocukların Gizliliği': 'Confidentialité des enfants',
  'dizi.jpg 13 yaşından küçük çocuklara yönelik değildir.':
      "dizi.jpg ne s'adresse pas aux enfants de moins de 13 ans.",
  'Hakların': 'Tes droits',
  'KVKK ve GDPR kapsamında verilerine erişme, düzeltme, silme ve taşıma hakkına sahipsin. Bu haklar için bize yazabilirsin: {}':
      "En vertu de la KVKK et du GDPR, tu as le droit d'accéder à tes données, de les corriger, de les supprimer et de les transférer. Pour cela, écris-nous : {}",
  'Değişiklikler': 'Modifications',
  'Bu politika değişirse yeni sürümü bu sayfada yayımlanır ve güncelleme tarihi yenilenir.':
      'Si cette politique change, la nouvelle version sera publiée sur cette page et la date sera mise à jour.',
  'Kayıt olarak {} kabul etmiş olursun.': "En t'inscrivant, tu acceptes la {}.",
  'Kullanıcılar': 'Utilisateurs',
  'Fotoğrafı değiştir': 'Changer la photo',
  'Yeniden konumlandır': 'Repositionner',
  'Bir şeyler ters gitti': 'Un problème est survenu',
  'Sosyal Bağlantılar': 'Liens sociaux',
  'Profilinde en fazla 3 bağlantı gösterilir.':
      "Jusqu'à 3 liens s'affichent sur ton profil.",
  'Platform seç': 'Choisis une plateforme',
  'Kullanıcı adın': "Ton nom d'utilisateur",
  'Ekle': 'Ajouter',
  'Kaldır': 'Retirer',
  'Kopyalandı: {}': 'Copié : {}',
  'Geçersiz kullanıcı adı': "Nom d'utilisateur invalide",
  'Bağlantı ekle': 'Ajouter un lien',
  'Spoiler olabilir — dokun ve gör':
      'Peut contenir des spoilers — touche pour voir',
  'Beğeni': "J'aime",
  'beğeni': "j'aime",
  'Beğen': 'Jaime',
  'Görüntülenme': 'Vues',
  'görüntülenme': 'vues',
  'Tam ekran': 'Plein écran',
  'Arama başarısız': 'Échec de la recherche',
  'Sonuç bulunamadı': 'Aucun résultat',
  'Dizi ve Filmler': 'Séries et films',
  'Kişiler': 'Personnes',
  'Ana Sayfa': 'Accueil',
  'Paylaş': 'Partager',
  'Yanıtlar': 'Réponses',
  'Yaklaşan Bölümler': 'Épisodes à venir',
  'Yetişme Listesi': 'Liste de rattrapage',
  'Şunu mu demek istedin': 'Vouliez-vous dire',
  'Spoiler': 'Spoiler',
  'Spoiler — dokun ve gör': 'Spoiler — appuie pour voir',
  'uyum': "d'affinité",
  'Uyum': 'Affinité',
  '{} ortak dizi': '{} séries en commun',
  '{} ortak film': '{} films en commun',
  'Bildirim Tercihleri': 'Préférences de notification',
  'Beğeniler': "J'aime",
  'Yeni takipçiler': 'Nouveaux abonnés',
  'Etiketlenmeler': 'Mentions',
  'Yeniden izledim': 'Revu',
  '{}. kez izlendi': 'Vu {} fois',
  'Gönderi bulunamadı': 'Publication introuvable',
  'Gözat': 'Parcourir',
  'Diziler': 'Séries',
  'Filmler': 'Films',
  'Tümü': 'Tout',
  'Farklı bir tür seç.': 'Essaie un autre genre.',
  'Gizlilik': 'Confidentialité',
  'İzlediklerimi gizle': 'Masquer ce que je regarde',
  'Profilinde izlediğin dizi ve filmler görünmez':
      "Les séries et films que tu as vus n'apparaîtront pas sur ton profil",
  'Yorumlarımı gizle': 'Masquer mes commentaires',
  'Profilinde yorumların görünmez':
      "Tes commentaires n'apparaîtront pas sur ton profil",
  'Tek bir dizi veya filmi, içeriğin sayfasındaki "Profilimde gizle" çipiyle gizleyebilirsin.':
      'Tu peux masquer une série ou un film précis avec la puce « Masquer sur mon profil » sur sa page.',
  'Geri Bildirim': 'Ton avis',
  'Uygulama hakkında görüş ve önerini yaz...':
      "Écris ton avis et tes suggestions sur l'appli...",
  'Teşekkürler! Geri bildirimin alındı.': 'Merci ! Ton avis a bien été reçu.',
  'Profilimde gizle': 'Masquer sur mon profil',
  'Yorumları': 'Commentaires',
  'Bu kullanıcı henüz bir şey izlememiş.':
      'Aucun contenu regardé pour le moment.',
  'Bu kullanıcı yorumlarını gizli tutmayı tercih ediyor.':
      'Cet utilisateur préfère garder ses commentaires privés.',
  'S{}B{}': 'S{}E{}',
  'Veri: JustWatch': 'Données : JustWatch',
  'Web Sitesi': 'Site web',
  'Diğer uygulamalar': 'Autres applis',
  'Bağlantıyı kopyala': 'Copier le lien',
  'Henüz kimseyi takip etmiyorsun.': 'Tu ne suis encore personne.',
  'Çevir': 'Traduire',
  'Çeviri şu an yapılamadı': 'Traduction impossible pour le moment',
  'Bağlantı açılamadı': 'Ouverture du lien impossible',
  'Orijinali göster': "Voir l'original",
  'Çeviriyi göster': 'Afficher la traduction',
  'Bu ay için yayın tarihi açıklanmış bölüm yok. Tarihler genelde birkaç hafta önceden duyurulur; açıklandıkça burada görünür.':
      "Aucun épisode avec une date de diffusion annoncée ce mois-ci. Les dates sont généralement révélées quelques semaines à l'avance et apparaîtront ici.",
  'devamı': 'plus',
  'Google ile devam et': 'Continuer avec Google',
  'Google girişi başarısız': 'Échec de la connexion Google',
  'Apple ile devam et': 'Continuer avec Apple',
  'Apple girişi başarısız': 'Échec de la connexion avec Apple',
  'Veri tasarrufu': 'Économiseur de données',
  'Wi-Fi ağında veri tasarrufu': 'Économiseur de données en Wi-Fi',
  'Mobil veride veri tasarrufu': 'Économiseur de données en données mobiles',
  'Açıkken fotoğraflar önceden indirilmez, yalnızca baktığın kare yüklenir.':
      'Quand il est activé, les photos ne sont pas téléchargées à lavance ; seule limage que tu regardes se charge.',
  'Çocuk güvenliği': 'Sécurité des enfants',
  'Türk Filmleri': 'Films turcs',
  'En Yüksek Puanlı Filmler': 'Films les mieux notés',
  'En Yüksek Puanlı Diziler': 'Séries les mieux notées',
  'En Çok İzlenen Filmler': 'Films les plus vus',
  'Popüler Diziler': 'Séries populaires',
  'En Çok Kazanan Filmler': 'Films les plus rentables',
  'Kült Filmler': 'Films cultes',
  'Yeni Filmler': 'Nouveaux films',
  'Yorum yap': 'Commenter',
  'Henüz yorum yok': 'Pas encore de commentaires',
  'Dizi ve filmlere yazdığın yorumlar burada toplanır.':
      'Tes commentaires sur les séries et films sont réunis ici.',
  'GIF ekle': 'Ajouter un GIF',
  'İlk yorumu sen yaz': 'Sois le premier à commenter',
  'Video altyazıları': 'Sous-titres vidéo',
  'Videolarda altyazı göster': 'Afficher les sous-titres sur les vidéos',
  'Videoda konuşulan cümle ekranda yazılır. Metin çeviridir: Türkçe konuşmada İngilizce, diğer dillerde Türkçe.':
      'La phrase prononcée dans la vidéo est écrite à l’écran. Le texte est une traduction : anglais dans la langue turque, turc dans d’autres langues.',
  'Güncelleme gerekli': 'Mise à jour requise',
  'Yeni sürüm var': 'Nouvelle version disponible',
  "Devam etmek için dizi.jpg'in yeni sürümünü yükle.":
      'Installe la nouvelle version de dizi.jpg pour continuer.',
  'Güncelle': 'Mettre à jour',
  'Daha sonra': 'Plus tard',
  'Takvim güncelleniyor': 'Mise à jour du calendrier',
  'Takvim güncellenemedi, eski liste gösteriliyor':
      'Impossible de mettre à jour le calendrier, ancienne liste affichée',
  'Bazı diziler yüklenemedi, liste eksik olabilir':
      'Certaines séries ne se sont pas chargées, la liste peut être incomplète',
  'Yenile': 'Actualiser',
  'Devam etmek için giriş yap': 'Connecte-toi pour continuer',
  'Puan vermek, yorum yazmak ve kitaplığını oluşturmak için ücretsiz bir hesap yeterli.':
      'Un compte gratuit suffit pour noter, commenter et créer ta bibliothèque.',
  'Şimdi değil': 'Pas maintenant',
  'Yorum yazmak için giriş yap': 'Connecte-toi pour écrire un commentaire',
  'Kişilere göndermek için giriş yap':
      'Connecte-toi pour envoyer à des personnes',
  'Beğenenler': 'Mentions Jaime',
  'Henüz beğeni yok': 'Pas encore de mentions Jaime',
  'Bu gönderiyi ilk beğenen sen ol':
      'Sois la premiere personne a aimer ce post',
  'Bu gönderiye yanıt': 'Reponse a cette publication',
  'Spoiler içeren gönderi': 'Publication avec spoilers',
  'Görsel gönderi': 'Publication avec photo',
  'Bu yorumu profilimde gizle': 'Masquer ce commentaire sur mon profil',
  'Yorum dizi ve film sayfasında durmaya devam eder':
      'Le commentaire reste sur la page de la série et du film',
  'Bu yorumu sil': 'Supprimer ce commentaire',
  'Yorum profilinde gizlendi': 'Commentaire masqué sur ton profil',
  'Geri al': 'Annuler',
  'Yorum silinsin mi?': 'Supprimer le commentaire ?',
  'Bu yorum her yerden kalıcı olarak silinir. Geri alınamaz.':
      'Ce commentaire sera supprimé définitivement partout. Impossible d’annuler.',
  'Yorum silindi': 'Commentaire supprimé',
  'Yorum profilinde tekrar görünüyor':
      'Le commentaire est de nouveau visible sur ton profil',
  'Gizlenen yorumun yok': 'Aucun commentaire masqué',
  'Profilindeki Yorumlar sekmesinde bir gönderiye basılı tutup gizlediklerin burada birikir.':
      'Les commentaires que tu masques en appuyant longuement sur une publication dans l’onglet Commentaires de ton profil s’accumulent ici.',
  'Gizlenen yorumlar': 'Commentaires masqués',
  'Gönderiye git': 'Aller à la publication',
  'Tekrar göster': 'Afficher à nouveau',
  'Yanıtlarımı gizle': 'Masquer mes réponses',
  'Başkalarının gönderilerine yazdığın yanıtlar yalnız sana görünür':
      'Les réponses que tu écris aux publications des autres ne sont visibles que par toi',
  'Profilinde gizlediğin yorumları tekrar göster':
      'Afficher à nouveau les commentaires que tu as masqués sur ton profil',
  'Sıralama': 'Tri',
  'Önerilen': 'Recommandé',
  'Kronolojik': 'Chronologique',
  'Gelen mesaj istekleri': 'Demandes de message reçues',
  'Mesaj isteğin yok': 'Aucune demande de message',
  'Takip etmediğin kişilerden gelen mesajlar burada görünür.':
      'Les messages des personnes que vous ne suivez pas apparaissent ici.',
  'Çevrimiçi durumumu gizle': 'Masquer mon statut en ligne',
  'Mesajlarda çevrimiçi olduğun ve son görülme zamanın başkalarına görünmez':
      'Les autres ne verront pas votre statut en ligne ni votre dernière connexion dans les messages',
  'İlk kullanıcılarımızdan biri. Geri bildirimleriyle uygulamanın bugün olduğu hale gelmesine katkı sağladı.':
      'Fait partie de nos premiers utilisateurs. Ses retours ont aidé cette application à devenir ce que nous avons maintenant.',
  'İlk kullanıcılarımızdan birisin. Geri bildirimlerinle uygulamanın bugün olduğu hale gelmesine katkı sağladın.':
      'Tu fais partie de nos premiers utilisateurs. Tes retours ont aidé cette application à devenir ce que nous avons maintenant.',
  'İleri': 'Suivant',
  'Önceki': 'Précédent',
  'Sonraki': 'Suivant',
  'Seçilen dosya okunamadı': 'Impossible de lire le fichier sélectionné',
  'Video açılamadı': 'Impossible d’ouvrir la vidéo',
  'En fazla {} medya seçebilirsin':
      'Vous pouvez sélectionner au maximum {} médias',
  'Hiçbir medya yüklenemedi': 'Aucun média envoyé',
  'Seçimi kaldır': 'Retirer de la sélection',
  'Tek yorumda toplam {} MB medya yükleyebilirsin':
      'Limite totale par commentaire : {} MB',
  '{} dosya okunamadı': '{} fichiers illisibles',
  '{} medya eklendi, {} yüklenemedi': '{} médias ajoutés, {} en échec',
  'Kırp': 'Recadrer',
  'Çiz': 'Dessiner',
  'Metin': 'Texte',
  'Emoji': 'Emoji',
  'Yinele': 'Rétablir',
  'Daha fazla': 'Plus',
  'Hazırlanıyor…': 'Préparation…',
  'Düzenlemeden çık?': 'Quitter la retouche ?',
  'Yaptığın değişiklikler kaybolacak.': 'Vos modifications seront perdues.',
  'Çık': 'Quitter',
  'Serbest çizim': 'Main levée',
  'Ok': 'Flèche',
  'Dikdörtgen': 'Rectangle',
  'Daire': 'Cercle',
  'Bulanıklaştır': 'Flouter',
  'Pikselleştir': 'Pixelliser',
  'Silgi': 'Gomme',
  'Çizgi kalınlığı': 'Épaisseur du trait',
  'Saydamlık': 'Opacité',
  'Dolgu': 'Remplissage',
  'Renk': 'Couleur',
  'Döndür': 'Pivoter',
  'Yansıt': 'Retourner',
  'Oran': 'Format',
  'Sıfırla': 'Réinitialiser',
  'Serbest': 'Libre',
  'Metin yaz...': 'Saisir du texte...',
  'Hizala': 'Aligner',
  'Arka plan': 'Arrière-plan',
  'Emoji ara...': 'Rechercher un emoji...',
  'Son kullanılanlar': 'Récents',
  'İfadeler ve insanlar': 'Smileys et personnes',
  'Hayvanlar ve doğa': 'Animaux et nature',
  'Yiyecek ve içecek': 'Nourriture et boissons',
  'Etkinlikler': 'Activités',
  'Seyahat ve yerler': 'Voyages et lieux',
  'Nesneler': 'Objets',
  'Semboller': 'Symboles',
  'Bayraklar': 'Drapeaux',
  'Görseli düzenle': 'Modifier la photo',
  'Düzenlendi': 'Modifiée',
  'Düzenlenemedi': 'Modification impossible',
  'Düzenlenen görsel çok büyük': 'Image modifiée trop volumineuse',
  'Videoyu düzenle': 'Modifier la video',
  'Sesi kapat': 'Couper le son',
  'Sesi aç': 'Activer le son',
  'İptal ediliyor…': 'Annulation…',
  'Bu biraz sürebilir': 'Cela peut prendre un moment',
  'Video çok büyük': 'La video est trop volumineuse',
  'Video hazırlanamadı': 'Impossible de preparer la video',
  'Başlangıç': 'Debut',
  'Bitiş': 'Fin',
  'En çok {} saniye seçebilirsin':
      'Vous pouvez selectionner jusqu a {} secondes',
  'Dosya en fazla {} MB olabilir': 'Le fichier peut faire au maximum {} MB',
  'Medya ekle': 'Ajouter un média',
  'Seçili medya kalmadı': 'Aucun média sélectionné',
  'Yorumuna eklemek için fotoğraf veya video seç.':
      'Choisis une photo ou une vidéo à ajouter à ton commentaire.',
  'Medya seç': 'Sélectionner un média',
  'Daha fazla ekle': 'En ajouter d’autres',
  'Medya seçilemedi': 'Impossible de sélectionner le média',
  'Favori oyuncular': 'Acteurs favoris',
  'Henüz favori oyuncun yok': 'Pas encore d acteurs favoris',
  'Bir oyuncunun sayfasını aç ve sağ üstteki kalbe dokun; buraya eklensin.':
      'Ouvre la page d un acteur et touche le coeur en haut a droite pour l ajouter ici.',
  '{} yapımdan {} tanesini izledin': 'Tu as vu {} sur {} titres',
  'izledin': 'vus',
  'İzlemedin': 'Non vu',
  'Yapım bulunamadı': 'Aucun titre trouve',
  'Bu kişinin listelenecek dizi veya filmi yok.':
      'Cette personne n a aucune serie ni film a lister.',
  'İzlediklerin hesabına bağlı; bu liste yalnız sana özel.':
      'Ton historique est lie a ton compte ; cette liste n appartient qu a toi.',
  'Hesabın kalıcı olarak askıya alındı':
      'Ton compte a été suspendu définitivement',
  'Hesabın geçici olarak askıya alındı':
      'Ton compte a été suspendu temporairement',
  'Hesabın askıya alındı': 'Ton compte est suspendu',
  'Sebep': 'Motif',
  'Kalan süre': 'Temps restant',
  'Süre dolunca hesabın kendiliğinden açılır':
      'Ton compte sera rouvert automatiquement à la fin du délai',
  'Bu süre boyunca okumaya devam edebilirsin; gönderi, yorum ve mesaj gönderemezsin.':
      'Tu peux continuer à lire pendant ce temps, mais tu ne peux pas publier, commenter ni envoyer de messages.',
  'Anladım': 'Compris',
  'Ayrıntı': 'Détails',
  '{} dakika': '{} minutes',
  'birkaç saniye': 'quelques secondes',
  'Karara itiraz et': 'Contester cette décision',
  'Neden bu cezanın kaldırılması gerektiğini anlat':
      'Explique pourquoi cette sanction devrait être levée',
  'İtirazını gönder': 'Envoyer ta contestation',
  'İtirazın incelemede': 'Ta contestation est en cours dexamen',
  'İtirazın reddedildi': 'Ta contestation a été rejetée',
  'İtirazın alındı, inceleniyor':
      'Ta contestation a bien été reçue et est en cours dexamen',
  'İtiraz en az 10 karakter olmalı':
      'La contestation doit faire au moins 10 caractères',
  'Yükleniyor...': 'Chargement...',
  'Bu bölüme puan ver': 'Notez cet épisode',
  '{} kişi puanladı': '{} personnes ont noté',
  'Bölüm izlendi olarak işaretlendi': 'Épisode marqué comme vu',
  'Sesli ara': 'Appel audio',
  'Görüntülü ara': 'Appel vidéo',
  'Çalıyor...': 'Sonnerie...',
  'Bağlanıyor...': 'Connexion...',
  'Cevapla': 'Répondre',
  'Reddet': 'Refuser',
  'Sessize al': 'Couper le micro',
  'Hoparlör': 'Haut-parleur',
  'Kamerayı kapat': 'Couper la caméra',
  'Kamerayı çevir': 'Changer de caméra',
  'Cevap yok': 'Pas de réponse',
  'Arama reddedildi': 'Appel refusé',
  'Meşgul': 'Occupé',
  'Bağlanılamadı': 'Connexion impossible',
  'Arama iptal edildi': 'Appel annulé',
  'Arama geçmişi': 'Historique des appels',
  'Gelen arama': 'Appel entrant',
  'Giden arama': 'Appel sortant',
  'Cevapsız arama': 'Appel manqué',
  'Arama şu anda kullanılamıyor':
      'Les appels sont indisponibles pour le moment',
  'Görüntülü arama şu anda kapalı, sesli arayabilirsin':
      'Les appels vidéo sont désactivés, tu peux passer un appel audio',
  'Bu kullanıcıyı arayamazsın': 'Tu ne peux pas appeler cet utilisateur',
  'Aramak için karşılıklı takipleşmelisiniz':
      'Vous devez vous suivre mutuellement pour vous appeler',
  'Bu kullanıcı şu anda aranamıyor':
      'Cet utilisateur ne peut pas être appelé pour le moment',
  'Çok fazla cevapsız arama. {} sonra tekrar dene':
      'Trop d’appels manqués. Réessaie dans {}',
  'Zaten bir aramadasın': 'Tu es déjà en appel',
  'Arama için mikrofon izni gerekiyor':
      'L’autorisation du microphone est nécessaire pour appeler',
  'Görüntülü arama için kamera ve mikrofon izni gerekiyor':
      'L’autorisation de la caméra et du microphone est nécessaire pour les appels vidéo',
  'Arama başlatılamadı': 'Impossible de démarrer l’appel',
  'Kullanıcı bulunamadı': 'Utilisateur introuvable',
  'Kendini arayamazsın': 'Tu ne peux pas t’appeler toi-même',
  'Arama artık geçerli değil': 'Cet appel n’est plus valide',
  'Bu aramanın tarafı değilsin': 'Tu ne fais pas partie de cet appel',
  'Arama bulunamadı': 'Appel introuvable',
  'Çok fazla istek, biraz bekle': 'Trop de requêtes, patiente un peu',
  'Bağlantı koptu': 'Connexion perdue',
  'Arama süre sınırına ulaştı': 'L’appel a atteint sa limite de durée',
  'Arama en fazla 4 saat sürebilir': 'Un appel peut durer au maximum 4 heures',
  'Arama sona erdi': 'L’appel est terminé',
  'Sesli arama': 'Appel audio',
  'Görüntülü arama': 'Appel vidéo',
  'Gelen aramalar': 'Appels entrants',
  'Sesli ve görüntülü arama bildirimleri':
      'Notifications d’appels audio et vidéo',
  '{} sa': '{} h',
  'Sesli ve Görüntülü Aramalar': 'Appels audio et vidéo',
  'Aramaların içeriği kaydedilmez. Ses ve görüntü, cihazlar arasında uçtan uca şifreli (DTLS-SRTP) akar; sunucularımız bu trafiği çözemez, dinleyemez ve saklayamaz.':
      'Le contenu des appels n’est jamais enregistré. L’audio et la vidéo circulent chiffrés de bout en bout (DTLS-SRTP) entre les appareils ; nos serveurs ne peuvent ni déchiffrer, ni écouter, ni conserver ce trafic.',
  'Yalnızca arama üstverisi tutulur: kiminle, hangi yönde, ne zaman ve ne kadar sürdüğü. Bu kayıtlar 90 gün sonra otomatik silinir.':
      'Seules les métadonnées de l’appel sont conservées : avec qui, dans quel sens, quand et pendant combien de temps. Ces enregistrements sont supprimés automatiquement au bout de 90 jours.',
  'Doğrudan bağlantı kurulamazsa ses ve görüntü şifreli hâlde bir aktarma sunucusundan (TURN) geçer. Aktarma sunucusu da içeriği çözemez ve kaydetmez.':
      'Si une connexion directe est impossible, l’audio et la vidéo transitent chiffrés par un serveur relais (TURN). Ce relais ne peut pas non plus déchiffrer ni enregistrer le contenu.',
  'Mikrofon yalnızca sesli veya görüntülü arama sırasında, kamera ise yalnızca görüntülü arama sırasında kullanılır. Arama bitince ikisi de kapatılır.':
      'Le microphone n’est utilisé que pendant un appel audio ou vidéo, et la caméra uniquement pendant un appel vidéo. Les deux sont coupés à la fin de l’appel.',
  'Takip ettiğin {} kişi izledi': "{} personnes que tu suis l'ont regardé",
  'Yorum yaz...': 'Écris un commentaire...',
  'Sıradaki bölüm': 'Épisode suivant',
  'Doğrulanmış testçi': 'Testeur vérifié',
  'Takip, beğeni, yanıt, mesaj ve etiket bildirimleri':
      "Notifications de suivis, j'aime, réponses, messages et mentions",
  'Sen': 'Toi',
  'Aradığınız kişide sesli arama devre dışı':
      'Les appels vocaux sont désactivés pour cette personne',
  'Aradığınız kişide görüntülü arama devre dışı':
      'Les appels vidéo sont désactivés pour cette personne',
  'Sesli aramalara izin ver': 'Autoriser les appels vocaux',
  'Görüntülü aramalara izin ver': 'Autoriser les appels vidéo',
  'Kapalıyken kimse seni sesli arayamaz; arayan uyarı görür':
      "Si désactivé, personne ne peut t'appeler en vocal ; l'appelant voit un avis",
  'Kapalıyken kimse seni görüntülü arayamaz; arayan uyarı görür':
      "Si désactivé, personne ne peut t'appeler en vidéo ; l'appelant voit un avis",
  'Sesli arama kapalı. Ayarlar > Gizlilik bölümünden açabilirsin.':
      'Les appels vocaux sont désactivés. Tu peux les activer dans Paramètres > Confidentialité.',
  'Görüntülü arama kapalı. Ayarlar > Gizlilik bölümünden açabilirsin.':
      'Les appels vidéo sont désactivés. Tu peux les activer dans Paramètres > Confidentialité.',
  'Sesli ve görüntülü arama': 'Appels vocaux et vidéo',
  'Misafir hesaplar arama yapamaz. Hesap oluşturursan kullanabilirsin.':
      'Les comptes invités ne peuvent pas appeler. Crée un compte pour l’utiliser.',
  'Misafir hesaplar aranamaz': 'Impossible d’appeler un compte invité',
  'Puan dağılımı': 'Répartition des notes',
  '{} yıldız': '{} étoiles',
  'Yakında gelecek': 'Bientôt disponible',
  '{} {} yayınlandı': '{} {} est disponible',
  'Yeni bölüm yayınlandı': 'Nouvel épisode disponible',
  'Yeni bölümler': 'Nouveaux épisodes',
  'Geri': 'Retour',
  'Karşılamayı kapat': "Fermer l'introduction",
  'Adım {} / {}': 'Étape {} / {}',
  'Hadi başlayalım': "C'est parti",
  'Kaydet ve devam': 'Enregistrer et continuer',
  '{} tanesini ekle': 'Ajouter {}',
  'Şimdilik geç': 'Plus tard',
  'Doğum tarihin ne zaman?': 'Quelle est ta date de naissance ?',
  'Yaşına uygun içerik göstermek ve doğum gününü kutlamak için soruyoruz.':
      "On te le demande pour t'afficher des contenus adaptés à ton âge et pour te souhaiter ton anniversaire.",
  'Gün': 'Jour',
  'Ay': 'Mois',
  'Yıl': 'Année',
  'Doğum yılımı paylaşmak istemiyorum':
      'Je ne veux pas partager mon année de naissance',
  'Doğum tarihin profilinde herkese açık gösterilmez. Yalnız yaşa uygun içerik ve doğum günü kutlaması için kullanılır.':
      "Ta date de naissance n'est pas affichée publiquement sur ton profil. Elle sert uniquement aux contenus adaptés à l'âge et aux vœux d'anniversaire.",
  'Ocak': 'janvier',
  'Şubat': 'février',
  'Mart': 'mars',
  'Nisan': 'avril',
  'Mayıs': 'mai',
  'Haziran': 'juin',
  'Temmuz': 'juillet',
  'Ağustos': 'août',
  'Eylül': 'septembre',
  'Ekim': 'octobre',
  'Kasım': 'novembre',
  'Aralık': 'décembre',
  'Verilerini yanında getir': 'Emporte tes données avec toi',
  'Başka bir uygulamada izlediklerini takip ediyorsan buraya taşıyabilirsin. Sıfırdan başlamana gerek yok.':
      'Si tu suis ce que tu regardes dans une autre appli, tu peux tout transférer ici. Pas besoin de repartir de zéro.',
  'Dışa aktardığın ZIP/CSV dosyasını seç; izlediklerin, puanların ve listelerin aktarılsın.':
      'Choisis le fichier ZIP/CSV que tu as exporté : ce que tu as regardé, tes notes et tes listes seront importés.',
  'Aktarılıyor...': 'Import en cours...',
  'ZIP/CSV dosyası seç': 'Choisir un fichier ZIP/CSV',
  'Aceleye gerek yok: bunu sonra Ayarlar > Verilerim bölümünden de yapabilirsin.':
      'Rien ne presse : tu peux aussi le faire plus tard dans Paramètres > Mes données.',
  'Hangi filmleri izledin?': 'Quels films as-tu vus ?',
  'En çok izlenenler burada. Dokunduklarını izlediklerine ekleyelim.':
      'Voici les plus regardés. Touche ceux que tu as vus et on les ajoute à ton historique.',
  'SERİ FİLMLER': 'SAGAS',
  'Serilerden {} film eklendi': '{} films ajoutés depuis les sagas',
  'Film listesi yüklenemedi': 'Impossible de charger la liste des films',
  'Bu adımı geçebilirsin, sonra istediğini ekleyebilirsin.':
      'Tu peux passer cette étape et ajouter ce que tu veux plus tard.',
  '{} film izlediklerine eklendi': '{} films ajoutés à ton historique',
  'Film serileri': 'Sagas de films',
  'Bir seriyi baştan sona izlediysen tek dokunuşla ekle.':
      'Si tu as vu une saga du début à la fin, ajoute-la en une seule touche.',
  'Seri bulunamadı': 'Aucune saga trouvée',
  'Daha sonra tekrar deneyebilirsin.': 'Tu peux réessayer plus tard.',
  '{} film': '{} films',
  'Eklendi': 'Ajouté',
  'Tümünü izledim': 'Je les ai tous vus',
  'Peki ya diziler?': 'Et les séries ?',
  'İzlediklerini işaretle; seçtikçe sana benzeyenleri getireceğiz.':
      'Coche ce que tu as vu : au fil de tes choix, on te propose des séries similaires.',
  'Dizi listesi yüklenemedi': 'Impossible de charger la liste des séries',
  'Seçtiklerine benzeyenler': 'Similaires à tes choix',
  'Her şey hazır': 'Tout est prêt',
  "dizi.jpg'de neler yapabileceğine hızlıca göz at.":
      'Jette un œil rapide à ce que tu peux faire sur dizi.jpg.',
  'Profilini tamamla': 'Complète ton profil',
  "Ayarlar'dan bir bio, profil fotoğrafı ve kapak ekleyebilirsin.":
      'Depuis Paramètres, tu peux ajouter une bio, une photo de profil et une couverture.',
  'Takvimin seni bekliyor': "Ton calendrier t'attend",
  'İzlediğin dizilerin yeni bölümleri takvime düşer, bildirimle haber veririz.':
      "Les nouveaux épisodes de tes séries s'ajoutent au calendrier et on te prévient par notification.",
  'Puan ver, yorum yaz': 'Note et commente',
  'Bölüm bölüm puanla, spoiler perdesiyle yorum yaz, listeler oluştur.':
      'Note épisode par épisode, commente derrière un voile spoiler et crée des listes.',
  'Verilerin sende kalır': 'Tes données restent les tiennes',
  "Verilerin yalnız hesabına bağlı tutulur, satılmaz. Ayarlar'dan hepsini indirebilir ya da hesabını tamamen silebilirsin.":
      'Tes données sont conservées uniquement liées à ton compte et ne sont pas vendues. Depuis Paramètres, tu peux toutes les télécharger ou supprimer complètement ton compte.',
  'Engellenen kullanıcılar': 'Personnes bloquées',
  'Engellediğin kişileri gör, engeli kaldır':
      'Vois qui tu as bloqué et retire le blocage',
  'Engellediğin kimse yok': "Tu n'as bloqué personne",
  'Bir kullanıcıyı profilindeki üç nokta menüsünden engelleyebilirsin.':
      'Tu peux bloquer une personne depuis le menu à trois points de son profil.',
  '@{} engellensin mi?': 'Bloquer @{} ?',
  'Birbirinizin gönderilerini, yorumlarını ve profilini göremezsiniz; mesaj ve arama da gidemez. Varsa takip bağınız kopar ve engeli kaldırsan bile geri gelmez.':
      "Aucune des deux parties ne verra les publications, les commentaires ni le profil de l'autre ; les messages et les appels seront également impossibles. S'il existe un lien d'abonnement, il est rompu et ne revient pas, même si tu retires le blocage.",
  'Bu kullanıcıyı engelledin': 'Tu as bloqué cette personne',
  'Gönderilerini, yorumlarını ve mesajlarını görmüyorsun; o da seninkileri göremiyor.':
      'Tu ne vois pas les publications, les commentaires ni les messages de cette personne ; elle ne voit pas les tiens non plus.',
  '{} yeni bir yapımda: {}': '{} est dans une nouvelle production : {}',
  'Favori kişinden yeni yapım': "Nouvelle production d'une personne favorite",
  '{} için bildirimler': 'Notifications pour {}',
  'Tüm bildirimler': 'Toutes les notifications',
  'Uygulamada ve telefonda': "Dans l'appli et sur le téléphone",
  'Yalnız uygulama içi': "Dans l'appli uniquement",
  'Telefon bildirimi gönderilmez':
      "Aucune notification n'est envoyée sur le téléphone",
  'Kapalı': 'Désactivé',
  'Bu kişi için bildirim yok': 'Aucune notification pour cette personne',
  'Favori kişilerin yeni yapımları':
      'Nouvelles productions de tes personnes favorites',
  'Kapalıyken kimse seni sesli arayamaz; arayan "devre dışı" uyarısı görür':
      'Si désactivé, personne ne peut t\'appeler en vocal ; l\'appelant voit l\'avis "désactivés"',
  'Kapalıyken kimse seni görüntülü arayamaz; arayan "devre dışı" uyarısı görür':
      'Si désactivé, personne ne peut t\'appeler en vidéo ; l\'appelant voit l\'avis "désactivés"',

  // --- Hareketlerim / İstatistiklerim / gizlilik ---
  'Hareketlerim': 'Mon activité',
  'Beğenilerin, yorumların, izlemelerin tek akışta':
      'Tes j’aime, commentaires et visionnages au même endroit',
  'Henüz hareketin yok': 'Pas encore d’activité',
  'Beğendiğin, yorumladığın, izlediğin her şey burada birikir.':
      'Tout ce que tu aimes, commentes et regardes s’accumule ici.',
  'Bu süzgeçte hareket yok': 'Aucune activité dans ce filtre',
  'Başka bir tür seç ya da "Hepsi"ne dön.':
      'Choisis un autre type ou reviens à « Tout ».',
  'Hepsi': 'Tout',
  'Puan': 'Note',
  'Tepki': 'Réaction',
  'İzleme': 'Visionnage',
  'Durum': 'Statut',
  'Liste': 'Liste',
  'Yorum yaptın': 'Tu as commenté',
  'Yorumu beğendin': 'Tu as aimé un commentaire',
  '{} puan verdin': 'Tu as noté {}',
  'Tepki verdin': 'Tu as réagi',
  'Listene ekledin': 'Tu as ajouté à une liste',
  'Takip etmeye başladın': 'Tu as commencé à suivre',
  '"İzleyeceğim" işaretledin': 'Marqué « À voir »',
  '"İzliyorum" işaretledin': 'Marqué « Je regarde »',
  '"Bitirdim" işaretledin': 'Marqué « Terminé »',
  '"Bıraktım" işaretledin': 'Marqué « Abandonné »',
  'Silinmiş içerik': 'Contenu supprimé',
  'şimdi': 'à l’instant',
  '{} g': '{} j',
  '{} y': '{} a',
  'İstatistiklerim': 'Mes statistiques',
  'Gönderilerinin görüntülenmesi ve beğenisi':
      'Vues et j’aime de tes publications',
  'Henüz gönderin yok': 'Tu n’as pas encore de publication',
  'Bir dizi ya da filme yorum yazdığında görüntülenme ve beğeni sayıların burada birikmeye başlar.':
      'Dès que tu commentes une série ou un film, tes vues et j’aime commencent à s’accumuler ici.',
  'Tüm zamanlar': 'Depuis toujours',
  'Son {} gün': '{} derniers jours',
  'Görüntülenme geçmişi henüz birikmeye başlamadı; ilk gün verisi yarın görünür.':
      'L’historique des vues n’a pas encore commencé à s’accumuler ; les données du premier jour apparaîtront demain.',
  'Görüntülenme geçmişi {} tarihinden beri birikiyor ({} günlük veri var), bu yüzden {} günlük sayı henüz eksik.':
      'L’historique des vues s’accumule depuis le {} ({} jours de données), donc le total sur {} jours est encore incomplet.',
  'Beğeni geçmişi {} tarihinden beri kayıtlı.':
      'L’historique des j’aime est enregistré depuis le {}.',
  'Bu dönemde gösterilecek gönderi yok.':
      'Aucune publication à afficher pour cette période.',
  'eksik veri': 'données incomplètes',
  'Profilinde izlediğin dizi ve filmler, izleme sayaçların, ekran süren ve uyum kartın başkalarına görünmez':
      'Les séries et films que tu as vus, tes compteurs de visionnage, ton temps d’écran et ta carte d’affinité ne seront pas visibles par les autres sur ton profil',
  'Takipçi listemi gizle': 'Masquer ma liste d’abonnés',
  'Seni kimlerin takip ettiğini başkaları göremez; takipçi sayın görünmeye devam eder':
      'Les autres ne peuvent pas voir qui te suit ; ton nombre d’abonnés reste visible',
  'Takip ettiklerimi gizle': 'Masquer les personnes que je suis',
  'Kimleri takip ettiğini başkaları göremez; takip sayın görünmeye devam eder':
      'Les autres ne peuvent pas voir qui tu suis ; ton nombre d’abonnements reste visible',
  'Bu kullanıcı takipçilerini gizli tutmayı tercih ediyor.':
      'Cet utilisateur préfère garder ses abonnés privés.',
  'Bu kullanıcı takip ettiklerini gizli tutmayı tercih ediyor.':
      'Cet utilisateur préfère garder privées les personnes qu’il suit.',

  // --- md.23 gönderi istatistikleri ---
  'Gönderi istatistikleri': 'Statistiques de la publication',
  'Yalnız kendi gönderilerinin istatistiklerini görebilirsin.':
      'Tu ne peux voir que les statistiques de tes propres publications.',
  'Erişim': 'Portée',
  'Görüntüleyen': 'Spectateurs',
  'Kaç farklı kişi gördü (son {} gün)':
      'Combien de personnes différentes l’ont vu (derniers {} jours)',
  'Paylaşım': 'Partages',
  'Bu gönderiden sonra': 'Après cette publication',
  'Profil ziyareti': 'Visites du profil',
  'Yeni takip': 'Nouveaux abonnements',
  'İçeriğe tıklama': 'Clics sur le contenu',
  'Spoiler perdesini açan': 'Ont révélé le spoiler',
  'Paylaşım, profil ziyareti, takip, içeriğe tıklama, kaynak kırılımı ve spoiler ölçüleri {} tarihinden beri birikiyor; daha eskisi ölçülmedi.':
      'Les partages, visites du profil, abonnements, clics sur le contenu, la répartition par source et les mesures de spoiler s’accumulent depuis le {} ; ce qui précède n’a pas été mesuré.',
  'Zamana yayılmış görüntülenme': 'Vues dans le temps',
  'Görüntülenme nereden geldi': 'D’où viennent les vues',
  'Kimler gördü': 'Qui l’a vu',
  'Günlük veri henüz birikmeye başlamadı.':
      'Les données quotidiennes n’ont pas encore commencé à s’accumuler.',
  'Grafik için en az iki günlük veri gerekiyor. Veri {} tarihinden beri birikiyor.':
      'Le graphique nécessite au moins deux jours de données. Les données s’accumulent depuis le {}.',
  'En çok görüntülenme paylaşımdan sonraki {}. gün geldi: {}':
      'Le plus de vues est arrivé le {}ᵉ jour après la publication : {}',
  'Günlük görüntülenme {} tarihinden beri birikiyor; öncesi ölçülmedi.':
      'Les vues quotidiennes s’accumulent depuis le {} ; ce qui précède n’a pas été mesuré.',
  'Kaynak kırılımı yeni ölçülmeye başladı; bu gönderi henüz etiketli bir görüntülenme almadı.':
      'La répartition par source vient tout juste d’être mesurée ; cette publication n’a pas encore reçu de vue étiquetée.',
  'Takipçi/keşif kırılımı yeni ölçülmeye başladı; henüz veri yok.':
      'La répartition entre abonnés et découverte vient tout juste d’être mesurée ; il n’y a pas encore de données.',
  'Takip edenler': 'Abonnés',
  'Keşiften gelenler': 'Depuis Découvrir',
  'Reels': 'Reels',
  'Dizi/film sayfası': 'Page série/film',
  'Paylaşılan bağlantı': 'Lien partagé',
  'yeni ölçülüyor': 'mesure récente',
  'Kendi ortalamanla aynı düzeyde': 'Au niveau de ta propre moyenne',
  'Kendi ortalamanın %{} üstünde': '{} % au-dessus de ta propre moyenne',
  'Kendi ortalamanın %{} altında': '{} % en dessous de ta propre moyenne',
  'Etkileşim oranı': 'Taux d’engagement',
  '(beğeni + yorum) ÷ görüntülenme': '(j’aime + commentaires) ÷ vues',
  'Toplam görüntülenme': 'Vues totales',
  '{} ile {} arasında toplam {} görüntülenme':
      'Entre le {} et le {} : {} vues au total',
  'İstatistikleri gör': 'Voir les statistiques',

  // --- md.23 gizlilik politikası ---
  'Gönderi sahibine gösterilen istatistikler için, gönderinin kaç kez ve hangi yüzeyden (akış, profil, tam ekran akış, dizi/film sayfası, paylaşılan bağlantı) görüntülendiği, görüntüleyenin o an gönderi sahibini takip edip etmediği, gönderiden profile/içeriğe geçiş, gönderi üzerinden kurulan takip, paylaşım ve spoiler perdesi açılması toplu sayaçlar olarak tutulur. Bu sayaçlarda kullanıcı kimliği, IP adresi veya zaman damgası bulunmaz; kimin ne yaptığı sorgulanamaz.':
      'Pour les statistiques présentées à la personne propriétaire d’une publication, sont conservés sous forme de compteurs agrégés : combien de fois et depuis quelle surface (fil, profil, fil en plein écran, page série/film, lien partagé) la publication a été vue, si la personne qui l’a vue suivait alors la personne propriétaire, le passage de la publication au profil ou au contenu, les abonnements créés depuis la publication, les partages et l’ouverture du voile de spoiler. Ces compteurs ne contiennent ni identité d’utilisateur, ni adresse IP, ni horodatage ; il est impossible de savoir qui a fait quoi.',
  'Bir gönderiyi kaç farklı kişinin gördüğünü sayabilmek için, görüntüleyen başına geri çevrilemez bir anahtarlı özet (kullanıcı kimliğinden veya IP adresinden türetilen kriptografik kısaltma) 90 gün saklanır. Gönderi sahibine yalnız sayı gösterilir; görüntüleyenlerin kimliği hiçbir koşulda paylaşılmaz.':
      'Afin de pouvoir compter combien de personnes différentes ont vu une publication, un condensé à clé irréversible par personne l’ayant vue (une réduction cryptographique dérivée de l’identité d’utilisateur ou de l’adresse IP) est conservé pendant 90 jours. Seul le nombre est présenté à la personne propriétaire de la publication ; l’identité de celles qui l’ont vue n’est en aucun cas partagée.',

  // --- İzleyeceğim tekilliği (izleme silme onayı) ---
  'İzleyeceklerine taşınsın mı?': 'Déplacer vers « À voir » ?',
  'Bir içerik ya izlenecektir ya izlenmiştir. Devam edersen bu dizideki {} izleme işaretin silinecek.':
      'Un titre est soit à voir, soit vu. Si tu continues, tes {} marques de visionnage sur cette série seront supprimées.',
  'Bir içerik ya izlenecektir ya izlenmiştir. Devam edersen bu filmdeki "izledim" işaretin kaldırılacak.':
      'Un titre est soit à voir, soit vu. Si tu continues, ta marque « Vu » sur ce film sera retirée.',
  'Sil ve taşı': 'Supprimer et déplacer',

  // --- Yapım ekibi / firma + seviye unvanları + doğum günü ---
  'Yapım Ekibi': 'Équipe technique',
  'Yaratıcı': 'Création',
  'Yönetmen': 'Réalisation',
  'Senaryo': 'Scénario',
  'Yapımcı': 'Production',
  'Yapım Firmaları': 'Sociétés de production',
  'Yapım Firması': 'Société de production',
  'Firma bulunamadı': 'Société introuvable',
  'Bu firmanın bu türde listelenecek yapımı yok.':
      'Cette société n’a aucun titre de ce type.',
  'Doğum günün kutlu olsun!': 'Joyeux anniversaire !',
  'İyi ki doğdun, iyi ki buradasın.':
      'Ta naissance, c’était une bonne idée. Ta présence ici aussi.',
  'Bugün {} yaşına girdin': 'Aujourd’hui, tu as {} ans',
  'Teşekkürler': 'Merci',
  'İki Adımlı Doğrulama': 'Validation en deux étapes',
  'Girişte e-postana kod gönderilir':
      'Un code est envoyé à ton e-mail lors de la connexion',
  'Kod {} adresine gönderildi': 'Code envoyé à {}',
  '6 haneli kod': 'Code à 6 chiffres',
  'Doğrula': 'Vérifier',
  'Yeniden gönder': 'Renvoyer',
  'Vazgeç': 'Annuler',
  'Yeni kod gönderildi': 'Nouveau code envoyé',
  'Google ile girişte kod sorulmaz; Google kendi doğrulamasını yapar.':
      "Aucun code n'est demandé lors de la connexion avec Google ; Google effectue sa propre vérification.",
  'Kapatmak için de kod gerekir.': 'La désactivation demande aussi un code.',
  'E-postana erişemezsen hesabına giremezsin.':
      'Sans accès à ton e-mail, tu ne peux plus te connecter à ton compte.',
  'İki adımlı doğrulama açıldı': 'Validation en deux étapes activée',
  'İki adımlı doğrulama kapatıldı': 'Validation en deux étapes désactivée',
  'Videonun ne kadarı izlendi': 'Jusqu’où la vidéo est regardée',
  'Videoyu başlatanlar': 'Ont lancé la vidéo',
  'videonun %{} noktasına ulaştı': 'ont atteint {} % de la vidéo',
  'İzleyenlerin yarısı videonun en az %{} kadarını gördü.':
      'La moitié du public a vu au moins {} % de la vidéo.',
  'Sonuna kadar izleyen: %{}': 'Vu jusqu’à la fin : {} %',
  'Eğri için en az {} izlenme gerekiyor; şu an {} izlenme var.':
      'La courbe nécessite au moins {} lectures ; il y en a {} pour le moment.',
  'Eğri {} izlenmeden çıkarıldı ve seçili zaman aralığından etkilenmez.':
      'La courbe est basée sur {} lectures et ne dépend pas de la période choisie.',
  'İzlenme süresi {} tarihinden beri ölçülüyor; daha eski görüntülenmeler bu sayıya girmez.':
      'La durée de visionnage est mesurée depuis le {} ; les vues plus anciennes ne comptent pas.',
  '{} izlenme. Videonun yarısına ulaşan %{}, sonuna ulaşan %{}.':
      '{} lectures. {} % ont atteint la moitié, {} % la fin.',
  // Yüzde işaretinin YERİ dile göre değişir; sayı tek başına
  // basılamaz (bkz. gonderi_istatistik.dart). Kalıp bu dosyanın
  // kendi mevcut yüzde çevirilerinden çıkarıldı.
  '%{}': '{} %',
  'Seviye {}': 'Niveau {}',
  'Sonraki seviyeye {} puan kaldı': '{} points avant le niveau suivant',
  'Gönderilerin': 'Tes publications',
  '{} gönderi': '{} publications',
  '{} gönderi · {} görüntülenme': '{} publications · {} vues',
  'önceki {} güne göre': 'vs {} jours précédents',
  'önceki {} güne göre %{} arttı':
      'par rapport aux {} jours précédents, hausse de {} %',
  'önceki {} güne göre %{} azaldı':
      'par rapport aux {} jours précédents, baisse de {} %',
  'önceki {} güne göre değişmedi':
      'sans changement par rapport aux {} jours précédents',
  'Günlük görüntülenme: en düşük {}, en yüksek {}':
      'Vues quotidiennes : minimum {}, maximum {}',
  'Etkileşim oranı henüz ölçülemedi': 'Taux d’engagement pas encore mesurable',
  'Oranlar': 'Taux',
  'Beğenme oranı': 'Taux de j’aime',
  'Yorum oranı': 'Taux de commentaires',
  'Paylaşma oranı': 'Taux de partages',
  'İçeriğe gitme oranı': 'Taux de clics contenu',
  'Profile gitme oranı': 'Taux de visites profil',
  'Ziyaretten takibe dönüşüm': 'Visite vers abonnement',
  'beğeni ÷ görüntülenme': 'j’aime ÷ vues',
  'yorum ÷ görüntülenme': 'commentaires ÷ vues',
  'paylaşım ÷ görüntülenme': 'partages ÷ vues',
  'içeriğe tıklama ÷ görüntülenme': 'clics sur le contenu ÷ vues',
  'profil ziyareti ÷ görüntülenme': 'visites du profil ÷ vues',
  'yeni takip ÷ profil ziyareti': 'nouveaux abonnements ÷ visites du profil',
  'Oran için en az {} görüntülenme gerekiyor; şu an {} var.':
      'Les taux nécessitent au moins {} vues ; il y en a {} pour le moment.',
  'Oranlar gönderinin ömür boyu sayılarından çıkar; aşağıdaki gün seçimi bunları değiştirmez.':
      'Les taux proviennent des totaux depuis la publication ; le choix des jours ci-dessous ne les modifie pas.',
  'Bu sayılar gönderinin ömür boyu toplamıdır (kendi notu olan kutular hariç); aşağıdaki gün seçimi yalnız grafiği değiştirir.':
      'Ces chiffres sont les totaux depuis la publication (sauf les blocs qui ont leur propre note) ; le choix des jours ci-dessous ne modifie que le graphique.',
  'Ortalama izlenme: videonun %{} kadarı':
      'Visionnage moyen : {} % de la vidéo',
  'Elde tutma eğrisinin 20 kovasının ortalaması.':
      'La moyenne des 20 segments de la courbe.',
  'Dizi, film, kişi veya şirket ara...':
      'Cherche une série, un film, une personne ou une société...',
  'Şirketler': 'Sociétés',
  'Senarist': 'Scénariste',
  'Bölüm puanları': 'Notes des épisodes',
  'S{}': 'S{}',
  'E{}': 'E{}',
  'Bölüm puanları yüklenemedi': 'Impossible de charger les notes',
  'Giriş gerekli': 'Connexion requise',
  'Geçersiz oturum': 'Session invalide',
  'Oturum sonlandı, tekrar giriş yap': 'Session terminée, reconnecte-toi',
  'Çok fazla istek; biraz sonra tekrar dene':
      'Trop de requêtes ; réessaie un peu plus tard',
  'Sunucuda geçici olarak yer yok, birazdan tekrar dene':
      'Plus d’espace sur le serveur pour l’instant, réessaie dans un moment',
  'Devam eden yapımlar ({})': 'Productions en cours ({})',
  'Diziler ({})': 'Séries ({})',
  'Filmler ({})': 'Films ({})',
  'Tüm yapımlar': 'Toutes les productions',
  'Medya alanın doldu; yer açmak için eski yüklemelerini sil':
      'Ton espace média est plein ; supprime d’anciens envois pour libérer de la place',
  'E-posta/kullanıcı adı veya şifre hatalı':
      'E-mail/identifiant ou mot de passe incorrect',
  'Bu e-posta veya kullanıcı adı zaten kayıtlı':
      'Cet e-mail ou identifiant est déjà pris',
  'Google doğrulaması başarısız': 'Échec de la vérification Google',
  'Kod geçersiz veya süresi dolmuş': 'Code invalide ou expiré',
  'Bu kullanıcıyla mesajlaşamazsın': 'Tu ne peux pas écrire à ce compte',
  'Kendini takip edemezsin': 'Tu ne peux pas te suivre toi-même',
  'Bu kullanıcıyı takip edemezsin': 'Tu ne peux pas suivre ce compte',
  'Kendini engelleyemezsin': 'Tu ne peux pas te bloquer toi-même',
  'Yorum 1-1000 karakter olmalı': 'Le commentaire doit faire 1–1000 caractères',
  'Boş mesaj gönderilemez': 'Impossible d\'envoyer un message vide',
  'Yorum bulunamadı': 'Commentaire introuvable',
  'Şifre hatalı': 'Mot de passe incorrect',
  'Şifre en az 6 karakter olmalı':
      'Le mot de passe doit faire au moins 6 caractères',
  'Yalnızca GIF, PNG, JPEG veya WebP yüklenebilir':
      'Seuls GIF, PNG, JPEG ou WebP sont acceptés',
  'Desteklenen türler: GIF, PNG, JPEG, WebP, MP4, WebM, ses':
      'Types acceptés : GIF, PNG, JPEG, WebP, MP4, WebM, audio',
  'Geçerli bir ZIP dosyası değil': 'Ce n\'est pas un fichier ZIP valide',
  'Verilerini e-postayla almak için önce hesabına e-posta bağlamalısın':
      'Pour recevoir tes données par e-mail, lie d\'abord une adresse',
  'Bu içerikte izleme kaydın var. "İzleyeceğim" demek için önce izleme işaretlerin silinmeli.':
      'Tu as déjà des marques de visionnage. Pour « Je vais regarder », il faut d’abord les effacer.',
  'Arama şu anda kapalı': 'Les appels sont coupés pour le moment',
  'Misafir hesaplar arama ayarlarını açamaz; hesap oluşturunca kullanabilirsin':
      'Les comptes invités ne peuvent pas activer les appels ; crée un compte',
  'Sunucu yeniden başlatılıyor, birazdan tekrar dene':
      'Le serveur redémarre, réessaie dans un instant',
  'Puan 1-10 arası olmalı': 'La note doit être entre 1 et 10',
  'En fazla 10 medya eklenebilir': 'Tu peux ajouter 10 médias au plus',
  'Bu cihazdan yeni hesap açılamıyor':
      'Impossible de créer un compte depuis cet appareil',
  'Geçerli e-posta, kullanıcı adı ve en az 6 karakter şifre gerekli':
      'Il faut un e-mail valide, un identifiant et un mot de passe d\'au moins 6 caractères',
  'Geçerli e-posta ve en az 6 karakter şifre gerekli':
      'Il faut un e-mail valide et un mot de passe d\'au moins 6 caractères',
  'Kullanıcı adı 3-20 karakter; küçük harf, rakam, nokta, tire ve alt çizgi kullanılabilir (başta/sonda nokta-tire olamaz)':
      'L\'identifiant doit faire 3–20 caractères ; minuscules, chiffres, points, tirets et underscores (pas au début ni à la fin)',
  'Bu hesap zaten bağlı': 'Ce compte est déjà lié',
  'Hesap oluşturulamadı': 'Impossible de créer le compte',
  'Misafir hesabı oluşturulamadı': 'Impossible de créer un compte invité',
  'Hesap bulunamadı': 'Compte introuvable',
  'Bu hesapta e-posta yok': 'Ce compte n\'a pas d\'e-mail',
  'İki adımlı doğrulama zaten bu durumda':
      'La validation en deux étapes est déjà dans cet état',
  'Kod yanlış': 'Code incorrect',
  'Kodun süresi doldu': 'Le code a expiré',
  'Kullanıcının e-postası yok (misafir hesap)':
      'Cet utilisateur n\'a pas d\'e-mail (compte invité)',
  'Bio en fazla 300 karakter olabilir':
      'La bio peut faire 300 caractères au plus',
  'Geçersiz ülke': 'Pays invalide',
  'Geçersiz sosyal bağlantı': 'Lien social invalide',
  'Geçersiz doğum tarihi': 'Date de naissance invalide',
  'dizi.jpg için en az 13 yaşında olmalısın.':
      'Tu dois avoir au moins 13 ans pour utiliser dizi.jpg.',
  'En az 13 yaşında olmalısın': 'Tu dois avoir au moins 13 ans',
  'Liste adı gerekli': 'Le nom de la liste est requis',
  'Ad en fazla 60, açıklama 300 karakter olabilir':
      'Le nom peut faire 60 caractères, la description 300',
  'Liste bulunamadı': 'Liste introuvable',
  'Değiştirilecek tercih yok': 'Rien à mettre à jour',
  'İnceleme en fazla 2000 karakter olabilir':
      'L\'avis peut faire 2000 caractères au plus',
  'Önce içeriği "bitirdim" olarak işaretle':
      'Marque d\'abord le titre comme terminé',
  'Önce kişiyi favorile': 'Ajoute d\'abord la personne en favori',
  'Bölüm yorumu için sezon ve bolum birlikte gerekli':
      'Pour un commentaire d\'épisode, saison et épisode sont tous deux requis',
  'Yanıtlanan yorum bulunamadı': 'Commentaire auquel tu réponds introuvable',
  'Geri bildirim 3-2000 karakter olmalı':
      'Le retour doit faire 3–2000 caractères',
  'Başlık ve metin gerekli (en az 2 karakter)':
      'Le titre et le texte sont requis (au moins 2 caractères)',
  'Kendine mesaj gönderemezsin': 'Tu ne peux pas t\'envoyer un message',
  'Mesaj en fazla 2000 karakter olabilir':
      'Le message peut faire 2000 caractères au plus',
  'Mesaj 1-2000 karakter olmalı': 'Le message doit faire 1–2000 caractères',
  'Mesaj bulunamadı': 'Message introuvable',
  'Mesaj bulunamadı veya düzenlenemez': 'Message introuvable ou non modifiable',
  'Yalnız sana gelen mesajı şikayet edebilirsin':
      'Tu ne peux signaler qu\'un message qui t\'a été envoyé',
  'Sebep gerekli': 'Le motif est requis',
  'İtiraz edilecek aktif bir ceza yok':
      'Il n\'y a pas de sanction active à contester',
  'Zaten incelenmeyi bekleyen bir itirazın var':
      'Tu as déjà un recours en attente d\'examen',
  'Bu ceza için itirazın zaten incelendi':
      'Ton recours pour cette sanction a déjà été examiné',
  'Medya bağlantısının süresi dolmuş': 'Le lien média a expiré',
  'Bu medya için imzalı bağlantı gerekli':
      'Un lien signé est requis pour ce média',
  'Arama hizmeti yeniden başlatılıyor, birazdan tekrar dene':
      'Le service d\'appels redémarre ; réessaie un peu plus tard',
  'Puan göstergesi': 'Légende des notes',
  'İzleme İstatistiklerim': 'Mes statistiques de visionnage',
  'Bir bölümü ya da filmi izlendi olarak işaretlediğinde istatistiklerin burada birikmeye başlar.':
      'Dès que tu marques un épisode ou un film comme vu, tes statistiques commencent à s’accumuler ici.',
  '{} sa {} dk': '{} h {} min',
  'Yaklaşık ekran süresi': 'Temps d’écran approximatif',
  'İzleme serisi': 'Série de visionnage',
  'Şu anki seri': 'Série actuelle',
  'En uzun seri': 'Plus longue série',
  'Haftanın günleri': 'Jours de la semaine',
  'En çok izlediklerin': 'Les plus regardés',
  'Bu dönemde izleme yok': 'Aucun visionnage sur cette période',
  'Paz': 'Dim',
  'Pzt': 'Lun',
  'Sal': 'Mar',
  'Çar': 'Mer',
  'Per': 'Jeu',
  'Cum': 'Ven',
  'Cmt': 'Sam',
  '{} bölüm': '{} épisodes',
  '{} bölüm~tekil': '{} épisode',
  '{} kez': '{} fois',
  '{} kez~tekil': '{} fois',
  'Son {} gün · günlük': '{} derniers jours · par jour',
  'Son {} gün · haftalık': '{} derniers jours · par semaine',
  'Ekran süren, serin ve en çok izlediklerin':
      'Ton temps d’écran, ta série et tes favoris',
  'Daralt': 'Réduire',
  'Listeyi düzenle': 'Modifier la liste',
  'Bitti': 'Terminé',
  'Sırala': 'Réordonner',
  'Herkese göster': 'Montrer à tout le monde',
  'Başkalarından gizle': 'Masquer aux autres',
  'Listeden kaldır': 'Retirer de la liste',
  'Listeden kaldırılsın mı?': 'Retirer de la liste ?',
  'Bu yapım listeden çıkarılacak. Gizlemek istersen göz simgesini kullanabilirsin.':
      'Ce titre sera retiré de la liste. Si tu veux seulement le masquer, utilise l’icône en forme d’œil.',
  'Sıralama kaydedilemedi': 'La réorganisation n’a pas pu être enregistrée',
  'Değişiklik kaydedilemedi': 'La modification n’a pas pu être enregistrée',
  'Kaldırılamadı': 'Impossible de retirer',
  'Popülerlik': 'Popularité',
  'TMDB puanı': 'Note TMDB',
  'Yapım yılı': 'Année de production',
  'dizi.jpg puanı': 'Note dizi.jpg',
  'İzlenme': 'Vues',
  'Yorum sayısı': 'Nombre de commentaires',
  'Tüm raflara uygulanır': 'S\'applique à toutes les étagères',
  'En popüler {} yapım arasında sıralandı':
      'Trié parmi les {} titres les plus populaires',
  'Altyazı görünümü': 'Apparence des sous-titres',
  'Renk, yazı tipi, boyut, ayrıt ve opaklık':
      'Couleur, police, taille, bord et opacité',
  'Bu bir örnek altyazıdır.': 'Ceci est un exemple de sous-titre.',
  'Ayar değiştikçe önizleme anında güncellenir.':
      'L\'aperçu se met à jour instantanément à chaque changement.',
  'Yazı biçimi': 'Style du texte',
  'Yazı rengi': 'Couleur du texte',
  'Yazı opaklığı': 'Opacité du texte',
  'Yazı tipi': 'Police',
  'Yazı boyutu': 'Taille du texte',
  'Karakter ayrıtı': 'Bord des caractères',
  'Ayrıt türü': 'Type de bord',
  'Ayrıt rengi': 'Couleur du bord',
  'Yok': 'Aucun',
  'Dış çizgi': 'Contour',
  'Gölge': 'Ombre portée',
  'Kabartma': 'En relief',
  'Oyma': 'En creux',
  'Arka plan rengi': 'Couleur d\'arrière-plan',
  'Arka plan opaklığı': 'Opacité de l\'arrière-plan',
  'Arka plan yazının hemen arkasındaki dolgudur; pencere ise tüm altyazı bloğunun arkasındaki daha geniş yüzey.':
      'L\'arrière-plan est le fond juste derrière le texte ; la fenêtre est la surface plus large derrière tout le bloc de sous-titres.',
  'Pencere': 'Fenêtre',
  'Pencere rengi': 'Couleur de la fenêtre',
  'Pencere opaklığı': 'Opacité de la fenêtre',
  'Altyazı görünümü sıfırlansın mı?':
      'Réinitialiser l\'apparence des sous-titres ?',
  'Renk, yazı tipi, boyut, ayrıt ve opaklık ayarları varsayılana döner.':
      'Les réglages de couleur, police, taille, bord et opacité reviendront aux valeurs par défaut.',
  'Altyazı görünümü varsayılana döndü':
      'Apparence des sous-titres réinitialisée',
  '{} indiriliyor…': 'Téléchargement de {}…',
  'Seçince indirilir': 'Téléchargée à la sélection',
  'Yazı tipi indirilemedi. Varsayılan yazı tipiyle gösteriliyor.':
      'Impossible de télécharger la police. Affichage avec la police par défaut.',
  'Beyaz': 'Blanc',
  'Siyah': 'Noir',
  'Kırmızı': 'Rouge',
  'Yeşil': 'Vert',
  'Mavi': 'Bleu',
  'Sarı': 'Jaune',
  'Camgöbeği': 'Cyan',
  'Macenta': 'Magenta',
  'Yapım bütçesi': 'Budget de production',
  'Yapım bütçesi ve dünya çapında hasılat':
      'Budget de production et recettes mondiales',
  'Dizinin yayın durumu': 'Statut de diffusion de la série',
  'Devam ediyor': 'En cours',
  'Sona erdi': 'Terminée',
  'İptal edildi': 'Annulée',
  'Yapımda': 'En production',
  'Planlandı': 'Prévue',
  'Pilot bölüm': 'Épisode pilote',
  'Ad': 'Nom',
  'Adın': 'Ton nom',
  'Kullanıcı adı': "Nom d'utilisateur",
  'Kullanıcı adını değiştir': "Changer de nom d'utilisateur",
  'Kullanıcı adını 90 günde bir değiştirebilirsin':
      "Tu peux changer de nom d'utilisateur une fois tous les 90 jours",
  'Değiştir': 'Changer',
  '{} gün sonra değiştirebilirsin': 'Tu pourras le changer dans {} jours',
  'Kullanıcı adın: @{}': "Ton nom d'utilisateur : @{}",
  'Bu kullanıcı adı zaten alınmış': "Ce nom d'utilisateur est déjà pris",
  'Bu kullanıcı adı şu an başka bir hesaba ayrılmış':
      "Ce nom d'utilisateur est actuellement réservé à un autre compte",
  'Bu zaten senin kullanıcı adın': "C'est déjà ton nom d'utilisateur",
  'Kullanıcı adı 3-20 karakter; küçük harf, rakam, nokta, tire ve alt çizgi kullanılabilir':
      "L'identifiant doit faire 3–20 caractères ; minuscules, chiffres, points, tirets et underscores",
  'Profilinde kullanıcı adının üstünde görünür. Boş bırakabilirsin.':
      "Affiché au-dessus de ton nom d'utilisateur sur ton profil. Tu peux le laisser vide.",
  'Değiştirdikten sonra 90 gün boyunca tekrar değiştiremezsin. Eski kullanıcı adın 90 gün boyunca sana ayrılır — istersen geri dönebilirsin, o sürede başkası alamaz. Eski adına giden bağlantılar artık profilini açmaz.':
      "Après le changement, tu ne pourras plus en changer pendant 90 jours. Ton ancien nom d'utilisateur te reste réservé 90 jours — tu peux y revenir et personne d'autre ne peut le prendre pendant ce temps. Les liens vers ton ancien nom n'ouvriront plus ton profil.",
  'Etkinliğim': 'Activité',
  'Tercihler': 'Préférences',
  'Gizlilik ve güvenlik': 'Confidentialité et sécurité',
  'Destek': 'Assistance',
  'Hesap': 'Compte',
  '{} dizileri': 'Séries de {}',
  '{} filmleri': 'Films de {}',
  'En üste taşı': 'Placer en tête',
  'Sırayı sıfırla': 'Réinitialiser l’ordre',
  'Listede ara': 'Rechercher dans la liste',
  'Elle sıra sıfırlansın mı?': 'Réinitialiser l’ordre manuel ?',
  'Liste varsayılan sırasına (en son işaretlediğin önce) döner.':
      'La liste revient à son ordre par défaut (le plus récemment marqué en premier).',
  'Sıra sıfırlanamadı': 'Impossible de réinitialiser l’ordre',
  'Listenin en üstüne taşındı': 'Placé en tête de liste',
  'Adlar yükleniyor…': 'Chargement des titres…',
  'Afişe basılı tutup sürükle. Uzaktaki bir yapımı öne almak için "En üste taşı"yı kullan.':
      'Appuie longuement sur une affiche pour la déplacer. Pour faire remonter un titre éloigné, utilise « Placer en tête ».',
  'Aramada sürükleme kapalı; "En üste taşı" ile öne al ({} sonuç).':
      'Le glisser-déposer est désactivé dans la recherche ; utilise « Placer en tête » pour le faire remonter ({} résultats).',
  'Görünüm': 'Affichage',
  'En çok izlediğin filmler': 'Tes films les plus vus',
  'Süreler tahmindir: bölüm ~{} dk, film ~{} dk sayılır':
      'Les durées sont estimées : un épisode compte ~{} min, un film ~{} min',
  'Bu kullanıcı adı sistem tarafından ayrılmış':
      "Ce nom d'utilisateur est réservé par le système",
  'Ekran süresi': 'Temps d’écran',
  "Süreler TMDB'deki gerçek bölüm ve film süreleridir":
      'Les durées sont les durées réelles des épisodes et des films sur TMDB',
  'Tüm zamanlar: {} bölüm · {} film · {}':
      'Depuis toujours : {} épisodes · {} films · {}',
  'Kabul et': 'Accepter',
  'Reddedilenler': 'Refusées',
  'İstekler': 'Demandes',
  'Reddettiğin istek yok': 'Aucune demande refusée',
  'Reddettiğin istekler burada durur; dilersen geri kabul edebilirsin.':
      'Les demandes refusées restent ici ; tu peux les accepter à nouveau quand tu veux.',
  'Ne izlesem?': 'Je regarde quoi ?',
  'Karışık': 'Mixte',
  'Çarkı çevir': 'Tourner la roue',
  'Tekrar çevir': 'Retourner la roue',
  'Bu türde içerik yok': 'Aucun titre de ce type',
  'Bütçe': 'Budget',
  'dizi.jpg AI özeti': 'Résumé IA dizi.jpg',
  'Daha fazlasını gör': 'Voir plus',
  'Yıldız sistemi': 'Système d\'étoiles',
  '{} yıldız üzerinden': 'Sur {} étoiles',
  'Önizleme': 'Aperçu',
  'dokununca açılır': 's\'ouvre au toucher',
  'Bir azalt': 'Diminuer de un',
  'Bir artır': 'Augmenter de un',
  'Ölçek kaydedilemedi': 'Échelle non enregistrée',
  'Ölçeği değiştirmek puanlarını silmez; aynı puanlar yeni ölçekte gösterilir.':
      'Changer d\'échelle ne supprime pas vos notes ; elles sont affichées sur la nouvelle échelle.',
  'En aşağıya gönder': 'Envoyer en bas',
  'Listenin en altına taşındı': 'Déplacé en bas de la liste',
  'Oturumun sonlandı': 'Votre session a pris fin',
  'Bu hesabın oturumları kapatıldı ya da şifresi değişti. Devam etmek için tekrar giriş yap.':
      'Ce compte a été déconnecté partout ou son mot de passe a changé. Reconnectez-vous pour continuer.',
  'Son izleme: {}': 'Vu pour la dernière fois : {}',
  '{} tarihinde izledin': 'Vu le {}',
  'Geri bildirimine yanıt verdik': 'Nous avons répondu à ton retour',
  'Bu yanıt artık görüntülenemiyor.': 'Cette réponse n\'est plus disponible.',
  'Gönderdiğin geri bildirim': 'Le retour que tu as envoyé',
  'Dizi veya film seç (zorunlu)': 'Choisis une série ou un film (obligatoire)',
  'Bölüm seç (isteğe bağlı)': 'Choisis un épisode (facultatif)',
  '{}. sezon {}. bölüm': 'Saison {}, épisode {}',
  'Ne düşünüyorsun?': 'Qu\'en penses-tu ?',
  '{}/{} yükleniyor': 'Envoi {}/{}',
  'Fotoğraf/video ekle': 'Ajouter une photo ou une vidéo',
  'Paylaşmak için önce bir dizi veya film seç.':
      'Choisis d\'abord une série ou un film à partager.',
  'Sezon seç': 'Choisis une saison',
  '{}. sezon': 'Saison {}',
  'Bölüm seçme (yalnız dizi hakkında)': 'Aucun épisode (à propos de la série)',
  'Sezonlar yüklenemedi.': 'Impossible de charger les saisons.',
  'Bu dizide bölüm bilgisi yok.':
      'Aucune information d\'épisode pour cette série.',
  'Bu sezonda bölüm yok.': 'Aucun épisode dans cette saison.',
  '{}. bölüm': 'Épisode {}',
  'Dizi veya film ara...': 'Rechercher séries et films...',
  'İzlediğin dizi ve film hakkında yorum paylaş':
      'Partage un commentaire sur ce que tu as regardé',
  'Yapım seç (zorunlu)': 'Choisis un titre (obligatoire)',
  'Paylaşmak için önce bir yapım seç.': 'Choisis d\'abord un titre à partager.',
  'Kişi': 'Personne',
  'Yapım firması': 'Société de production',
  'Yapım ara...': 'Rechercher des titres...',
  'Çarkı çevirmek için sürükle veya dokun':
      'Fais glisser ou touche pour tourner la roue',
  'Arşiv': 'Archive',
  'Yüklediklerim': 'Mes envois',
  'GIF ara...': 'Rechercher des GIF...',
  'GIF yükle': 'Envoyer un GIF',
  'GIF etiketleri': 'Étiquettes du GIF',
  'Virgülle ayır. Aramada bu kelimelerle bulunur.':
      'Séparez par des virgules. Ces mots servent à le retrouver.',
  'gülme, alkış, şaşkın': 'rire, applaudissements, surpris',
  'Yükle': 'Envoyer',
  'Henüz GIF yok': 'Pas encore de GIF',
  'Bu aramaya uyan GIF yok': 'Aucun GIF ne correspond à cette recherche',
  'Arşiv yeni. İlk GIF i sen yükle — sen hemen kullanırsın, onaydan sonra herkes görür.':
      'L\'archive est nouvelle. Envoie le premier GIF — tu peux l\'utiliser tout de suite, tout le monde le verra après validation.',
  'GIF yüklendi. Sen hemen kullanabilirsin; herkese açık arşive onaydan sonra girer.':
      'GIF envoyé. Tu peux l\'utiliser tout de suite ; il rejoint l\'archive publique après validation.',
  'Onay bekliyor': 'En attente de validation',
  'GIF listesi yüklenemedi': 'Impossible de charger la liste des GIF',
  'GIF yüklenemedi': 'Impossible d\'envoyer le GIF',
  'Bu GIF i şikayet et': 'Signaler ce GIF',
  'Uygunsuz içerik yöneticiye bildirilir ve arşivden kaldırılabilir.':
      'Le contenu inapproprié est signalé à un modérateur et peut être retiré de l\'archive.',
  'Şikayetin alındı': 'Ton signalement a été reçu',
  'Şikayet gönderilemedi': 'Le signalement n\'a pas pu être envoyé',
  'Dizinin kendisi': 'La série elle-même',
  'Sezon ya da bölüm seçme': 'Ni saison ni épisode',
  'Tüm {}. sezon': 'Toute la saison {}',
  'Bölüm seçme': 'Aucun épisode',
  'En fazla {} yapım ekleyebilirsin.': 'Tu peux ajouter jusqu’à {} titres.',
  'Bu zaten ekli.': 'Déjà ajouté.',
  'Dizi, film, oyuncu ya da yönetmen etiketle — gönderin onların sayfasında da görünür.':
      'Identifie une série, un film, un acteur ou un réalisateur — ton post apparaîtra aussi sur leur page.',
  'Etiket ekle': 'Ajouter un tag',
  'Dizi, film, oyuncu ya da yönetmen etiketle':
      'Identifier une série, un film, un acteur ou un réalisateur',
  'Gönderiden vazgeç': 'Abandonner le post',
  'Yazdıkların kaydedilmeyecek.': 'Ce que tu as écrit ne sera pas enregistré.',
  'Yazmaya devam et': 'Continuer à écrire',
  'Bulunamadı. Adın yazılışını değiştirip dene.':
      'Introuvable. Essaie une autre orthographe.',
  'Dizi, film, oyuncu, yönetmen ya da yapım firması ara.':
      'Cherche une série, un film, un acteur, un réalisateur ou un studio.',
  'E-posta adresini değiştir': 'Changer ton adresse e-mail',
  'E-posta bağlı değil': 'Aucune adresse e-mail liée',
  'Geçerli bir e-posta adresi yaz': 'Saisis une adresse e-mail valide',
  'Bu zaten hesabının e-posta adresi':
      'C\'est déjà l\'adresse e-mail de ton compte',
  'Kod gönder': 'Envoyer le code',
  'E-posta adresin güncellendi': 'Ton adresse e-mail a été mise à jour',
  'Bu e-posta başka bir hesapta kayıtlı':
      'Cet e-mail est enregistré sur un autre compte',
  'Doğrulama kodu YENİ adrese gönderilir; adres ancak kodu girdikten sonra değişir. Şifre sıfırlama ve iki adımlı giriş kodları bundan sonra yeni adrese gider.':
      'Le code de vérification est envoyé à la NOUVELLE adresse ; l\'adresse ne change qu\'après la saisie du code. Les codes de réinitialisation du mot de passe et de connexion en deux étapes iront ensuite à la nouvelle adresse.',
  'Hesabındaki e-posta adresi geçerli değil, bu yüzden e-posta teslim edilemiyor. Doğru adresi bağlamak için destekle iletişime geç.':
      'L\'adresse e-mail de ton compte n\'est pas valide, l\'e-mail ne peut donc pas être remis. Contacte le support pour lier la bonne adresse.',
  'Bu adrese kod gönderilemedi, adresi kontrol et':
      'Impossible d\'envoyer un code à cette adresse, vérifie-la',
  'ya da': 'ou',
  'Google ile doğrula': 'Vérifier avec Google',
  'Apple ile doğrula': 'Vérifier avec Apple',
  'Hesabını Google ya da Apple ile açtıysan şifren yoktur; kimliğini o hesapla doğrula.':
      'Si tu as créé ton compte avec Google ou Apple, tu n’as pas de mot de passe ; vérifie ton identité avec ce compte.',
  'Hesabını Google ile açtıysan şifren yoktur; kimliğini Google ile doğrula.':
      'Si tu as créé ton compte avec Google, tu n\'as pas de mot de passe ; vérifie ton identité avec Google.',
  'Google doğrulaması bu hesapla eşleşmedi':
      'La vérification Google ne correspond pas à ce compte',
  'Bu hesap Google ile açıldı; doğrulamak için Google ile giriş yap':
      'Ce compte a été créé avec Google ; connecte-toi avec Google pour vérifier',
  // --- 31 Agu 2026: kitaplik paylasimi + reels ceviri + sohbet detayi ---
  'Bu liste gizli': 'Cette liste est privée',
  'Bu kullanıcı izleme listelerini gizli tutmayı tercih ediyor.':
      'Cet utilisateur préfère garder ses listes privées.',
  'Bu listede henüz içerik yok': "Aucun titre dans cette liste pour l'instant",
  'Otomatik çeviri': 'Traduction automatique',
  'Otomatik çeviri açıldı': 'Traduction automatique activée',
  'Otomatik çeviri kapatıldı': 'Traduction automatique désactivée',
  'Sohbet sessize alındı': 'Discussion mise en sourdine',
  'Sessize alma kaldırıldı': 'Sourdine désactivée',
  'Tema özelleştir': 'Personnaliser le thème',
  'Sohbette ara': 'Rechercher dans la discussion',
  'Mesajlarda ara...': 'Rechercher dans les messages...',
  'Sonuç yok': 'Aucun résultat',
  'Profili gör': 'Voir le profil',
  'Medya ve dosyalar': 'Médias et fichiers',
  'Henüz medya yok': "Aucun média pour l'instant",
  'Varsayılan': 'Par défaut',
  'Mor': 'Violet',
  'Pembe': 'Rose',
  'Turuncu': 'Orange',
  'Orijinal metin gösteriliyor': 'Texte original affiché',
  'İnternet bağlantısı yok': 'Aucune connexion Internet',
  'Sunucu yanıt vermedi': 'Le serveur n’a pas répondu',
  'Güvenli bağlantı kurulamadı': 'Impossible d’établir une connexion sécurisée',
  'Sunucudan beklenmeyen bir yanıt geldi': 'Réponse inattendue du serveur',
  'İzlediğim Diziler': 'Séries vues',
  'İzlediğim Filmler': 'Films vus',
  'Satır görünümü': 'Vue liste',
  'Afiş görünümü': 'Vue affiches',
  'Satır görünümünde sürükleme kapalı; "En üste taşı" ile öne al.':
      'Le glisser-déposer est désactivé en vue liste ; utilise « Placer en tête » pour le remonter.',
  'Spoiler uyarısını göster': 'Afficher l\'avertissement de spoiler',
  'Kapalıyken spoiler işaretli gönderiler perdesiz, doğrudan gösterilir.':
      'Si désactivé, les publications marquées comme spoiler s\'affichent directement, sans voile.',
  'Etiketler': 'Identifications',
  'Galeri': 'Galerie',
  'Kamera': 'Caméra',
  'Dosya': 'Fichier',
  'GIF': 'GIF',
  'Dizi / Film': 'Série / Film',
  'Sesli mesaj (basılı tut)': 'Message vocal (maintenir)',
  'Kaydetmek için basılı tut': 'Maintenez pour enregistrer',
  'Kaydırarak iptal': 'Glissez pour annuler',
  'Kamera açılamadı': 'Impossible d’ouvrir la caméra',
  'Dosya seçilemedi': 'Impossible de sélectionner le fichier',
  'Dosya gönderilemedi': 'Impossible d’envoyer le fichier',
  'En yeni mesaja in': 'Aller au dernier message',
  'Gönderilemedi · tekrar dene': 'Non envoyé · réessayer',
  'Dosya açılamadı': 'Impossible d’ouvrir le fichier',
  'Temizle': 'Effacer',
  'Oyuncu': 'Acteur',
  'Böyle bir oda yok': "Cette salle n'existe pas",
  'Bu oda kapandı': 'Cette salle est fermée',
  'Bu odaya girmek için davet ya da oda kodu gerekli':
      'Il te faut une invitation ou un code pour entrer',
  'Oda dolu': 'La salle est pleine',
  'Bu odaya giremezsin': 'Tu ne peux pas entrer dans cette salle',
  'Bu odanın üyesi değilsin': "Tu n'es pas membre de cette salle",
  'Bunu yalnız oda sahibi yapabilir': "Seul l'hôte de la salle peut le faire",
  'Oda sahibi odayı kapatmalı': "L'hôte doit fermer la salle",
  'Zaten açık bir odan var': 'Tu as déjà une salle ouverte',
  'Oda kodu 6 karakter olmalı': 'Le code de salle doit faire 6 caractères',
  'Misafir hesaplar izleme odası açamaz. Hesap oluşturursan açabilirsin.':
      'Les comptes invités ne peuvent pas ouvrir de salle. Crée un compte pour en ouvrir une.',
  'Yalnız karşılıklı takipleştiğin kişileri davet edebilirsin':
      'Tu ne peux inviter que les personnes qui te suivent en retour',
  'Video en fazla {} GB olabilir': 'La vidéo peut faire {} Go au maximum',
  'Yalnızca MP4 veya WebM izlenebilir':
      'Seuls les MP4 ou WebM peuvent être lus',
  '{} odaya katıldı': '{} a rejoint la salle',
  '{} odadan ayrıldı': '{} a quitté la salle',
  '{} bir video yükledi': '{} a envoyé une vidéo',
  'bitti': 'terminé',
  '{} sn': '{} s',
  'Birlikte izle': 'Regarder ensemble',
  'Bir video yükle, arkadaşlarını davet et, aynı anda izleyin.':
      'Envoie une vidéo, invite tes amis, regardez en même temps.',
  'Henüz bir odan ya da davetin yok.': "Tu n'as encore ni salle ni invitation.",
  'Odaya katıl': 'Rejoindre une salle',
  'Oda kodu': 'Code de salle',
  'Katıl': 'Rejoindre',
  'Oda oluştur': 'Créer une salle',
  'Odalar 12 saat sonra kendiliğinden kapanır ve video silinir.':
      "Les salles se ferment d'elles-mêmes au bout de 12 heures et la vidéo est supprimée.",
  'Odam': 'Ma salle',
  '@{} odası': 'Salle de @{}',
  'Seni davet etti': "T'a invité",
  '{} kişi': '{} personnes',
  'Video yok': 'Pas de vidéo',
  '{} kaldı': '{} restant',
  'Davet': 'Invitation',
  '@{} davet edildi': '@{} invité',
  'Oda kodu kopyalandı': 'Code de salle copié',
  'Odayı kapat': 'Fermer la salle',
  'Odadan ayrıl': 'Quitter la salle',
  'Oda kapanacak ve video silinecek. Bu geri alınamaz.':
      "La salle sera fermée et la vidéo supprimée. C'est irréversible.",
  'Odadan çıkacaksın. Kodla ya da davetle geri dönebilirsin.':
      'Tu vas quitter la salle. Tu peux revenir avec le code ou une invitation.',
  'Ayrıl': 'Quitter',
  'İzleme odası': 'Salle de visionnage',
  'Odalar 12 saat sonra kendiliğinden kapanır.':
      "Les salles se ferment d'elles-mêmes au bout de 12 heures.",
  '{} kişi · {} kaldı': '{} personnes · {} restant',
  'Davet et': 'Inviter',
  'Bir video yükle, izlemeye başlayın':
      'Envoie une vidéo et commencez à regarder',
  'Oda sahibi henüz video yüklemedi': "L'hôte n'a pas encore envoyé de vidéo",
  'Video yükle': 'Envoyer une vidéo',
  'En fazla {} GB · MP4 veya WebM': "Jusqu'à {} Go · MP4 ou WebM",
  'Kaldığı yerden yükleniyor · {}/{} MB': "Reprise de l'envoi · {}/{} Mo",
  'Yükleniyor · {}/{} MB': 'Envoi · {}/{} Mo',
  'Videoyu değiştir': 'Changer de vidéo',
  'Oda sahibiyle eşleniyor': "Synchronisation avec l'hôte",
  'Sohbet boş': 'Le chat est vide',
  'İzlerken buradan konuşabilirsiniz.':
      'Vous pouvez discuter ici pendant le visionnage.',
  'oda sahibi': 'hôte',
  'davet bekliyor': 'invitation en attente',
  'çevrimdışı': 'hors ligne',
  'Mesaj yaz...': 'Écris un message...',
  'Oda kodunu kopyala': 'Copier le code de salle',
  'Oda kodu {} — kopyala': 'Code de salle {} — copier',
  'Arkadaşını davet et': 'Invite un ami',
  'kullanıcı adı': "nom d'utilisateur",
  'Yalnız karşılıklı takipleştiğin kişileri davet edebilirsin.':
      'Tu ne peux inviter que les personnes qui te suivent en retour.',
  'Oda açılamadı': "La salle n'a pas pu être ouverte",
  'Tam ekrandan çık': 'Quitter le plein écran',
  'Sohbeti gizle': 'Masquer le chat',
  'Sohbeti göster': 'Afficher le chat',
  'Fragman': 'Bande-annonce',
  'Tekrar oynat': 'Rejouer',
  '@{} seni izleme odasına davet etti':
      '@{} t’a invité dans une salle de visionnage',
  '{} · {} davet bekliyor': '{} · {} invitations en attente',
  'Sırada bekliyor': 'En attente',
  'Hazırlanıyor · %{}': 'Préparation · {} %',
  'Video, her cihazda oynayacak biçime çevriliyor.':
      'La vidéo est convertie pour être lue sur tous les appareils.',
  'Başka video yükle': 'Envoyer une autre vidéo',
  'Bu videoyu tarayıcı oynatamıyor. Telefondan aç.':
      'Ton navigateur ne peut pas lire cette vidéo. Ouvre-la sur ton téléphone.',
  'Bu videoyu iPhone oynatamıyor.': "L'iPhone ne peut pas lire cette vidéo.",
  'Tarayıcı için hazırla': 'Préparer pour le navigateur',
  'Başlat': 'Démarrer',
  'Bu videonun görüntü biçimi oynatılamıyor. MP4 (H.264) olarak çevirip yeniden dene.':
      'Le format de cette vidéo ne peut pas être lu. Convertis-la en MP4 (H.264) et réessaie.',
  'Bu dosyada görüntü yok': 'Ce fichier ne contient pas de vidéo',
  'Video okunamadı, dosya bozuk olabilir':
      "La vidéo n'a pas pu être lue, le fichier est peut-être corrompu",
  'Sunucuda yer kalmadı, biraz sonra tekrar dene':
      "Le serveur n'a plus d'espace, réessaie dans un instant",
  'Video bulunamadı': 'Vidéo introuvable',
  'Yetkili': 'Modérateur',
  'Kontrolü ver': 'Donner le contrôle',
  'Kontrolü al': 'Retirer le contrôle',
  'Videoyu oynatıp durdurabilir ve değiştirebilir.':
      'Peut lancer, mettre en pause et changer la vidéo.',
  'Artık videoyu yönetemez.': 'Ne peut plus gérer la vidéo.',
  'Bunu oda sahibi ve yetki verdiği kişiler yapabilir':
      "Seul l'hôte et les personnes qu'il autorise peuvent le faire",
  'Kendi yetkini değiştiremezsin': 'Tu ne peux pas changer tes propres droits',
  'Geçersiz yetki': 'Droit invalide',
  '{} artık videoyu yönetebilir': '{} peut désormais gérer la vidéo',
  '{} kontrolü bıraktı': "{} n'a plus le contrôle",
  'Kişi ara...': 'Rechercher des personnes...',
  'Karşılıklı takipleştiğin kimse yok':
      "Personne ne te suit en retour pour l'instant",
  'Böyle bir kullanıcı yok': "Cet utilisateur n'existe pas",
  'Oda kodunu paylaşarak da davet edebilirsin.':
      'Tu peux aussi inviter en partageant le code de salle.',
  'Karşılıklı takipleşmiyorsunuz': 'Vous ne vous suivez pas mutuellement',
  'Odada': 'Dans la salle',
  'Davet edildi': 'Invité',
  'Aşk': 'Amour',
  'Arkadaşlar': 'Amis',
  'Gece': 'Nuit',
  'Okyanus': 'Océan',
  'Gün batımı': 'Coucher de soleil',
  'Orman': 'Forêt',
  'Neon': 'Néon',
  'Temalar': 'Thèmes',
  'Renkler': 'Couleurs',
  'Sık kullanılanlar': 'Fréquemment utilisés',
  'Klavye': 'Clavier',
  'Kullanıcı adını seç': 'Choisis ton nom d’utilisateur',
  'Google ile giriş yaptığın için adını biz türettik. Profil bağlantın ve etiketlenmelerin bu adla görünür; şimdi kendin seç.':
      'Comme tu t’es connecté avec Google, nous avons généré un nom pour toi. Le lien de ton profil et les mentions l’utilisent — choisis le tien maintenant.',
  'Kontrol ediliyor...': 'Vérification...',
  '@{} müsait': '@{} est disponible',
  'Bu adı seç': 'Choisir ce nom',
  'Kullanıcı adın @{} oldu': 'Ton nom d’utilisateur est maintenant @{}',
  'Sonradan Ayarlar\'dan değiştirebilirsin; her değişiklikten sonra 90 gün beklenir.':
      'Tu pourras le changer plus tard dans les Paramètres ; chaque changement est suivi d’une attente de 90 jours.',
  'Değerlendirme': 'Notes',
  'Değerlendirmeler ({})': 'Notes ({})',
  'Henüz değerlendirme yok': 'Aucune note pour l\'instant',
  'Puan verilen dizi ve filmler burada listelenir.':
      'Les séries et films notés apparaissent ici.',
  'Değerlendirmeler yüklenemedi': 'Impossible de charger les notes',
  'Ayarla': 'Ajuster',
  'Filtre': 'Filtre',
  'Çıkartma': 'Sticker',
  'Parlaklık': 'Luminosité',
  'Kontrast': 'Contraste',
  'Doygunluk': 'Saturation',
  'Pozlama': 'Exposition',
  'Sıcaklık': 'Température',
  'Renk tonu': 'Teinte',
  'Ton': 'Nuance',
  'Soluk': 'Fondu',
  'Orijinal': 'Original',
  'Sıcak': 'Chaud',
  'Soğuk': 'Froid',
  'Sinematik': 'Cinéma',
  'Retro': 'Rétro',
  'Vintage': 'Vintage',
  'Siyah Beyaz': 'Noir et blanc',
  'Canlı': 'Vif',
  'Analog': 'Analogique',
  'Kes': 'Couper',
  'Hız': 'Vitesse',
  'Ses': 'Son',
  'Ses seviyesi': 'Volume',
  'Fotoğraf çek': 'Prendre une photo',
  'Video çek': 'Filmer une vidéo',
  'Kolaj': 'Collage',
  'Düzen': 'Disposition',
  'Boşluk': 'Espacement',
  'Köşe': 'Coins',
  'Kolaj oluşturulamadı': 'Impossible de créer le collage',
  'Kolaj en fazla {} fotoğraf alır': 'Un collage contient au plus {} photos',
  'Yer değiştirmek için iki fotoğrafa dokun':
      'Touchez deux photos pour les échanger',
  'Eleştirmen': 'Critiques',
  'Seyirci': 'Public',
  "2027'de Vizyona Girecek Filmler": "Films attendus en 2027",
  "2027'de Başlayacak Diziler": "Séries qui débutent en 2027",
  'Gizli hesap': 'Compte privé',
  'Açıkken profilini yalnız onayladığın takipçiler görür; yeni takipler sana istek olarak gelir':
      'Activé, seuls les abonnés que tu approuves voient ton profil ; les nouveaux abonnements t\'arrivent sous forme de demandes',
  'İstek Gönderildi': 'Demande envoyée',
  'Bu hesap gizli': 'Ce compte est privé',
  'İsteğin bekliyor; onaylanınca izlediklerini, yorumlarını ve listelerini görebilirsin.':
      'Ta demande est en attente ; une fois acceptée, tu verras ce qu\'il regarde, ses commentaires et ses listes.',
  'İzlediklerini, yorumlarını ve listelerini görmek için takip isteği gönder; onaylanınca profil açılır.':
      'Envoie une demande d\'abonnement pour voir ce qu\'il regarde, ses commentaires et ses listes ; le profil s\'ouvre une fois acceptée.',
  '@{} seni takip etmek istiyor': '@{} souhaite te suivre',
  '@{} takip isteğini kabul etti': '@{} a accepté ta demande d\'abonnement',
  'Onayla': 'Approuver',
  'Yeni liste oluştur': 'Créer une nouvelle liste',
  'Henüz listen yok.': 'Tu n\'as pas encore de liste.',
  'Telif hakkı onayı': 'Confirmation de droits d\'auteur',
  'Yalnızca hakkına sahip olduğun ya da paylaşma izni aldığın videoları yükle. Telif hakkı ihlali içeren videolar kaldırılır ve hesap kapatılabilir.':
      'N\'importe que des vidéos dont tu détiens les droits ou que tu es autorisé à partager. Les vidéos qui enfreignent le droit d\'auteur sont supprimées et le compte peut être fermé.',
  'Onaylıyorum': 'Je confirme',
  'Topluluk Kuralları': 'Règles de la communauté',
  'Saldırgan, taciz edici, nefret içeren, cinsel, şiddet yüceltici ya da telif hakkı ihlali içeren içeriklere ve kötüye kullanıma sıfır tolerans gösterilir. Her yorum, gönderi, mesaj, liste ve kullanıcı uygulama içinden şikâyet edilebilir; istenmeyen kullanıcılar engellenebilir. Şikâyet edilen içerik 24 saat içinde incelenir; kural ihlalinde içerik kaldırılır ve hesap kapatılabilir.':
      'Aucune tolérance pour les contenus injurieux, harcelants, haineux, sexuels, faisant l\'apologie de la violence ou enfreignant le droit d\'auteur, ni pour les abus du service. Chaque commentaire, publication, message, liste et utilisateur peut être signalé depuis l\'application ; les utilisateurs indésirables peuvent être bloqués. Les contenus signalés sont examinés sous 24 heures ; en cas d\'infraction, ils sont supprimés et le compte peut être fermé.',
  'Dosya verisi gerekli': 'Données du fichier requises',
  'Geçerli bir ZIP, CSV ya da JSON dosyası değil':
      'Ce n’est pas un fichier ZIP, CSV ou JSON valide',
  'Dosya boş': 'Le fichier est vide',
  'Bu dosya tanınmadı: TV Time, Letterboxd ya da dizi.jpg dosyası yükle':
      'Fichier non reconnu : importe un fichier TV Time, Letterboxd ou dizi.jpg',
  'Bu dosya tek başına aktarılamıyor: arşivin tamamını (.zip) ya da dizijpg.json dosyasını yükle':
      'Ce fichier ne peut pas être importé seul : envoie toute l’archive (.zip) ou le fichier dizijpg.json',
  'Ort.': 'Moy.',
  'En iyi bölüm': 'Meilleur épisode',
  'Sezonun bölümleri': 'Épisodes de la saison',
  'Ekran görüntüsü aldın': 'Tu as fait une capture d’écran',
  '{} ekran görüntüsü aldı': '{} a fait une capture d’écran',
  'Gönderiliyor': 'Envoi en cours',
  'İzlediğin her şeyi tek yerde takip et':
      'Suis tout ce que tu regardes au même endroit',
  'Keşfet\'e göz at': 'Parcourir Découvrir',
  'Giriş yap': 'Se connecter',
  'Neler yapabilirsin?': 'Que peux-tu faire ?',
  'Bölüm bölüm takip': 'Suivi épisode par épisode',
  'Kaldığın yeri asla kaybetme: sezonları ve bölümleri tek dokunuşla işaretle, ilerlemeni gör.':
      'Ne perds jamais le fil : coche saisons et épisodes d’un geste et vois ta progression.',
  'Yeni bölümler ve vizyon tarihleri takviminde; hiçbir bölümü kaçırma.':
      'Nouveaux épisodes et dates de sortie dans ton calendrier ; ne rate aucun épisode.',
  'Puanla ve yorumla': 'Noter et commenter',
  'Ondalıklı puan ver, yorum yaz; IMDb, Rotten Tomatoes ve Metacritic puanlarını yan yana gör.':
      'Donne des notes décimales, écris des critiques ; compare les scores IMDb, Rotten Tomatoes et Metacritic côte à côte.',
  'Kendi listelerin': 'Tes propres listes',
  'Listeler oluştur, sırala ve paylaş; izleme listen her cihazında seninle.':
      'Crée, trie et partage des listes ; ta liste à voir te suit sur tous tes appareils.',
  'Arkadaşlarınla': 'Avec tes amis',
  'Akışta arkadaşlarının ne izlediğini gör, yorumlarına tepki ver, mesajlaş.':
      'Vois dans ton fil ce que regardent tes amis, réagis à leurs critiques, écris-leur.',
  'İzleme odasında aynı anda izleyin ve sohbet edin.':
      'Regardez en même temps dans une salle de visionnage et discutez.',
  'Yapım verileri: TMDB': 'Données des titres : TMDB',
  'Galeriye kaydet': 'Enregistrer dans la galerie',
  'Galeriye kaydedildi': 'Enregistré dans la galerie',
  'Galeriye kaydediliyor...': 'Enregistrement dans la galerie...',
  'Galeri izni verilmedi': 'Autorisation galerie refusée',
  'Kaydedilemedi': 'Enregistrement impossible',
  '{} medyayı galeriye kaydet': 'Enregistrer {} médias dans la galerie',
  '{}/{} kaydedildi': '{}/{} enregistrés',
  'Hemen kaydol': 'S’inscrire maintenant',
  'Dizilerini bölüm bölüm işaretle, film listeni tut, puanla ve yorumla; arkadaşlarının ne izlediğini gör. Web ve mobilde.':
      'Coche tes séries épisode par épisode, tiens ta liste de films, note et commente, et vois ce que regardent tes amis. Sur le web et sur mobile.',
  '45 dilde. Web, Android ve iOS.': 'En 45 langues. Web, Android et iOS.',
  'Takma ad': 'Surnom',
  'Bu adı yalnız sen görürsün': 'Toi seul(e) vois ce nom',
  'Takma ad kaydedilemedi': 'Le surnom n\'a pas pu être enregistré',
  'Tema karşı tarafa iletilemedi':
      'Le thème n\'a pas pu être envoyé à l\'autre personne',
  'Seçtiğin tema karşı tarafta da görünür':
      'Le thème choisi apparaît aussi chez l\'autre personne',
  'Alıcı bir kez görebilir': 'Le destinataire peut le voir une fois',
  'Açıldı': 'Ouvert',
  'Flaş': 'Flash',
  'Fotoğraf çekilemedi': 'Impossible de prendre la photo',
  'Galeri izni gerekli': 'Accès à la galerie requis',
  'Galeride medya yok': 'Aucun média dans la galerie',
  'Galeriden seç': 'Choisir dans la galerie',
  'Görmek için dokun': 'Touche pour voir',
  'Konum': 'Position',
  'Konum alınamadı': 'Position introuvable',
  'Konum izni gerekli': 'Autorisation de localisation requise',
  'Konum servisi kapalı': 'Services de localisation désactivés',
  'Konumum': 'Ma position',
  'Mesaj ekle...': 'Ajouter un message...',
  'Son fotoğrafların burada görünsün diye galerine erişim gerekir.':
      'L\'accès à la galerie est requis pour afficher tes photos récentes ici.',
  'Tek kullanımlık': 'Vue unique',
  'Tek kullanımlık fotoğraf': 'Photo à vue unique',
  'Tek kullanımlık medya': 'Média à vue unique',
  'Tek kullanımlık video': 'Vidéo à vue unique',
  'Tek kullanımlık yalnız tek fotoğraf veya videoda':
      'La vue unique ne fonctionne qu\'avec une seule photo ou vidéo',
  'Video kaydedilemedi': 'Impossible d\'enregistrer la vidéo',
  'Video kaydı başlamadı': 'L\'enregistrement vidéo n\'a pas démarré',
  'Çek': 'Capturer',
  'Çekmek için dokun, video için basılı tut':
      'Touche pour photographier, maintiens pour filmer',
  'İzin ver': 'Autoriser',
  'az önce': 'à l’instant',
  '{} dk önce': 'il y a {} min',
  '{} dk önce~tekil': 'il y a {} min',
  '{} saat önce': 'il y a {} heures',
  '{} saat önce~tekil': 'il y a {} heure',
  '{} gün önce': 'il y a {} jours',
  '{} gün önce~tekil': 'il y a {} jour',
  '{} hafta önce': 'il y a {} semaines',
  '{} hafta önce~tekil': 'il y a {} semaine',
  // Mesaj/sohbet silme + belge indirme (16 Eyl 2026)
  'Bu mesaj silindi': 'Ce message a été supprimé',
  'Benden sil': 'Supprimer pour moi',
  'Herkesten sil': 'Supprimer pour tout le monde',
  'Sohbeti sil': 'Supprimer la discussion',
  'Karşı taraftan da sil': 'Supprimer aussi chez l’autre personne',
  'Yalnız senden silinir': 'Supprimée uniquement pour vous',
  '{} için de silinir': 'Supprimée aussi pour {}',
  'Bu sohbet senden silinecek. Geri alınamaz.':
      'Cette discussion sera supprimée pour vous. Cette action est irréversible.',
  'Bu sohbet iki taraftan da silinecek. Geri alınamaz.':
      'Cette discussion sera supprimée des deux côtés. Cette action est irréversible.',
  'Sohbet silinemedi': 'Impossible de supprimer la discussion',
  'İndiriliyor': 'Téléchargement',
  'İndirildi': 'Téléchargé',
  'Dosya indirilemedi': 'Impossible de télécharger le fichier',
  'Bu dosyayı açacak uygulama yok':
      'Aucune application ne peut ouvrir ce fichier',
  'Dosyanın yalnız başı gösteriliyor': 'Seul le début du fichier est affiché',
  // Kanon listeleri + çark seçeneği (kanon, 16 Eyl 2026)
  'Ölmeden İzlenmesi Gereken 100 Film': '100 films à voir avant de mourir',
  'Ölmeden İzlenmesi Gereken 250 Film': '250 films à voir avant de mourir',
  'Ölmeden İzlenmesi Gereken 500 Film': '500 films à voir avant de mourir',
  'Ölmeden İzlenmesi Gereken 1000 Film': '1000 films à voir avant de mourir',
  'Ölmeden İzlenmesi Gereken 10 Dizi': '10 séries à voir avant de mourir',
  'Ölmeden İzlenmesi Gereken 25 Dizi': '25 séries à voir avant de mourir',
  'Ölmeden İzlenmesi Gereken 50 Dizi': '50 séries à voir avant de mourir',
  'Ölmeden İzlenmesi Gereken 100 Dizi': '100 séries à voir avant de mourir',
  'İzlediklerimi gösterme': 'Masquer ce que j’ai vu',
  'Bu listede izlemediğin yapım kalmamış':
      'Plus rien de non vu dans cette liste',
  'Liste adı kaydedilemedi': 'Le nom de la liste n\'a pas pu être enregistré',
  'Liste adı boş olamaz': 'Le nom de la liste ne peut pas être vide',
  "dizi.jpg'de Bu Hafta En Çok İzlenen 10 Film":
      "Top 10 films de la semaine sur dizi.jpg",
  "dizi.jpg'de Bu Hafta En Çok İzlenen 10 Dizi":
      "Top 10 séries de la semaine sur dizi.jpg",
  "dizi.jpg'de Bu Ay En Çok İzlenen 10 Film":
      "Top 10 films du mois sur dizi.jpg",
  "dizi.jpg'de Bu Ay En Çok İzlenen 10 Dizi":
      "Top 10 séries du mois sur dizi.jpg",
  "dizi.jpg'de 2026'da En Çok İzlenen 50 Film":
      "Top 50 films de 2026 sur dizi.jpg",
  "dizi.jpg'de 2026'da En Çok İzlenen 50 Dizi":
      "Top 50 séries de 2026 sur dizi.jpg",
  "Bir süre gösterme": "Masquer un moment",
  "Bu liste bir ay boyunca akışta görünmeyecek":
      "Cette liste n'apparaîtra pas dans ton fil pendant un mois",
  "Liste gizlenemedi": "La liste n'a pas pu être masquée",
};
