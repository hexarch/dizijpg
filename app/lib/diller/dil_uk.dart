/// Otomatik üretilen çeviri haritası (uk).
const Map<String, String> cevirilerUk = {
  'dizi.jpg uygulamasını indir': 'Завантажити застосунок dizi.jpg',
  'Daha hızlı gezinme, bildirimler ve tam ekran deneyim uygulamada.':
      'Швидша навігація, сповіщення та повноекранний режим — у застосунку.',
  'Uygulamayı indir': 'Завантажити застосунок',
  'Tarayıcıda devam et': 'Продовжити у браузері',
  'Bir bağlantı yapıştır ya da video yükle, arkadaşlarını davet et, aynı anda izleyin.':
      'Вставте посилання або завантажте відео, запросіть друзів і дивіться разом.',
  'URL yapıştır': 'Вставити посилання',
  'Yapıştır': 'Вставити',
  'Panoda görsel yok': 'У буфері обміну немає зображення',
  'Video bağlantısı': 'Посилання на відео',
  'Odada aç': 'Відкрити в кімнаті',
  'Doğrudan video adresi': 'Пряма адреса відео',
  '{} videosu': 'Відео {}',
  'Başka video seç': 'Вибрати інше відео',
  'Bu video adresi açılamadı': 'Не вдалося відкрити цю адресу відео',
  'Oda sahibi henüz video seçmedi': 'Власник кімнати ще не вибрав відео',
  'Bir video bağlantısı yapıştır ya da dosya yükle':
      'Вставте посилання на відео або завантажте файл',
  '{} bir video bağlantısı ekledi': '{} додав(ла) посилання на відео',
  'Desteklenen siteler: {} · doğrudan video adresi (.mp4, .webm)':
      'Підтримувані сайти: {} · пряма адреса відео (.mp4, .webm)',
  'Bu adres desteklenmiyor. Desteklenen siteler: {} · doğrudan video adresi (.mp4, .webm)':
      'Ця адреса не підтримується. Підтримувані сайти: {} · пряма адреса відео (.mp4, .webm)',
  'Bu adres desteklenmiyor. {} bağlantısı ya da doğrudan bir video adresi (.mp4) yapıştır.':
      'Ця адреса не підтримується. Вставте посилання {} або пряму адресу відео (.mp4).',
  '{} · doğrudan video adresi (.mp4) · ya da en fazla {} GB dosya':
      '{} · пряма адреса відео (.mp4) · або файл до {} ГБ',
  '{} · doğrudan video adresi': '{} · Пряма адреса відео',
  'Bu canlı yayın biçimini (m3u8) tarayıcı oynatamaz; telefon uygulamasında açılır.':
      'Браузер не відтворює цей формат (m3u8); він відкриється в застосунку на телефоні.',
  'Yorumun bu sayfada görünecek': 'Твій коментар з’явиться на цій сторінці',
  'Hepsini gördün, baştan gösteriyoruz':
      'Ви бачили все, ми показуємо вам з самого початку',
  'Aktarılacak tanınan veri bulunamadı.':
      'Не знайдено розпізнаних даних для імпорту.',
  'Arama': 'Пошук',
  'Aramaya başla': 'Почни пошук',
  'Son aramalar': 'Останні пошуки',
  'Ayarlar': 'Налаштування',
  'Bağla': 'Прив’язати',
  'Bio': 'Біо',
  'Bitirdim': 'Завершено',
  'Bunları da Beğenebilirsin': 'Тобі може сподобатися',
  'Bölüm': 'Епізод',
  'Bıraktım': 'Покинуто',
  'Dil': 'Мова',
  'Dil ara...': 'Пошук мови...',
  'Dizi': 'Серіал',
  'Dizi ve filmlerini takip et': 'Стеж за своїми серіалами й фільмами',
  'Dizi yorumu': 'Коментар до серіалу',
  'Dizi, film veya kişi ara...': 'Пошук серіалів, фільмів і людей...',
  'Dosya en fazla 30MB olabilir': 'Файл не більше 30 МБ',
  'Dosya en fazla {}MB olabilir': 'Файл не більше {} МБ',
  'E-posta': 'Ел. пошта',
  'E-posta veya kullanıcı adı': 'Ел. пошта або ім’я користувача',
  'En az 2 harf yaz': 'Введи щонайменше 2 літери',
  'Film': 'Фільм',
  'Film yorumu': 'Коментар до фільму',
  'Fotoğraf / video ekle': 'Додати фото / відео',
  'Fotoğraf veya GIF — profil ve kapak resmi':
      'Фото або GIF — аватар і обкладинка',
  'Giriş Yap': 'Увійти',
  'Gönder': 'Надіслати',
  'Gönderildi': 'Надіслано',
  'Haftanın Dizileri': 'Серіали тижня',
  'Haftanın Filmleri': 'Фільми тижня',
  'Henüz izleme kaydın yok': 'У тебе поки немає історії переглядів',
  'Henüz listen yok — Profil sekmesinden oluştur.':
      'У тебе поки немає списків — створи у вкладці «Профіль».',
  'Henüz yorum yok.': 'Поки немає коментарів.',
  'Hesabın bağlandı!': 'Акаунт прив’язано!',
  'Hesabın yok mu? Kayıt ol': 'Немає акаунта? Зареєструйся',
  'Hesabını Bağla': 'Прив’яжи акаунт',
  'Hesap Oluştur': 'Створити акаунт',
  'Kapak resmi ekle': 'Додати обкладинку',
  'Kaydet': 'Зберегти',
  'Kendinden bahset...': 'Розкажи про себе...',
  'Keşfet': 'Огляд',
  'Kimseyi takip etmiyor': 'Немає підписок',
  'Kişi ara': 'Пошук людей',
  'Kişi yorumu': 'Коментар про людину',
  'Konuk Oyuncular': 'Запрошені актори',
  'Kullanıcı adı (küçük harf)': 'Ім’я користувача (малими літерами)',
  'Kullanıcı adı ara...': 'Пошук за ім’ям користувача...',
  'Liste adı': 'Назва списку',
  'Listeye Ekle': 'Додати до списку',
  'Misafir hesabındasın — e-postanla bağla, verilerini kaybetme!':
      'Ти в гостьовому акаунті — прив’яжи пошту, не втрать свої дані!',
  'Misafir olarak devam et': 'Продовжити як гість',
  'Oluştur': 'Створити',
  'Oyuncular': 'Актори',
  'Profil': 'Профіль',
  'Profil kaydedildi': 'Профіль збережено',
  'Puanla': 'Оцінити',
  'Puanı Sil': 'Видалити оцінку',
  'Puanın': 'Твоя оцінка',
  'Sezonlar': 'Сезони',
  'Sunucu hatası ({})': 'Помилка сервера ({})',
  'S{} · {}. Bölüm': 'S{} · E{}',
  'S{}B{} yorumu': 'Коментар до S{}E{}',
  'Takibi Bırak': 'Відписатися',
  'Takip': 'Підписки',
  'Takip Edilenler': 'Підписки',
  'Takip Et': 'Підписатися',
  'Takipçi': 'Підписники',
  'Takipçi yok': 'Немає підписників',
  'Takipçiler': 'Підписники',
  'Takvim': 'Календар',
  'Tamam': 'Гаразд',
  'Tekrar Dene': 'Повторити',
  'Tekrar dene': 'Спробуй ще раз',
  'Toplam İzleme Süresi': 'Загальний екранний час',
  'Tüm Zamanların En İyileri': 'Найкращі за весь час',
  "Tüm verini (izleme, puan, yorum, liste) TV Time uyumlu ZIP olarak al ya da TV Time / Letterboxd ZIP'ini içe aktar.":
      'Завантаж усі свої дані (перегляди, оцінки, коментарі, списки) у ZIP, сумісному з TV Time, або імпортуй ZIP із TV Time / Letterboxd.',
  'Tümünü Kaldır': 'Прибрати все',
  'Tümünü İzledim': 'Все переглянуто',
  'Türk Dizileri': 'Турецькі серіали',
  'Veri içe aktar (.zip / .csv)': 'Імпорт даних (.zip / .csv)',
  'Verilerim': 'Мої дані',
  'Verilerimi dışa aktar (e-posta)': 'Експорт моїх даних (на пошту)',
  'Verilerini dışa aktar': 'Експорт твоїх даних',
  'Yaklaşan bölüm yok.\nDizi detayından "İzliyorum" durumuna al, yeni bölümleri burada takip et.':
      'Немає майбутніх епізодів.\nПостав серіалу статус «Дивлюся» на його сторінці й стеж за новими епізодами тут.',
  'Yapımları': 'Фільмографія',
  'Yeni Diziler': 'Нові серіали',
  'Yeni Liste': 'Новий список',
  'Yeni kullanıcı adı (isteğe bağlı)': 'Нове ім’я користувача (необов’язково)',
  'Yorum': 'Коментар',
  'Yorumunu yaz...': 'Напиши коментар...',
  'Zaten hesabın var mı? Giriş yap': 'Уже є акаунт? Увійди',
  'ort. {}': 'сер. {}',
  'takip': 'підписки',
  'takipçi': 'підписники',
  '{} / {} bölüm': '{} / {} епізодів',
  '{} TMDB': '{} TMDB',
  '{} ay': '{} міс.',
  '{} ay~tekil': '{} міс.',
  '{} bölüm izlendi': '{} епізодів переглянуто',
  '{} dizi.jpg': '{} dizi.jpg',
  '{} dk': '{} хв',
  '{} dk~tekil': '{} хв',
  '{} gün': '{} дн.',
  '{} gün~tekil': '{} дн.',
  '{} içerik': '{} тайтлів',
  '{} içerik · otomatik': '{} тайтлів · авто',
  '{} kayıt atlandı (eşleşmedi).': '{} записів пропущено (немає збігу).',
  '{} saat': '{} год',
  '{} saat~tekil': '{} год',
  '{} sezon': '{} сезонів',
  '{} yıl': '{} років',
  '{} yıl~tekil': '{} рік',
  '{}. Bölüm': 'Епізод {}',
  '{}. Sezon': 'Сезон {}',
  'Çıkış Yap': 'Вийти',
  'Ülke': 'Країна',
  'Ülke ara...': 'Пошук країни...',
  'Ülke seç': 'Вибери країну',
  'İlk yorumu sen yaz!': 'Напиши перший коментар!',
  'İncelemeler': 'Рецензії',
  'İncelemen (isteğe bağlı)': 'Твоя рецензія (необов’язково)',
  'İptal': 'Скасувати',
  'İzlediklerim': 'Історія переглядів',
  'İzledim': 'Переглянуто',
  'İzledin': 'Переглянуто',
  'İzleme geçmişin ve listelerin korunur; artık her cihazdan girebilirsin.':
      'Історія переглядів і списки збережуться; тепер можна входити з будь-якого пристрою.',
  'İzleme geçmişin, puanların, yorumların ve listelerin ZIP olarak kayıtlı e-posta adresine gönderilecektir.':
      'Історію переглядів, оцінки, коментарі та списки буде надіслано ZIP-архівом на твою пошту.',
  'İzleyeceğim': 'Буду дивитися',
  'İzliyorum': 'Дивлюся',
  'İçe aktarım tamamlandı': 'Імпорт завершено',
  'Şifre': 'Пароль',
  '• profil bilgisi': '• дані профілю',
  '• {} kitaplık kaydı': '• {} записів бібліотеки',
  '• {} favori': '• {} обраних',
  '• {} izleme kaydı': '• {} записів перегляду',
  '• {} liste': '• {} списків',
  '• {} puan': '• {} оцінок',
  '• {} yorum': '• {} коментарів',
  '{}+ yapım': '{}+ робіт',
  'Yorumlar': 'Коментарі',
  'Yorumlar ({})': 'Коментарі ({})',
  'Yorumları ({})': 'Коментарі ({})',
  'Listelerim': 'Мої списки',
  'Nereden izledin?': 'Де дивилися?',
  'Diziye git': 'До серіалу',
  'TV / Kanal': 'ТБ / канал',
  'Sinema': 'Кінотеатр',
  'DVD / Blu-ray': 'DVD / Blu-ray',
  'Diğer': 'Інше',
  'Akış': 'Стрічка',
  'Akışın boş.\nİzlediğin dizi ve filmlere yorum yapılınca burada görünecek.':
      "Твоя стрічка порожня.\nКоли коментуватимуть серіали й фільми, які ти дивишся, це з'явиться тут.",
  'Listeleri ({})': 'Списки ({})',
  'Liste boş.': 'Цей список порожній.',
  'Yanıtla': 'Відповісти',
  '@{} kullanıcısına yanıt veriyorsun': 'Ти відповідаєш @{}',
  'İzlediğim Diziler ({})': 'Переглянуті серіали ({})',
  'İzlediğim Filmler ({})': 'Переглянуті фільми ({})',
  'Tümünü gör': 'Показати всі',
  'Bildirimler': 'Сповіщення',
  'Henüz bildirim yok': 'Поки немає сповіщень',
  '@{} yorumuna yanıt verdi': 'Нова відповідь на твій коментар: @{}',
  '@{} yorumunu beğendi': '@{} подобається твій коментар',
  '{} ve {} yorumunu beğendi': '{} і {} вподобали твій коментар',
  '{} ve {} kişi yorumunu beğendi': '{} і ще {} людей вподобали твій коментар',
  '@{} seni takip etti': 'Новий підписник: @{}',
  '@{} sana mesaj gönderdi': 'Нове повідомлення від @{}',
  'Mesajlar': 'Повідомлення',
  'Mesaj': 'Повідомлення',
  'Mesaja eklenecek': 'Буде додано до повідомлення',
  'Mesajını yaz...': 'Напиши повідомлення...',
  'Henüz sohbetin yok.\nBir profile girip mesaj gönderebilirsin.':
      'Поки немає чатів.\nВідкрий профіль і надішли повідомлення.',
  'Şifreni mi unuttun?': 'Забули пароль?',
  'Şifreyi Sıfırla': 'Скинути пароль',
  'E-postadaki kod': 'Код із листа',
  'Yeni şifre': 'Новий пароль',
  'Sana Özel': 'Для тебе',
  '{} özetin': 'Твої підсумки {}',
  'Yıllık izleme istatistiklerin': 'Річна статистика переглядів',
  'Rozetler': 'Значки',
  'Verdiğin puan': 'Поставлені оцінки',
  'Ortalama puanın': 'Середня оцінка',
  'En çok izlediğin diziler': 'Найбільш переглянуті серіали',
  'İlk Bölüm': 'Перша серія',
  '100 Bölüm': '100 серій',
  '1000 Bölüm': '1000 серій',
  'İlk Film': 'Перший фільм',
  '50 Film': '50 фільмів',
  'İlk Yorum': 'Перший коментар',
  '25 Yorum': '25 коментарів',
  '10 Puan': '10 оцінок',
  'İlk Takipçi': 'Перший підписник',
  '10 Dizi Bitirdin': 'Завершено 10 серіалів',
  'İzlemeyi Bıraktım': 'Більше не дивлюся',
  'İzlediği Diziler ({})': 'Переглянуті серіали ({})',
  'İzlediği Filmler ({})': 'Переглянуті фільми ({})',
  '500 Bölüm': '500 серій',
  '5000 Bölüm': '5000 серій',
  '10 Film': '10 фільмів',
  '100 Film': '100 фільмів',
  '100 Yorum': '100 коментарів',
  '50 Puan': '50 оцінок',
  '100 Puan': '100 оцінок',
  '10 Takipçi': '10 підписників',
  '50 Takipçi': '50 підписників',
  '25 Dizi Bitirdin': 'Завершено 25 серіалів',
  '50 Dizi Bitirdin': 'Завершено 50 серіалів',
  '10 Beğeni': '10 вподобань',
  '100 Beğeni': '100 вподобань',
  'yazıyor...': 'друкує...',
  'ses kaydediyor...': 'записує аудіо...',
  'Profil düzeni': 'Порядок розділів профілю',
  'Konumla ve kırp': 'Розташуй та обріж',
  'Listeye eklendi': 'Додано до списку',
  'Listeyi sil?': 'Видалити список?',
  'Sil': 'Видалити',
  'Bu içerik hiç izlenmemiş olarak işaretlenecek ve listelerinden kaldırılacak.':
      'Тайтл буде позначено як непереглянутий і вилучено з твоїх списків.',
  'Gelecek bölüm bugün': 'Наступна серія сьогодні',
  'Gelecek bölüm {} gün sonra': 'Наступна серія через {} дн.',
  'Tema': 'Тема',
  'Sistem': 'Системна',
  'Koyu': 'Темна',
  'Açık': 'Світла',
  'İzleyenler': 'Глядачі',
  'Yorumunu yaz... (@ ile etiketle)': 'Напиши коментар... (@ щоб позначити)',
  '@{} bir yorumda seni etiketledi': 'Згадка від @{} у коментарі',
  'Mesajı sil': 'Видалити повідомлення',
  'Mesaj silinemedi': 'Не вдалося видалити повідомлення',
  'Bağlantı geçersiz veya sayfa bulunamadı':
      'Посилання недійсне або сторінку не знайдено',
  "Keşfet'e dön": 'Назад до Огляду',
  'Düzenle': 'Редагувати',
  'Mesajı düzenle': 'Редагувати повідомлення',
  'Yanıtlanıyor': 'Відповідь',
  'düzenlendi': 'змінено',
  'Fotoğraf': 'Фото',
  'Video': 'Відео',
  'İçerik': 'Контент',
  'çevrimiçi': 'онлайн',
  'son görülme {} dk önce': 'бачили {} хв тому',
  'son görülme {} saat önce': 'бачили {} год тому',
  'son görülme {} gün önce': 'бачили {} дн тому',
  'Yorumlar yüklenemedi': 'Не вдалося завантажити коментарі',
  'Favori': 'Обране',
  'Listeye ekle': 'Додати до списку',
  'İçerik paylaş': 'Поділитися',
  'Kapat': 'Закрити',
  'İzlediğin dizi ve filmleri işaretledikçe burada toplanır.':
      'Усе, що ти позначиш переглянутим, збирається тут.',
  'Henüz sohbetin yok': 'Ще немає розмов',
  'Bir profile girip mesaj gönderebilirsin.':
      'Відкрий профіль і надішли повідомлення.',
  'Listeyi sil': 'Видалити список',
  'Puan kaydedilemedi': 'Не вдалося зберегти оцінку',
  'Nerede İzlenir': 'Де дивитися',
  'Abonelik': 'Підписка',
  'Kirala': 'Оренда',
  'Satın al': 'Купити',
  'Devam et': 'Продовжити',
  'Şikayet et': 'Поскаржитися',
  'Engelle': 'Заблокувати',
  'Engeli kaldır': 'Розблокувати',
  'Kullanıcı engellendi': 'Користувача заблоковано',
  'Engel kaldırıldı': 'Блокування знято',
  'Şikayet sebebi': 'Причина скарги',
  'Spam veya yanıltıcı': 'Спам або оманливе',
  'Taciz veya nefret söylemi': 'Домагання або мова ворожнечі',
  'Uygunsuz / cinsel içerik': 'Неприйнятний / сексуальний контент',
  'Şiddet veya tehlikeli içerik': 'Насильство або небезпечний контент',
  'Telif hakkı ihlali': 'Порушення авторських прав',
  'Şikayetin alındı, teşekkürler': 'Твою скаргу отримано, дякуємо',
  'Hesabımı Sil': 'Видалити акаунт',
  'Hesabın ve tüm verin (izleme, puan, yorum, liste, mesaj) kalıcı olarak silinecek. Bu işlem geri alınamaz.':
      'Твій акаунт і всі дані (історія, оцінки, коментарі, списки, повідомлення) буде видалено назавжди. Це не можна скасувати.',
  'Şifreni gir': 'Введи пароль',
  'Hesabımı kalıcı sil': 'Видалити акаунт назавжди',
  'Sesli mesaj': 'Голосове повідомлення',
  'Görüldü': 'Переглянуто',
  'Gönderi': 'Допис',
  'Ses gönderilemedi': 'Не вдалося надіслати голосове повідомлення',
  'Kaydediliyor…': 'Запис…',
  'Ses oynatılamadı': 'Не вдалося відтворити аудіо',
  'Oynat': 'Відтворити',
  'Fragmanı oynat': 'Відтворити трейлер',
  'Altyazıyı aç': 'Увімкнути субтитри',
  'Altyazıyı kapat': 'Вимкнути субтитри',
  '10 saniye ileri': 'Наперед на 10 секунд',
  '10 saniye geri': 'Назад на 10 секунд',
  'Duraklat': 'Пауза',
  'Liste görünümü': 'Список',
  'Takvim görünümü': 'Календар',
  'Önceki ay': 'Попередній місяць',
  'Sonraki ay': 'Наступний місяць',
  'Bu gün bölüm yok': 'Цього дня немає серій',
  'Gizlilik Politikası': 'Політика конфіденційності',
  'Son güncelleme: {}': 'Останнє оновлення: {}',
  'Bu politika, dizi.jpg uygulamasını ve dizijpg.com sitesini kullandığında hangi verilerin toplandığını, nasıl kullanıldığını ve haklarının neler olduğunu açıklar.':
      'Ця політика пояснює, які дані збираються, як вони використовуються і які в тебе права, коли ти користуєшся застосунком dizi.jpg та сайтом dizijpg.com.',
  'Topladığımız Veriler': 'Які дані ми збираємо',
  'Hesap: e-posta adresi, kullanıcı adı ve şifre. Şifreler geri döndürülemez şekilde özetlenerek saklanır; misafir hesaplar e-postasız kullanılabilir.':
      "Акаунт: адреса електронної пошти, ім'я користувача та пароль. Паролі зберігаються у вигляді незворотного хешу; гостьовими акаунтами можна користуватися без пошти.",
  'Profil: avatar, kapak görseli, bio ve ülke gibi eklemeyi seçtiğin bilgiler.':
      'Профіль: відомості, які ти додаєш за бажанням, — аватар, обкладинка, біо та країна.',
  'Kullanım: izleme geçmişin, puanların, yorumların, listelerin, tepkilerin ve favorilerin.':
      'Використання: твоя історія переглядів, оцінки, коментарі, списки, реакції та улюблене.',
  'Mesajlar: yazılı, görselli ve sesli mesajların sunucularımızda saklanır. Mesajlar uçtan uca şifreli değildir; yalnızca şikayet edilirse moderasyon amacıyla incelenir.':
      'Повідомлення: твої текстові, графічні та голосові повідомлення зберігаються на наших серверах. Повідомлення не мають наскрізного шифрування; їх переглядають лише в разі скарги, з метою модерації.',
  'Yüklenen medya: profiline, yorumlarına ve mesajlarına eklediğin fotoğraf, GIF, video ve ses kayıtları.':
      'Завантажені файли: фото, GIF, відео та голосові записи, які ти додаєш у профіль, коментарі та повідомлення.',
  'Teknik: IP adresi, yaklaşık konum (ülke/şehir düzeyi), cihaz platformu, uygulama sürümü ve hata kayıtları. Bunlar güvenlik ve hata ayıklama için tutulur.':
      'Технічні дані: IP-адреса, приблизне розташування (країна/місто), платформа пристрою, версія застосунку та журнали помилок. Вони зберігаються для безпеки та налагодження.',
  'Kullanım istatistikleri: hangi cihaz türü, işletim sistemi ve tarayıcıyla girildiği kaba sınıflar hâlinde günlük toplam sayaçlara eklenir; tarayıcı kimliğinin kendisi saklanmaz ve bu sayılar kişilere bağlanamaz.':
      'Статистика використання: тип пристрою, операційна система та браузер, з яких відбувається вхід, додаються до щоденних загальних лічильників у вигляді грубих категорій; сам ідентифікатор браузера не зберігається, і ці числа неможливо пов’язати з окремими людьми.',
  "Bildirimler: push bildirimleri için cihaz token'ı ve dil tercihin saklanır. Bildirimleri cihazının ayarlarından istediğin zaman kapatabilirsin.":
      'Сповіщення: для push-сповіщень ми зберігаємо токен твого пристрою та обрану мову. Сповіщення можна будь-коли вимкнути в налаштуваннях пристрою.',
  'Verileri Nasıl Kullanırız': 'Як ми використовуємо твої дані',
  'Verilerini yalnızca hizmeti sunmak, hesabını korumak, bildirim göndermek, hataları gidermek ve kötüye kullanımı önlemek için kullanırız. Verilerini satmayız, reklam amacıyla kimseyle paylaşmayız.':
      'Ми використовуємо твої дані лише для роботи сервісу, захисту акаунта, надсилання сповіщень, виправлення помилок і запобігання зловживанням. Ми не продаємо твої дані й нікому не передаємо їх із рекламною метою.',
  'Çerezler ve Yerel Depolama': 'Cookie та локальне сховище',
  'Oturumunu açık tutmak ve dil/tema gibi tercihlerini hatırlamak için yerel depolama kullanırız. Sitenin hangi bölümlerinin ne kadar kullanıldığını toplu olarak görmek için web tarafında Google Analytics çalışır; bu araç tarayıcına kendi ölçüm çerezini yazar. Reklam veya reklam hedefleme çerezi yoktur.':
      'Ми використовуємо локальне сховище, щоб ви залишалися в акаунті та щоб запам’ятовувати налаштування, як-от мову й тему. У вебверсії працює Google Analytics, щоб ми могли в узагальненому вигляді бачити, які частини сайту використовуються; цей інструмент записує у ваш браузер власний файл cookie для вимірювання. Рекламних і таргетингових файлів cookie немає.',
  'Üçüncü Taraf Hizmetler': 'Сторонні сервіси',
  "Dizi ve film bilgileri TMDB'den, izleme sağlayıcı bilgisi JustWatch'tan alınır. Push bildirimleri Google Firebase üzerinden iletilir, web trafiği ölçümü Google Analytics ile yapılır, site trafiği Cloudflare tarafından korunur. Bu hizmetler kendi gizlilik politikalarına tabidir.":
      'Інформація про серіали та фільми надходить із TMDB, а дані про стрімінгові сервіси — з JustWatch. Push-повідомлення доставляються через Google Firebase, вебтрафік вимірюється за допомогою Google Analytics, а трафік сайту захищає Cloudflare. Ці сервіси підпадають під власні політики конфіденційності.',
  'Saklama ve Silme': 'Зберігання та видалення',
  'Verilerin hesabın açık olduğu sürece saklanır. Ayarlar\'daki "Hesabımı Sil" ile hesabını kalıcı olarak silebilirsin; verilerin anında, yedeklerdeki kopyaları en geç 14 gün içinde silinir. Hata kayıtları 30 gün sonra otomatik silinir.':
      'Твої дані зберігаються, поки існує акаунт. Через «Видалити акаунт» у налаштуваннях можна видалити акаунт назавжди; дані стираються одразу, а їхні копії в резервних копіях — щонайпізніше за 14 днів. Журнали помилок автоматично видаляються за 30 днів.',
  "Verilerini Ayarlar'dan ZIP olarak dışa aktarabilirsin; arşiv e-posta adresine gönderilir.":
      'Ти можеш вивантажити свої дані у вигляді ZIP із налаштувань; архів надійде на твою електронну пошту.',
  'Güvenlik': 'Безпека',
  'Veriler şifreli bağlantı (HTTPS) üzerinden taşınır ve erişimi sınırlı sunucularda saklanır.':
      "Дані передаються зашифрованими з'єднаннями (HTTPS) і зберігаються на серверах з обмеженим доступом.",
  'Çocukların Gizliliği': 'Конфіденційність дітей',
  'dizi.jpg 13 yaşından küçük çocuklara yönelik değildir.':
      'dizi.jpg не призначений для дітей до 13 років.',
  'Hakların': 'Твої права',
  'KVKK ve GDPR kapsamında verilerine erişme, düzeltme, silme ve taşıma hakkına sahipsin. Bu haklar için bize yazabilirsin: {}':
      'Згідно з KVKK та GDPR ти маєш право на доступ до своїх даних, їх виправлення, видалення та перенесення. Із цих питань напиши нам: {}',
  'Değişiklikler': 'Зміни',
  'Bu politika değişirse yeni sürümü bu sayfada yayımlanır ve güncelleme tarihi yenilenir.':
      'Якщо політика зміниться, нову версію буде опубліковано на цій сторінці, а дату оновлено.',
  'Kayıt olarak {} kabul etmiş olursun.': 'Реєструючись, ти приймаєш {}.',
  'Kullanıcılar': 'Користувачі',
  'Fotoğrafı değiştir': 'Змінити фото',
  'Yeniden konumlandır': 'Перекадрувати',
  'Bir şeyler ters gitti': 'Щось пішло не так',
  'Sosyal Bağlantılar': 'Соцмережі',
  'Profilinde en fazla 3 bağlantı gösterilir.':
      'У профілі показується до 3 посилань.',
  'Platform seç': 'Обери платформу',
  'Kullanıcı adın': "Твоє ім'я користувача",
  'Ekle': 'Додати',
  'Kaldır': 'Прибрати',
  'Kopyalandı: {}': 'Скопійовано: {}',
  'Geçersiz kullanıcı adı': "Неприпустиме ім'я користувача",
  'Bağlantı ekle': 'Додати посилання',
  'Spoiler olabilir — dokun ve gör':
      'Можливі спойлери — торкнись, щоб переглянути',
  'Beğeni': 'Вподобання',
  'beğeni': 'вподобання',
  'Beğen': 'Подобається',
  'Görüntülenme': 'Перегляди',
  'görüntülenme': 'перегляди',
  'Tam ekran': 'На весь екран',
  'Arama başarısız': 'Пошук не вдався',
  'Sonuç bulunamadı': 'Нічого не знайдено',
  'Dizi ve Filmler': 'Серіали та фільми',
  'Kişiler': 'Люди',
  'Ana Sayfa': 'Головна',
  'Paylaş': 'Поділитися',
  'Yanıtlar': 'Відповіді',
  'Yaklaşan Bölümler': 'Найближчі серії',
  'Yetişme Listesi': 'Список надолуження',
  'Şunu mu demek istedin': 'Можливо, ви мали на увазі',
  'Spoiler': 'Спойлер',
  'Spoiler — dokun ve gör': 'Спойлер — торкнись, щоб побачити',
  'uyum': 'збігу',
  'Uyum': 'Збіг',
  '{} ortak dizi': '{} спільних серіалів',
  '{} ortak film': '{} спільних фільмів',
  'Bildirim Tercihleri': 'Налаштування сповіщень',
  'Beğeniler': 'Вподобання',
  'Yeni takipçiler': 'Нові підписники',
  'Etiketlenmeler': 'Згадки',
  'Yeniden izledim': 'Переглянуто знову',
  '{}. kez izlendi': 'Переглянуто {} разів',
  'Gönderi bulunamadı': 'Допис не знайдено',
  'Gözat': 'Огляд',
  'Diziler': 'Серіали',
  'Filmler': 'Фільми',
  'Tümü': 'Усі',
  'Farklı bir tür seç.': 'Спробуй інший жанр.',
  'Gizlilik': 'Приватність',
  'İzlediklerimi gizle': 'Приховати, що я дивлюся',
  'Profilinde izlediğin dizi ve filmler görünmez':
      'Переглянуті серіали та фільми не буде видно у твоєму профілі',
  'Yorumlarımı gizle': 'Приховати мої коментарі',
  'Profilinde yorumların görünmez':
      'Твої коментарі не буде видно у твоєму профілі',
  'Tek bir dizi veya filmi, içeriğin sayfasındaki "Profilimde gizle" çipiyle gizleyebilirsin.':
      'Окремий серіал чи фільм можна приховати чипом «Приховати в моєму профілі» на його сторінці.',
  'Geri Bildirim': "Зворотний зв'язок",
  'Uygulama hakkında görüş ve önerini yaz...':
      'Напиши свою думку та пропозиції щодо застосунку...',
  'Teşekkürler! Geri bildirimin alındı.': 'Дякуємо! Твій відгук отримано.',
  'Profilimde gizle': 'Приховати в моєму профілі',
  'Yorumları': 'Коментарі',
  'Bu kullanıcı henüz bir şey izlememiş.':
      'Цей користувач ще нічого не дивився.',
  'Bu kullanıcı yorumlarını gizli tutmayı tercih ediyor.':
      'Цей користувач воліє тримати свої коментарі прихованими.',
  'S{}B{}': 'S{}E{}',
  'Veri: JustWatch': 'Дані: JustWatch',
  'Web Sitesi': 'Вебсайт',
  'Diğer uygulamalar': 'Інші застосунки',
  'Bağlantıyı kopyala': 'Копіювати посилання',
  'Henüz kimseyi takip etmiyorsun.': 'Ти поки ні на кого не підписаний.',
  'Çevir': 'Перекласти',
  'Çeviri şu an yapılamadı': 'Переклад зараз недоступний',
  'Bağlantı açılamadı': 'Не вдалося відкрити посилання',
  'Orijinali göster': 'Показати оригінал',
  'Çeviriyi göster': 'Показати переклад',
  'Bu ay için yayın tarihi açıklanmış bölüm yok. Tarihler genelde birkaç hafta önceden duyurulur; açıklandıkça burada görünür.':
      'Цього місяця немає серій з оголошеною датою виходу. Дати зазвичай повідомляють за кілька тижнів, і вони з’являться тут.',
  'devamı': 'ще',
  'Google ile devam et': 'Продовжити з Google',
  'Google girişi başarısız': 'Не вдалося ввійти через Google',
  'Apple ile devam et': 'Продовжити з Apple',
  'Apple girişi başarısız': 'Не вдалося увійти через Apple',
  'Veri tasarrufu': 'Економія трафіку',
  'Wi-Fi ağında veri tasarrufu': 'Економія трафіку у Wi-Fi',
  'Mobil veride veri tasarrufu': 'Економія трафіку в мобільній мережі',
  'Açıkken fotoğraflar önceden indirilmez, yalnızca baktığın kare yüklenir.':
      'Коли увімкнено, фото не завантажуються заздалегідь — вантажиться лише той кадр, який ти дивишся.',
  'Çocuk güvenliği': 'Безпека дітей',
  'Türk Filmleri': 'Турецькі фільми',
  'En Yüksek Puanlı Filmler': 'Фільми з найвищим рейтингом',
  'En Yüksek Puanlı Diziler': 'Серіали з найвищим рейтингом',
  'En Çok İzlenen Filmler': 'Найпопулярніші фільми',
  'Popüler Diziler': 'Популярні серіали',
  'En Çok Kazanan Filmler': 'Найкасовіші фільми',
  'Kült Filmler': 'Культові фільми',
  'Yeni Filmler': 'Нові фільми',
  'Yorum yap': 'Коментувати',
  'Henüz yorum yok': 'Поки немає коментарів',
  'Dizi ve filmlere yazdığın yorumlar burada toplanır.':
      'Твої коментарі до серіалів і фільмів збираються тут.',
  'GIF ekle': 'Додати GIF',
  'İlk yorumu sen yaz': 'Залиш перший коментар',
  'Video altyazıları': 'Субтитри відео',
  'Videolarda altyazı göster': 'Показувати субтитри на відео',
  'Videoda konuşulan cümle ekranda yazılır. Metin çeviridir: Türkçe konuşmada İngilizce, diğer dillerde Türkçe.':
      'На екрані написане речення, яке вимовляється у відео. Текст перекладений: англійська мова турецькою, турецька іншими мовами.',
  'Güncelleme gerekli': 'Потрібне оновлення',
  'Yeni sürüm var': 'Доступна нова версія',
  "Devam etmek için dizi.jpg'in yeni sürümünü yükle.":
      'Встанови нову версію dizi.jpg, щоб продовжити.',
  'Güncelle': 'Оновити',
  'Daha sonra': 'Пізніше',
  'Takvim güncelleniyor': 'Календар оновлюється',
  'Takvim güncellenemedi, eski liste gösteriliyor':
      'Не вдалося оновити календар, показано старий список',
  'Bazı diziler yüklenemedi, liste eksik olabilir':
      'Деякі серіали не завантажилися, список може бути неповним',
  'Yenile': 'Оновити',
  'Devam etmek için giriş yap': 'Увійдіть, щоб продовжити',
  'Puan vermek, yorum yazmak ve kitaplığını oluşturmak için ücretsiz bir hesap yeterli.':
      'Безкоштовного облікового запису достатньо, щоб оцінювати, коментувати та створювати свою бібліотеку.',
  'Şimdi değil': 'Не зараз',
  'Yorum yazmak için giriş yap': 'Увійдіть, щоб написати коментар',
  'Kişilere göndermek için giriş yap': 'Увійдіть, щоб надіслати людям',
  'Beğenenler': 'Уподобали',
  'Henüz beğeni yok': 'Ще немає вподобань',
  'Bu gönderiyi ilk beğenen sen ol': 'Оцініть цей допис першим',
  'Bu gönderiye yanıt': 'Відповідь на цей допис',
  'Spoiler içeren gönderi': 'Допис зі спойлерами',
  'Görsel gönderi': 'Допис із фото',
  'Bu yorumu profilimde gizle': 'Приховати цей коментар у моєму профілі',
  'Yorum dizi ve film sayfasında durmaya devam eder':
      'Коментар залишається на сторінці серіалу і фільму',
  'Bu yorumu sil': 'Видалити цей коментар',
  'Yorum profilinde gizlendi': 'Коментар приховано в твоєму профілі',
  'Geri al': 'Скасувати',
  'Yorum silinsin mi?': 'Видалити коментар?',
  'Bu yorum her yerden kalıcı olarak silinir. Geri alınamaz.':
      'Цей коментар буде остаточно видалено всюди. Це неможливо скасувати.',
  'Yorum silindi': 'Коментар видалено',
  'Yorum profilinde tekrar görünüyor': 'Коментар знову видно в твоєму профілі',
  'Gizlenen yorumun yok': 'Немає прихованих коментарів',
  'Profilindeki Yorumlar sekmesinde bir gönderiye basılı tutup gizlediklerin burada birikir.':
      'Коментарі, які ти приховуєш, довго натискаючи на допис у вкладці Коментарі свого профілю, збираються тут.',
  'Gizlenen yorumlar': 'Приховані коментарі',
  'Gönderiye git': 'Перейти до допису',
  'Tekrar göster': 'Показати знову',
  'Yanıtlarımı gizle': 'Приховати мої відповіді',
  'Başkalarının gönderilerine yazdığın yanıtlar yalnız sana görünür':
      'Відповіді, які ти пишеш на дописи інших людей, видно лише тобі',
  'Profilinde gizlediğin yorumları tekrar göster':
      'Показати знову коментарі, приховані тобою у своєму профілі',
  'Sıralama': 'Сортування',
  'Önerilen': 'Рекомендовані',
  'Kronolojik': 'Хронологічний',
  'Gelen mesaj istekleri': 'Вхідні запити на повідомлення',
  'Mesaj isteğin yok': 'Немає запитів на повідомлення',
  'Takip etmediğin kişilerden gelen mesajlar burada görünür.':
      'Тут зʼявляються повідомлення від людей, на яких ви не підписані.',
  'Çevrimiçi durumumu gizle': 'Приховати мій статус у мережі',
  'Mesajlarda çevrimiçi olduğun ve son görülme zamanın başkalarına görünmez':
      'Інші не бачитимуть ваш статус у мережі та час останньої активності в повідомленнях',
  'İlk kullanıcılarımızdan biri. Geri bildirimleriyle uygulamanın bugün olduğu hale gelmesine katkı sağladı.':
      'Серед наших перших користувачів. Ці відгуки допомогли застосунку стати таким, яким він є сьогодні.',
  'İlk kullanıcılarımızdan birisin. Geri bildirimlerinle uygulamanın bugün olduğu hale gelmesine katkı sağladın.':
      'Ти серед наших перших користувачів. Твої відгуки допомогли застосунку стати таким, яким він є сьогодні.',
  'İleri': 'Далі',
  'Önceki': 'Попередній',
  'Sonraki': 'Наступний',
  'Seçilen dosya okunamadı': 'Не вдалося прочитати вибраний файл',
  'Video açılamadı': 'Не вдалося відкрити відео',
  'En fazla {} medya seçebilirsin': 'Можна вибрати не більше {} файлів',
  'Hiçbir medya yüklenemedi': 'Не вдалося завантажити жодного файлу',
  'Seçimi kaldır': 'Прибрати з вибору',
  'Tek yorumda toplam {} MB medya yükleyebilirsin':
      'В одному коментарі можна завантажити до {} МБ',
  '{} dosya okunamadı': 'Не вдалося прочитати файлів: {}',
  '{} medya eklendi, {} yüklenemedi': 'Додано: {}, не завантажено: {}',
  'Kırp': 'Обрізати',
  'Çiz': 'Малювати',
  'Metin': 'Текст',
  'Emoji': 'Емодзі',
  'Yinele': 'Повторити',
  'Daha fazla': 'Більше',
  'Hazırlanıyor…': 'Підготовка…',
  'Düzenlemeden çık?': 'Вийти з редагування?',
  'Yaptığın değişiklikler kaybolacak.': 'Внесені зміни буде втрачено.',
  'Çık': 'Вийти',
  'Serbest çizim': 'Від руки',
  'Ok': 'Стрілка',
  'Dikdörtgen': 'Прямокутник',
  'Daire': 'Коло',
  'Bulanıklaştır': 'Розмити',
  'Pikselleştir': 'Пікселізувати',
  'Silgi': 'Гумка',
  'Çizgi kalınlığı': 'Товщина лінії',
  'Saydamlık': 'Непрозорість',
  'Dolgu': 'Заливка',
  'Renk': 'Колір',
  'Döndür': 'Обернути',
  'Yansıt': 'Віддзеркалити',
  'Oran': 'Пропорції',
  'Sıfırla': 'Скинути',
  'Serbest': 'Довільно',
  'Metin yaz...': 'Введіть текст...',
  'Hizala': 'Вирівняти',
  'Arka plan': 'Фон',
  'Emoji ara...': 'Пошук емодзі...',
  'Son kullanılanlar': 'Нещодавні',
  'İfadeler ve insanlar': 'Смайлики та люди',
  'Hayvanlar ve doğa': 'Тварини та природа',
  'Yiyecek ve içecek': 'Їжа та напої',
  'Etkinlikler': 'Активності',
  'Seyahat ve yerler': 'Подорожі та місця',
  'Nesneler': 'Об’єкти',
  'Semboller': 'Символи',
  'Bayraklar': 'Прапори',
  'Görseli düzenle': 'Редагувати фото',
  'Düzenlendi': 'Відредаговано',
  'Düzenlenemedi': 'Не вдалося відредагувати',
  'Düzenlenen görsel çok büyük': 'Відредаговане зображення завелике',
  'Videoyu düzenle': 'Редагувати відео',
  'Sesi kapat': 'Вимкнути звук',
  'Sesi aç': 'Увімкнути звук',
  'İptal ediliyor…': 'Скасування…',
  'Bu biraz sürebilir': 'Це може зайняти трохи часу',
  'Video çok büyük': 'Відео завелике',
  'Video hazırlanamadı': 'Не вдалося підготувати відео',
  'Başlangıç': 'Початок',
  'Bitiş': 'Кінець',
  'En çok {} saniye seçebilirsin': 'Можна вибрати щонайбільше {} секунд',
  'Dosya en fazla {} MB olabilir': 'Файл може важити максимум {} МБ',
  'Medya ekle': 'Додати медіа',
  'Seçili medya kalmadı': 'Медіа не вибрано',
  'Yorumuna eklemek için fotoğraf veya video seç.':
      'Виберіть фото або відео, щоб додати до коментаря.',
  'Medya seç': 'Вибрати медіа',
  'Daha fazla ekle': 'Додати ще',
  'Medya seçilemedi': 'Не вдалося вибрати медіа',
  'Favori oyuncular': 'Улюблені актори',
  'Henüz favori oyuncun yok': 'Ще немає улюблених акторів',
  'Bir oyuncunun sayfasını aç ve sağ üstteki kalbe dokun; buraya eklensin.':
      'Відкрийте сторінку актора і торкніться сердечка вгорі праворуч, щоб додати сюди.',
  '{} yapımdan {} tanesini izledin': 'Ви переглянули {} з {}',
  'izledin': 'переглянуто',
  'İzlemedin': 'Не переглянуто',
  'Yapım bulunamadı': 'Нічого не знайдено',
  'Bu kişinin listelenecek dizi veya filmi yok.':
      'У цієї людини немає серіалів чи фільмів для показу.',
  'İzlediklerin hesabına bağlı; bu liste yalnız sana özel.':
      'Історія переглядів прив язана до вашого облікового запису; цей список лише ваш.',
  'Hesabın kalıcı olarak askıya alındı':
      'Ваш обліковий запис заблоковано назавжди',
  'Hesabın geçici olarak askıya alındı':
      'Ваш обліковий запис тимчасово заблоковано',
  'Hesabın askıya alındı': 'Ваш обліковий запис заблоковано',
  'Sebep': 'Причина',
  'Kalan süre': 'Залишилося часу',
  'Süre dolunca hesabın kendiliğinden açılır':
      'Після завершення строку обліковий запис розблокується автоматично',
  'Bu süre boyunca okumaya devam edebilirsin; gönderi, yorum ve mesaj gönderemezsin.':
      'У цей час ви можете читати, але не можете публікувати, коментувати чи надсилати повідомлення.',
  'Anladım': 'Зрозуміло',
  'Ayrıntı': 'Докладніше',
  '{} dakika': '{} хвилин',
  'birkaç saniye': 'кілька секунд',
  'Karara itiraz et': 'Оскаржити рішення',
  'Neden bu cezanın kaldırılması gerektiğini anlat':
      'Поясніть, чому це покарання слід скасувати',
  'İtirazını gönder': 'Надіслати оскарження',
  'İtirazın incelemede': 'Ваше оскарження на розгляді',
  'İtirazın reddedildi': 'Ваше оскарження відхилено',
  'İtirazın alındı, inceleniyor': 'Оскарження отримано і перебуває на розгляді',
  'İtiraz en az 10 karakter olmalı':
      'Оскарження має містити щонайменше 10 символів',
  'Yükleniyor...': 'Завантаження...',
  'Bu bölüme puan ver': 'Оцініть цю серію',
  '{} kişi puanladı': 'Оцінили: {}',
  'Bölüm izlendi olarak işaretlendi': 'Серію позначено як переглянуту',
  'Sesli ara': 'Голосовий виклик',
  'Görüntülü ara': 'Відеовиклик',
  'Çalıyor...': 'Дзвонимо...',
  'Bağlanıyor...': 'Підключення...',
  'Cevapla': 'Відповісти',
  'Reddet': 'Відхилити',
  'Sessize al': 'Вимкнути мікрофон',
  'Hoparlör': 'Динамік',
  'Kamerayı kapat': 'Вимкнути камеру',
  'Kamerayı çevir': 'Змінити камеру',
  'Cevap yok': 'Немає відповіді',
  'Arama reddedildi': 'Виклик відхилено',
  'Meşgul': 'Зайнято',
  'Bağlanılamadı': 'Не вдалося зʼєднатися',
  'Arama iptal edildi': 'Виклик скасовано',
  'Arama geçmişi': 'Історія викликів',
  'Gelen arama': 'Вхідний виклик',
  'Giden arama': 'Вихідний виклик',
  'Cevapsız arama': 'Пропущений виклик',
  'Arama şu anda kullanılamıyor': 'Виклики зараз недоступні',
  'Görüntülü arama şu anda kapalı, sesli arayabilirsin':
      'Відеовиклики зараз вимкнено, ти можеш зателефонувати голосом',
  'Bu kullanıcıyı arayamazsın': 'Ти не можеш зателефонувати цьому користувачу',
  'Aramak için karşılıklı takipleşmelisiniz':
      'Щоб телефонувати, ви маєте підписатися одне на одного',
  'Bu kullanıcı şu anda aranamıyor':
      'Цьому користувачу зараз не можна зателефонувати',
  'Çok fazla cevapsız arama. {} sonra tekrar dene':
      'Забагато пропущених викликів. Спробуй за {}',
  'Zaten bir aramadasın': 'Ти вже на виклику',
  'Arama için mikrofon izni gerekiyor':
      'Для виклику потрібен дозвіл на мікрофон',
  'Görüntülü arama için kamera ve mikrofon izni gerekiyor':
      'Для відеовиклику потрібні дозволи на камеру та мікрофон',
  'Arama başlatılamadı': 'Не вдалося почати виклик',
  'Kullanıcı bulunamadı': 'Користувача не знайдено',
  'Kendini arayamazsın': 'Не можна телефонувати самому собі',
  'Arama artık geçerli değil': 'Цей виклик більше не дійсний',
  'Bu aramanın tarafı değilsin': 'Ти не учасник цього виклику',
  'Arama bulunamadı': 'Виклик не знайдено',
  'Çok fazla istek, biraz bekle': 'Забагато запитів, зачекай трохи',
  'Bağlantı koptu': 'Зʼєднання втрачено',
  'Arama süre sınırına ulaştı': 'Виклик досяг ліміту часу',
  'Arama en fazla 4 saat sürebilir': 'Виклик може тривати щонайбільше 4 години',
  'Arama sona erdi': 'Виклик завершено',
  'Sesli arama': 'Голосовий виклик',
  'Görüntülü arama': 'Відеовиклик',
  'Gelen aramalar': 'Вхідні виклики',
  'Sesli ve görüntülü arama bildirimleri':
      'Сповіщення про голосові та відеовиклики',
  '{} sa': '{} год',
  'Sesli ve Görüntülü Aramalar': 'Голосові та відеовиклики',
  'Aramaların içeriği kaydedilmez. Ses ve görüntü, cihazlar arasında uçtan uca şifreli (DTLS-SRTP) akar; sunucularımız bu trafiği çözemez, dinleyemez ve saklayamaz.':
      'Вміст викликів ніколи не записується. Аудіо та відео передаються між пристроями з наскрізним шифруванням (DTLS-SRTP); наші сервери не можуть їх розшифрувати, прослухати чи зберегти.',
  'Yalnızca arama üstverisi tutulur: kiminle, hangi yönde, ne zaman ve ne kadar sürdüğü. Bu kayıtlar 90 gün sonra otomatik silinir.':
      'Зберігаються лише метадані виклику: з ким, у якому напрямку, коли й скільки тривав. Ці записи автоматично видаляються через 90 днів.',
  'Doğrudan bağlantı kurulamazsa ses ve görüntü şifreli hâlde bir aktarma sunucusundan (TURN) geçer. Aktarma sunucusu da içeriği çözemez ve kaydetmez.':
      'Якщо пряме зʼєднання неможливе, аудіо та відео проходять у зашифрованому вигляді через сервер ретрансляції (TURN). Він також не може розшифрувати чи записати вміст.',
  'Mikrofon yalnızca sesli veya görüntülü arama sırasında, kamera ise yalnızca görüntülü arama sırasında kullanılır. Arama bitince ikisi de kapatılır.':
      'Мікрофон використовується лише під час голосового або відеовиклику, а камера — лише під час відеовиклику. Після завершення виклику обидва вимикаються.',
  'Takip ettiğin {} kişi izledi': 'Переглянули {} людей із твоїх підписок',
  'Yorum yaz...': 'Напиши коментар...',
  'Sıradaki bölüm': 'Наступний епізод',
  'Doğrulanmış testçi': 'Перевірений тестувальник',
  'Takip, beğeni, yanıt, mesaj ve etiket bildirimleri':
      'Сповіщення про підписки, вподобання, відповіді, повідомлення та згадки',
  'Sen': 'Ти',
  'Aradığınız kişide sesli arama devre dışı':
      'У цього користувача голосові дзвінки вимкнено',
  'Aradığınız kişide görüntülü arama devre dışı':
      'У цього користувача відеодзвінки вимкнено',
  'Sesli aramalara izin ver': 'Дозволити голосові дзвінки',
  'Görüntülü aramalara izin ver': 'Дозволити відеодзвінки',
  'Kapalıyken kimse seni sesli arayamaz; arayan uyarı görür':
      'Коли вимкнено, ніхто не подзвонить тобі голосом; той, хто дзвонить, побачить сповіщення',
  'Kapalıyken kimse seni görüntülü arayamaz; arayan uyarı görür':
      'Коли вимкнено, ніхто не подзвонить тобі по відео; той, хто дзвонить, побачить сповіщення',
  'Sesli arama kapalı. Ayarlar > Gizlilik bölümünden açabilirsin.':
      'Голосові дзвінки вимкнено. Увімкнути можна в Налаштування > Конфіденційність.',
  'Görüntülü arama kapalı. Ayarlar > Gizlilik bölümünden açabilirsin.':
      'Відеодзвінки вимкнено. Увімкнути можна в Налаштування > Конфіденційність.',
  'Sesli ve görüntülü arama': 'Голосові та відеодзвінки',
  'Misafir hesaplar arama yapamaz. Hesap oluşturursan kullanabilirsin.':
      'Гостьові акаунти не можуть телефонувати. Створи акаунт, щоб користуватися.',
  'Misafir hesaplar aranamaz': 'Гостьовим акаунтам не можна зателефонувати',
  'Puan dağılımı': 'Розподіл оцінок',
  '{} yıldız': '{} зірок',
  'Yakında gelecek': 'Незабаром',
  '{} {} yayınlandı': '{} {} вже вийшов',
  'Yeni bölüm yayınlandı': 'Вийшов новий епізод',
  'Yeni bölümler': 'Нові епізоди',
  'Geri': 'Назад',
  'Karşılamayı kapat': 'Закрити привітання',
  'Adım {} / {}': 'Крок {} / {}',
  'Hadi başlayalım': 'Почнімо',
  'Kaydet ve devam': 'Зберегти й продовжити',
  '{} tanesini ekle': 'Додати {}',
  'Şimdilik geç': 'Поки пропустити',
  'Doğum tarihin ne zaman?': 'Яка в тебе дата народження?',
  'Yaşına uygun içerik göstermek ve doğum gününü kutlamak için soruyoruz.':
      'Питаємо, щоб показувати контент за віком і привітати з днем народження.',
  'Gün': 'День',
  'Ay': 'Місяць',
  'Yıl': 'Рік',
  'Doğum yılımı paylaşmak istemiyorum': 'Не хочу показувати рік народження',
  'Doğum tarihin profilinde herkese açık gösterilmez. Yalnız yaşa uygun içerik ve doğum günü kutlaması için kullanılır.':
      'Дата народження не показується у профілі публічно. Вона потрібна лише для контенту за віком і привітання з днем народження.',
  'Ocak': 'Січень',
  'Şubat': 'Лютий',
  'Mart': 'Березень',
  'Nisan': 'Квітень',
  'Mayıs': 'Травень',
  'Haziran': 'Червень',
  'Temmuz': 'Липень',
  'Ağustos': 'Серпень',
  'Eylül': 'Вересень',
  'Ekim': 'Жовтень',
  'Kasım': 'Листопад',
  'Aralık': 'Грудень',
  'Verilerini yanında getir': 'Перенеси свої дані',
  'Başka bir uygulamada izlediklerini takip ediyorsan buraya taşıyabilirsin. Sıfırdan başlamana gerek yok.':
      'Якщо ведеш історію переглядів в іншому застосунку, перенеси її сюди. Починати з нуля не доведеться.',
  'Dışa aktardığın ZIP/CSV dosyasını seç; izlediklerin, puanların ve listelerin aktarılsın.':
      'Вибери ZIP/CSV-файл з експорту — перегляди, оцінки та списки перенесуться.',
  'Aktarılıyor...': 'Перенесення...',
  'ZIP/CSV dosyası seç': 'Вибрати ZIP/CSV-файл',
  'Aceleye gerek yok: bunu sonra Ayarlar > Verilerim bölümünden de yapabilirsin.':
      'Поспішати не треба: це можна зробити пізніше в розділі Налаштування > Мої дані.',
  'Hangi filmleri izledin?': 'Які фільми вже переглянуто?',
  'En çok izlenenler burada. Dokunduklarını izlediklerine ekleyelim.':
      'Тут найпопулярніше. Познач знайомі — додамо їх до переглянутого.',
  'SERİ FİLMLER': 'КІНОФРАНШИЗИ',
  'Serilerden {} film eklendi': 'З франшиз додано фільмів: {}',
  'Film listesi yüklenemedi': 'Не вдалося завантажити список фільмів',
  'Bu adımı geçebilirsin, sonra istediğini ekleyebilirsin.':
      'Цей крок можна пропустити й додати все пізніше.',
  '{} film izlediklerine eklendi': 'До переглянутого додано фільмів: {}',
  'Film serileri': 'Кінофраншизи',
  'Bir seriyi baştan sona izlediysen tek dokunuşla ekle.':
      'Уся франшиза переглянута? Додай її одним дотиком.',
  'Seri bulunamadı': 'Франшиз не знайдено',
  'Daha sonra tekrar deneyebilirsin.': 'Можна спробувати пізніше.',
  '{} film': 'Фільмів: {}',
  'Eklendi': 'Додано',
  'Tümünü izledim': 'Усе переглянуто',
  'Peki ya diziler?': 'А що щодо серіалів?',
  'İzlediklerini işaretle; seçtikçe sana benzeyenleri getireceğiz.':
      'Познач переглянуте — за твоїми позначками підберемо схоже.',
  'Dizi listesi yüklenemedi': 'Не вдалося завантажити список серіалів',
  'Seçtiklerine benzeyenler': 'Схоже на твій вибір',
  'Her şey hazır': 'Усе готово',
  "dizi.jpg'de neler yapabileceğine hızlıca göz at.":
      'Швидко поглянь, що можна робити в dizi.jpg.',
  'Profilini tamamla': 'Заповни профіль',
  "Ayarlar'dan bir bio, profil fotoğrafı ve kapak ekleyebilirsin.":
      'У розділі Налаштування можна додати Біо, фото профілю та обкладинку.',
  'Takvimin seni bekliyor': 'Тебе чекає Календар',
  'İzlediğin dizilerin yeni bölümleri takvime düşer, bildirimle haber veririz.':
      'Нові епізоди твоїх серіалів потрапляють у календар, а ми надішлемо сповіщення.',
  'Puan ver, yorum yaz': 'Оцінюй, пиши коментарі',
  'Bölüm bölüm puanla, spoiler perdesiyle yorum yaz, listeler oluştur.':
      'Оцінюй епізод за епізодом, пиши коментарі під завісою спойлера, створюй списки.',
  'Verilerin sende kalır': 'Твої дані залишаються твоїми',
  "Verilerin yalnız hesabına bağlı tutulur, satılmaz. Ayarlar'dan hepsini indirebilir ya da hesabını tamamen silebilirsin.":
      'Дані зберігаються лише разом із твоїм акаунтом і не продаються. У розділі Налаштування можна завантажити їх усі або повністю видалити акаунт.',
  'Engellenen kullanıcılar': 'Заблоковані користувачі',
  'Engellediğin kişileri gör, engeli kaldır':
      'Переглянути заблокованих і розблокувати',
  'Engellediğin kimse yok': 'Заблокованих немає',
  'Bir kullanıcıyı profilindeki üç nokta menüsünden engelleyebilirsin.':
      'Заблокувати користувача можна через меню з трьома крапками на сторінці профілю.',
  '@{} engellensin mi?': 'Заблокувати @{}?',
  'Birbirinizin gönderilerini, yorumlarını ve profilini göremezsiniz; mesaj ve arama da gidemez. Varsa takip bağınız kopar ve engeli kaldırsan bile geri gelmez.':
      'Ви не бачитимете дописи, коментарі та профілі одне одного; повідомлення й дзвінки теж не проходитимуть. Підписка між вами, якщо вона є, розірветься і не повернеться навіть після зняття блокування.',
  'Bu kullanıcıyı engelledin': 'Цього користувача заблоковано',
  'Gönderilerini, yorumlarını ve mesajlarını görmüyorsun; o da seninkileri göremiyor.':
      'Ти не бачиш дописи, коментарі та повідомлення цієї людини, а ця людина не бачить твоїх.',
  '{} yeni bir yapımda: {}': '{} у новому проєкті: {}',
  'Favori kişinden yeni yapım': 'Новий проєкт улюбленої людини',
  '{} için bildirimler': 'Сповіщення про {}',
  'Tüm bildirimler': 'Усі сповіщення',
  'Uygulamada ve telefonda': 'У застосунку й на телефоні',
  'Yalnız uygulama içi': 'Лише в застосунку',
  'Telefon bildirimi gönderilmez': 'Сповіщення на телефон не надсилаються',
  'Kapalı': 'Вимкнено',
  'Bu kişi için bildirim yok': 'Немає сповіщень про цю людину',
  'Favori kişilerin yeni yapımları': 'Нові проєкти улюблених людей',
  'Kapalıyken kimse seni sesli arayamaz; arayan "devre dışı" uyarısı görür':
      'Коли вимкнено, ніхто не подзвонить тобі голосом; той, хто дзвонить, побачить сповіщення "вимкнено"',
  'Kapalıyken kimse seni görüntülü arayamaz; arayan "devre dışı" uyarısı görür':
      'Коли вимкнено, ніхто не подзвонить тобі по відео; той, хто дзвонить, побачить сповіщення "вимкнено"',

  // --- Hareketlerim / İstatistiklerim / gizlilik ---
  'Hareketlerim': 'Моя активність',
  'Beğenilerin, yorumların, izlemelerin tek akışta':
      'Твої вподобання, коментарі та перегляди в одній стрічці',
  'Henüz hareketin yok': 'Поки немає активності',
  'Beğendiğin, yorumladığın, izlediğin her şey burada birikir.':
      'Тут збирається все, що ти вподобуєш, коментуєш і дивишся.',
  'Bu süzgeçte hareket yok': 'У цьому фільтрі немає активності',
  'Başka bir tür seç ya da "Hepsi"ne dön.':
      'Обери інший тип або повернись до «Усі».',
  'Hepsi': 'Усі',
  'Puan': 'Оцінка',
  'Tepki': 'Реакція',
  'İzleme': 'Перегляд',
  'Durum': 'Статус',
  'Liste': 'Список',
  'Yorum yaptın': 'Коментар додано',
  'Yorumu beğendin': 'Лайк коментарю',
  '{} puan verdin': 'Оцінка {}',
  'Tepki verdin': 'Реакція',
  'Listene ekledin': 'Додано до списку',
  'Takip etmeye başladın': 'Нова підписка',
  '"İzleyeceğim" işaretledin': 'Позначено «Буду дивитися»',
  '"İzliyorum" işaretledin': 'Позначено «Дивлюся»',
  '"Bitirdim" işaretledin': 'Позначено «Завершено»',
  '"Bıraktım" işaretledin': 'Позначено «Покинуто»',
  'Silinmiş içerik': 'Видалений вміст',
  'şimdi': 'зараз',
  '{} g': '{} д',
  '{} y': '{} р',
  'İstatistiklerim': 'Моя статистика',
  'Gönderilerinin görüntülenmesi ve beğenisi':
      'Перегляди та вподобання твоїх дописів',
  'Henüz gönderin yok': 'У тебе поки немає дописів',
  'Bir dizi ya da filme yorum yazdığında görüntülenme ve beğeni sayıların burada birikmeye başlar.':
      'Щойно ти прокоментуєш серіал чи фільм, тут почнуть накопичуватися перегляди та вподобання.',
  'Tüm zamanlar': 'За весь час',
  'Son {} gün': 'Останні {} днів',
  'Görüntülenme geçmişi henüz birikmeye başlamadı; ilk gün verisi yarın görünür.':
      'Історія переглядів ще не почала накопичуватися; дані за перший день з’являться завтра.',
  'Görüntülenme geçmişi {} tarihinden beri birikiyor ({} günlük veri var), bu yüzden {} günlük sayı henüz eksik.':
      'Історія переглядів накопичується з {} (є дані за {} днів), тому число за {} днів поки неповне.',
  'Beğeni geçmişi {} tarihinden beri kayıtlı.':
      'Історія вподобань ведеться з {}.',
  'Bu dönemde gösterilecek gönderi yok.': 'За цей період немає дописів.',
  'eksik veri': 'неповні дані',
  'Profilinde izlediğin dizi ve filmler, izleme sayaçların, ekran süren ve uyum kartın başkalarına görünmez':
      'Переглянуті серіали й фільми, лічильники переглядів, екранний час і картка збігу не будуть видні іншим у твоєму профілі',
  'Takipçi listemi gizle': 'Приховати список моїх підписників',
  'Seni kimlerin takip ettiğini başkaları göremez; takipçi sayın görünmeye devam eder':
      'Інші не побачать, хто на тебе підписаний; кількість підписників залишається видимою',
  'Takip ettiklerimi gizle': 'Приховати мої підписки',
  'Kimleri takip ettiğini başkaları göremez; takip sayın görünmeye devam eder':
      'Інші не побачать, на кого ти підписаний; кількість підписок залишається видимою',
  'Bu kullanıcı takipçilerini gizli tutmayı tercih ediyor.':
      'Цей користувач воліє тримати своїх підписників прихованими.',
  'Bu kullanıcı takip ettiklerini gizli tutmayı tercih ediyor.':
      'Цей користувач воліє тримати свої підписки прихованими.',

  // --- md.23 gönderi istatistikleri ---
  'Gönderi istatistikleri': 'Статистика допису',
  'Yalnız kendi gönderilerinin istatistiklerini görebilirsin.':
      'Статистика доступна лише для твоїх власних дописів.',
  'Erişim': 'Охоплення',
  'Görüntüleyen': 'Глядачі',
  'Kaç farklı kişi gördü (son {} gün)':
      'Скільки різних людей побачили (останні {} дн.)',
  'Paylaşım': 'Поширення',
  'Bu gönderiden sonra': 'Після цього допису',
  'Profil ziyareti': 'Переходи в профіль',
  'Yeni takip': 'Нові підписки',
  'İçeriğe tıklama': 'Переходи до контенту',
  'Spoiler perdesini açan': 'Відкрили спойлер',
  'Paylaşım, profil ziyareti, takip, içeriğe tıklama, kaynak kırılımı ve spoiler ölçüleri {} tarihinden beri birikiyor; daha eskisi ölçülmedi.':
      'Поширення, переходи в профіль, підписки, переходи до контенту, розподіл за джерелами та вимірювання спойлера накопичуються з {}; раніше це не вимірювалося.',
  'Zamana yayılmış görüntülenme': 'Перегляди за часом',
  'Görüntülenme nereden geldi': 'Звідки прийшли перегляди',
  'Kimler gördü': 'Хто побачив',
  'Günlük veri henüz birikmeye başlamadı.':
      'Щоденні дані ще не почали накопичуватися.',
  'Grafik için en az iki günlük veri gerekiyor. Veri {} tarihinden beri birikiyor.':
      'Для графіка потрібні дані щонайменше за два дні. Дані накопичуються з {}.',
  'En çok görüntülenme paylaşımdan sonraki {}. gün geldi: {}':
      'Найбільше переглядів припало на {}-й день після публікації: {}',
  'Günlük görüntülenme {} tarihinden beri birikiyor; öncesi ölçülmedi.':
      'Щоденні перегляди накопичуються з {}; раніше це не вимірювалося.',
  'Kaynak kırılımı yeni ölçülmeye başladı; bu gönderi henüz etiketli bir görüntülenme almadı.':
      'Розподіл за джерелами почали вимірювати щойно; цей допис ще не отримав жодного позначеного перегляду.',
  'Takipçi/keşif kırılımı yeni ölçülmeye başladı; henüz veri yok.':
      'Розподіл «підписники/огляд» почали вимірювати щойно; даних поки немає.',
  'Takip edenler': 'Підписники',
  'Keşiften gelenler': 'З огляду',
  'Reels': 'Reels',
  'Dizi/film sayfası': 'Сторінка серіалу/фільму',
  'Paylaşılan bağlantı': 'Надіслане посилання',
  'yeni ölçülüyor': 'нове вимірювання',
  'Kendi ortalamanla aynı düzeyde': 'На рівні твого середнього',
  'Kendi ortalamanın %{} üstünde': 'На {}% вище твого середнього',
  'Kendi ortalamanın %{} altında': 'На {}% нижче твого середнього',
  'Etkileşim oranı': 'Рівень залученості',
  '(beğeni + yorum) ÷ görüntülenme': '(вподобання + коментарі) ÷ перегляди',
  'Toplam görüntülenme': 'Усього переглядів',
  '{} ile {} arasında toplam {} görüntülenme':
      'З {} по {}: усього {} переглядів',
  'İstatistikleri gör': 'Переглянути статистику',

  // --- md.23 gizlilik politikası ---
  'Gönderi sahibine gösterilen istatistikler için, gönderinin kaç kez ve hangi yüzeyden (akış, profil, tam ekran akış, dizi/film sayfası, paylaşılan bağlantı) görüntülendiği, görüntüleyenin o an gönderi sahibini takip edip etmediği, gönderiden profile/içeriğe geçiş, gönderi üzerinden kurulan takip, paylaşım ve spoiler perdesi açılması toplu sayaçlar olarak tutulur. Bu sayaçlarda kullanıcı kimliği, IP adresi veya zaman damgası bulunmaz; kimin ne yaptığı sorgulanamaz.':
      'Для статистики, яку бачить автор допису, у вигляді зведених лічильників зберігається: скільки разів і з якої поверхні (стрічка, профіль, повноекранна стрічка, сторінка серіалу/фільму, надіслане посилання) допис переглянули, чи був глядач у той момент підписаний на автора, переходи з допису в профіль або до контенту, підписки, оформлені через допис, поширення та відкриття спойлерної завіси. У цих лічильниках немає ані ідентифікатора користувача, ані IP-адреси, ані позначки часу; дізнатися, хто що зробив, неможливо.',
  'Bir gönderiyi kaç farklı kişinin gördüğünü sayabilmek için, görüntüleyen başına geri çevrilemez bir anahtarlı özet (kullanıcı kimliğinden veya IP adresinden türetilen kriptografik kısaltma) 90 gün saklanır. Gönderi sahibine yalnız sayı gösterilir; görüntüleyenlerin kimliği hiçbir koşulda paylaşılmaz.':
      'Щоб можна було порахувати, скільки різних людей побачили допис, для кожного глядача на 90 днів зберігається незворотна згортка з ключем (криптографічне скорочення, виведене з ідентифікатора користувача або IP-адреси). Автору допису показується лише число; особи глядачів не розкриваються за жодних обставин.',

  // --- İzleyeceğim tekilliği (izleme silme onayı) ---
  'İzleyeceklerine taşınsın mı?': 'Перенести в «Буду дивитися»?',
  'Bir içerik ya izlenecektir ya izlenmiştir. Devam edersen bu dizideki {} izleme işaretin silinecek.':
      'Тайтл або ще не переглянутий, або переглянутий. Якщо продовжиш, твої позначки перегляду в цьому серіалі ({}) буде видалено.',
  'Bir içerik ya izlenecektir ya izlenmiştir. Devam edersen bu filmdeki "izledim" işaretin kaldırılacak.':
      'Тайтл або ще не переглянутий, або переглянутий. Якщо продовжиш, твою позначку «Переглянуто» для цього фільму буде знято.',
  'Sil ve taşı': 'Видалити й перенести',

  // --- Yapım ekibi + yapım firmaları ---
  'Yapım Ekibi': 'Знімальна група',
  'Yaratıcı': 'Творець',
  'Yönetmen': 'Режисер',
  'Senaryo': 'Сценарій',
  'Yapımcı': 'Продюсер',
  'Yapım Firmaları': 'Студії',
  'Yapım Firması': 'Студія',
  'Firma bulunamadı': 'Студію не знайдено',
  'Bu firmanın bu türde listelenecek yapımı yok.':
      'У цієї студії немає тайтлів такого типу.',
  // --- İzleyici seviyesi ve unvanları (cinsiyetsiz) ---
  // --- Doğum günü kutlama kartı ---
  'Doğum günün kutlu olsun!': 'З днем народження!',
  'İyi ki doğdun, iyi ki buradasın.': 'Як добре, що ти є і що ти поруч.',
  'Bugün {} yaşına girdin': 'Сьогодні тобі виповнилося {}',
  'Teşekkürler': 'Дякую',
  // --- İki adımlı doğrulama (2FA) ---
  'İki Adımlı Doğrulama': 'Двоетапна перевірка',
  'Girişte e-postana kod gönderilir': 'Під час входу код надходить на пошту',
  'Kod {} adresine gönderildi': 'Код надіслано на {}',
  '6 haneli kod': 'Код із 6 цифр',
  'Doğrula': 'Підтвердити',
  'Yeniden gönder': 'Надіслати знову',
  'Vazgeç': 'Скасувати',
  'Yeni kod gönderildi': 'Новий код надіслано',
  'Google ile girişte kod sorulmaz; Google kendi doğrulamasını yapar.':
      'Під час входу через Google код не запитується; Google виконує власну перевірку.',
  'Kapatmak için de kod gerekir.': 'Для вимкнення також потрібен код.',
  'E-postana erişemezsen hesabına giremezsin.':
      'Без доступу до пошти вхід в акаунт буде неможливий.',
  'İki adımlı doğrulama açıldı': 'Двоетапну перевірку ввімкнено',
  'İki adımlı doğrulama kapatıldı': 'Двоетапну перевірку вимкнено',
  // --- Video elde tutma eğrisi ---
  'Videonun ne kadarı izlendi': 'Яку частину відео дивилися',
  'Videoyu başlatanlar': 'почали дивитися відео',
  'videonun %{} noktasına ulaştı': 'додивилися до {}% відео',
  'İzleyenlerin yarısı videonun en az %{} kadarını gördü.':
      'Половина глядачів переглянула щонайменше {}% відео.',
  'Sonuna kadar izleyen: %{}': 'Додивилися до кінця: {}%',
  'Eğri için en az {} izlenme gerekiyor; şu an {} izlenme var.':
      'Для кривої потрібно щонайменше {} переглядів; зараз їх {}.',
  'Eğri {} izlenmeden çıkarıldı ve seçili zaman aralığından etkilenmez.':
      'Крива враховує всі перегляди ({}) і не залежить від вибраного періоду.',
  'İzlenme süresi {} tarihinden beri ölçülüyor; daha eski görüntülenmeler bu sayıya girmez.':
      'Час перегляду вимірюється з {}; давніші перегляди сюди не входять.',
  '{} izlenme. Videonun yarısına ulaşan %{}, sonuna ulaşan %{}.':
      'Переглядів: {}. До середини відео дійшли {}%, до кінця — {}%.',
  // Yüzde işaretinin YERİ dile göre değişir; sayı tek başına
  // basılamaz (bkz. gonderi_istatistik.dart). Kalıp bu dosyanın
  // kendi mevcut yüzde çevirilerinden çıkarıldı.
  '%{}': '{}%',

  // --- md.42/26 istatistikler + seviye ---
  'Seviye {}': 'Рівень {}',
  'Sonraki seviyeye {} puan kaldı': 'Бали до наступного рівня: {}',
  'Gönderilerin': 'Твої дописи',
  '{} gönderi': 'Дописи: {}',
  '{} gönderi · {} görüntülenme': 'Дописи: {} · Перегляди: {}',
  'önceki {} güne göre': 'до попередніх {} днів',
  'önceki {} güne göre %{} arttı': 'до попередніх {} днів: зростання на {}%',
  'önceki {} güne göre %{} azaldı': 'до попередніх {} днів: спад на {}%',
  'önceki {} güne göre değişmedi': 'до попередніх {} днів: без змін',
  'Günlük görüntülenme: en düşük {}, en yüksek {}':
      'Перегляди за день: найменше {}, найбільше {}',
  'Etkileşim oranı henüz ölçülemedi': 'Рівень залученості поки не виміряно',
  'Oranlar': 'Показники',
  'Beğenme oranı': 'Частка вподобань',
  'Yorum oranı': 'Частка коментарів',
  'Paylaşma oranı': 'Частка поширень',
  'İçeriğe gitme oranı': 'Частка переходів до контенту',
  'Profile gitme oranı': 'Частка переходів у профіль',
  'Ziyaretten takibe dönüşüm': 'З візиту в підписку',
  'beğeni ÷ görüntülenme': 'вподобання ÷ перегляди',
  'yorum ÷ görüntülenme': 'коментарі ÷ перегляди',
  'paylaşım ÷ görüntülenme': 'поширення ÷ перегляди',
  'içeriğe tıklama ÷ görüntülenme': 'переходи до контенту ÷ перегляди',
  'profil ziyareti ÷ görüntülenme': 'переходи в профіль ÷ перегляди',
  'yeni takip ÷ profil ziyareti': 'нові підписки ÷ переходи в профіль',
  'Oran için en az {} görüntülenme gerekiyor; şu an {} var.':
      'Для показників потрібно щонайменше {} переглядів; зараз їх {}.',
  'Oranlar gönderinin ömür boyu sayılarından çıkar; aşağıdaki gün seçimi bunları değiştirmez.':
      'Показники рахуються з чисел допису за весь час; вибір днів нижче їх не змінює.',
  'Bu sayılar gönderinin ömür boyu toplamıdır (kendi notu olan kutular hariç); aşağıdaki gün seçimi yalnız grafiği değiştirir.':
      'Це числа допису за весь час (крім блоків із власною приміткою); вибір днів нижче змінює лише графік.',
  'Ortalama izlenme: videonun %{} kadarı': 'Середній перегляд: {}% відео',
  'Elde tutma eğrisinin 20 kovasının ortalaması.':
      'Середнє з 20 інтервалів кривої утримання.',
  'Dizi, film, kişi veya şirket ara...':
      'Пошук серіалів, фільмів, людей і компаній...',
  'Şirketler': 'Компанії',
  'Senarist': 'Сценарист',
  'Bölüm puanları': 'Оцінки серій',
  'S{}': 'S{}',
  'E{}': 'E{}',
  'Bölüm puanları yüklenemedi': 'Не вдалося завантажити оцінки серій',
  'Giriş gerekli': 'Потрібно увійти',
  'Geçersiz oturum': 'Недійсний сеанс',
  'Oturum sonlandı, tekrar giriş yap': 'Сеанс завершено, увійди знову',
  'Çok fazla istek; biraz sonra tekrar dene':
      'Забагато запитів; спробуй трохи пізніше',
  'Sunucuda geçici olarak yer yok, birazdan tekrar dene':
      'Зараз на сервері немає місця, спробуйте трохи згодом',
  'Devam eden yapımlar ({})': 'Поточні проєкти ({})',
  'Diziler ({})': 'Серіали ({})',
  'Filmler ({})': 'Фільми ({})',
  'Tüm yapımlar': 'Усі проєкти',
  'Medya alanın doldu; yer açmak için eski yüklemelerini sil':
      'Місце для медіа закінчилося; видаліть старі завантаження',
  'E-posta/kullanıcı adı veya şifre hatalı':
      'Неправильна пошта/ім’я або пароль',
  'Bu e-posta veya kullanıcı adı zaten kayıtlı':
      'Ця пошта або ім’я вже зайняті',
  'Google doğrulaması başarısız': 'Перевірка Google не вдалася',
  'Kod geçersiz veya süresi dolmuş': 'Код недійсний або прострочений',
  'Bu kullanıcıyla mesajlaşamazsın': 'Не можна написати цьому користувачу',
  'Kendini takip edemezsin': 'Не можна підписатися на себе',
  'Bu kullanıcıyı takip edemezsin': 'Не можна підписатися на цього користувача',
  'Kendini engelleyemezsin': 'Не можна заблокувати себе',
  'Yorum 1-1000 karakter olmalı': 'Коментар має бути 1–1000 символів',
  'Boş mesaj gönderilemez': 'Не можна надіслати порожнє повідомлення',
  'Yorum bulunamadı': 'Коментар не знайдено',
  'Şifre hatalı': 'Неправильний пароль',
  'Şifre en az 6 karakter olmalı': 'Пароль має бути щонайменше 6 символів',
  'Yalnızca GIF, PNG, JPEG veya WebP yüklenebilir':
      'Можна завантажити лише GIF, PNG, JPEG або WebP',
  'Desteklenen türler: GIF, PNG, JPEG, WebP, MP4, WebM, ses':
      'Підтримувані типи: GIF, PNG, JPEG, WebP, MP4, WebM, аудіо',
  'Geçerli bir ZIP dosyası değil': 'Це не чинний файл ZIP',
  'Verilerini e-postayla almak için önce hesabına e-posta bağlamalısın':
      'Щоб отримати дані на пошту, спочатку прив’яжи email',
  'Bu içerikte izleme kaydın var. "İzleyeceğim" demek için önce izleme işaretlerin silinmeli.':
      'У тебе є позначки перегляду. Щоб поставити «Буду дивитися», спочатку їх зніми.',
  'Arama şu anda kapalı': 'Дзвінки зараз вимкнені',
  'Misafir hesaplar arama ayarlarını açamaz; hesap oluşturunca kullanabilirsin':
      'Гостьові акаунти не вмикають дзвінки; створи акаунт',
  'Sunucu yeniden başlatılıyor, birazdan tekrar dene':
      'Сервер перезапускається, спробуй за мить',
  'Puan 1-10 arası olmalı': 'Оцінка має бути від 1 до 10',
  'En fazla 10 medya eklenebilir': 'Можна додати щонайбільше 10 файлів',
  'Bu cihazdan yeni hesap açılamıyor':
      'З цього пристрою не можна створити обліковий запис',
  'Geçerli e-posta, kullanıcı adı ve en az 6 karakter şifre gerekli':
      'Потрібна чинна пошта, ім\'я користувача і пароль щонайменше з 6 символів',
  'Geçerli e-posta ve en az 6 karakter şifre gerekli':
      'Потрібна чинна пошта і пароль щонайменше з 6 символів',
  'Kullanıcı adı 3-20 karakter; küçük harf, rakam, nokta, tire ve alt çizgi kullanılabilir (başta/sonda nokta-tire olamaz)':
      'Ім\'я 3–20 символів; малі літери, цифри, крапки, дефіси й підкреслення (не з країв)',
  'Bu hesap zaten bağlı': 'Цей обліковий запис уже прив\'язано',
  'Hesap oluşturulamadı': 'Не вдалося створити обліковий запис',
  'Misafir hesabı oluşturulamadı':
      'Не вдалося створити гостьовий обліковий запис',
  'Hesap bulunamadı': 'Обліковий запис не знайдено',
  'Bu hesapta e-posta yok': 'У цього облікового запису немає пошти',
  'İki adımlı doğrulama zaten bu durumda':
      'Двокрокова перевірка вже в цьому стані',
  'Kod yanlış': 'Невірний код',
  'Kodun süresi doldu': 'Термін коду минув',
  'Kullanıcının e-postası yok (misafir hesap)':
      'У користувача немає пошти (гостьовий обліковий запис)',
  'Bio en fazla 300 karakter olabilir': 'Біо щонайбільше 300 символів',
  'Geçersiz ülke': 'Недійсна країна',
  'Geçersiz sosyal bağlantı': 'Недійсне соціальне посилання',
  'Geçersiz doğum tarihi': 'Недійсна дата народження',
  'dizi.jpg için en az 13 yaşında olmalısın.':
      'Щоб користуватися dizi.jpg, вам має бути щонайменше 13 років.',
  'En az 13 yaşında olmalısın': 'Вам має бути щонайменше 13 років',
  'Liste adı gerekli': 'Потрібна назва списку',
  'Ad en fazla 60, açıklama 300 karakter olabilir':
      'Назва щонайбільше 60, опис 300 символів',
  'Liste bulunamadı': 'Список не знайдено',
  'Değiştirilecek tercih yok': 'Немає що оновлювати',
  'İnceleme en fazla 2000 karakter olabilir': 'Огляд щонайбільше 2000 символів',
  'Önce içeriği "bitirdim" olarak işaretle':
      'Спочатку познач тайтл як переглянутий',
  'Önce kişiyi favorile': 'Спочатку додай особу в обране',
  'Bölüm yorumu için sezon ve bolum birlikte gerekli':
      'Для коментаря до серії потрібні і сезон, і серія',
  'Yanıtlanan yorum bulunamadı': 'Коментар, на який відповідаєш, не знайдено',
  'Geri bildirim 3-2000 karakter olmalı': 'Відгук має бути 3–2000 символів',
  'Başlık ve metin gerekli (en az 2 karakter)':
      'Потрібні заголовок і текст (щонайменше 2 символи)',
  'Kendine mesaj gönderemezsin': 'Не можна написати самому собі',
  'Mesaj en fazla 2000 karakter olabilir':
      'Повідомлення щонайбільше 2000 символів',
  'Mesaj 1-2000 karakter olmalı': 'Повідомлення має бути 1–2000 символів',
  'Mesaj bulunamadı': 'Повідомлення не знайдено',
  'Mesaj bulunamadı veya düzenlenemez':
      'Повідомлення не знайдено або його не можна змінити',
  'Yalnız sana gelen mesajı şikayet edebilirsin':
      'Можна поскаржитися лише на повідомлення, надіслане тобі',
  'Sebep gerekli': 'Потрібна причина',
  'İtiraz edilecek aktif bir ceza yok': 'Немає чинної кари для оскарження',
  'Zaten incelenmeyi bekleyen bir itirazın var':
      'У тебе вже є скарга на розгляді',
  'Bu ceza için itirazın zaten incelendi':
      'Скаргу щодо цієї кари вже розглянуто',
  'Medya bağlantısının süresi dolmuş': 'Термін медіапосилання минув',
  'Bu medya için imzalı bağlantı gerekli':
      'Для цього медіа потрібне підписане посилання',
  'Arama hizmeti yeniden başlatılıyor, birazdan tekrar dene':
      'Служба дзвінків перезапускається, спробуй трохи згодом',
  'Puan göstergesi': 'Позначення оцінок',
  'İzleme İstatistiklerim': 'Моя статистика переглядів',
  'Bir bölümü ya da filmi izlendi olarak işaretlediğinde istatistiklerin burada birikmeye başlar.':
      'Щойно позначиш серію або фільм як переглянуті, твоя статистика почне накопичуватися тут.',
  '{} sa {} dk': '{} год {} хв',
  'Yaklaşık ekran süresi': 'Приблизний екранний час',
  'İzleme serisi': 'Серія переглядів',
  'Şu anki seri': 'Поточна серія',
  'En uzun seri': 'Найдовша серія',
  'Haftanın günleri': 'Дні тижня',
  'En çok izlediklerin': 'Найбільше переглянуто',
  'Bu dönemde izleme yok': 'У цей період переглядів немає',
  'Paz': 'нд',
  'Pzt': 'пн',
  'Sal': 'вт',
  'Çar': 'ср',
  'Per': 'чт',
  'Cum': 'пт',
  'Cmt': 'сб',
  '{} bölüm': '{} серій',
  '{} bölüm~tekil': '{} серія',
  '{} kez': '{} разів',
  '{} kez~tekil': '{} раз',
  'Son {} gün · günlük': 'Останні {} днів · щодня',
  'Son {} gün · haftalık': 'Останні {} днів · щотижня',
  'Ekran süren, serin ve en çok izlediklerin':
      'Твій екранний час, серія та найпопулярніші',
  'Daralt': 'Згорнути',
  'Listeyi düzenle': 'Редагувати список',
  'Bitti': 'Готово',
  'Sırala': 'Змінити порядок',
  'Herkese göster': 'Показати всім',
  'Başkalarından gizle': 'Приховати від інших',
  'Listeden kaldır': 'Прибрати зі списку',
  'Listeden kaldırılsın mı?': 'Прибрати зі списку?',
  'Bu yapım listeden çıkarılacak. Gizlemek istersen göz simgesini kullanabilirsin.':
      'Цей тайтл буде прибрано зі списку. Якщо хочеш лише сховати його, скористайся піктограмою ока.',
  'Sıralama kaydedilemedi': 'Не вдалося зберегти порядок',
  'Değişiklik kaydedilemedi': 'Не вдалося зберегти зміну',
  'Kaldırılamadı': 'Не вдалося прибрати',
  'Popülerlik': 'Популярність',
  'TMDB puanı': 'Оцінка TMDB',
  'Yapım yılı': 'Рік випуску',
  'dizi.jpg puanı': 'Оцінка dizi.jpg',
  'İzlenme': 'Перегляди',
  'Yorum sayısı': 'Кількість коментарів',
  'Tüm raflara uygulanır': 'Застосовується до всіх полиць',
  'En popüler {} yapım arasında sıralandı':
      'Відсортовано серед {} найпопулярніших творів',
  'Altyazı görünümü': 'Вигляд субтитрів',
  'Renk, yazı tipi, boyut, ayrıt ve opaklık':
      'Колір, шрифт, розмір, край і непрозорість',
  'Bu bir örnek altyazıdır.': 'Це приклад субтитрів.',
  'Ayar değiştikçe önizleme anında güncellenir.':
      'Перегляд оновлюється миттєво під час зміни налаштувань.',
  'Yazı biçimi': 'Стиль тексту',
  'Yazı rengi': 'Колір тексту',
  'Yazı opaklığı': 'Непрозорість тексту',
  'Yazı tipi': 'Шрифт',
  'Yazı boyutu': 'Розмір тексту',
  'Karakter ayrıtı': 'Край символів',
  'Ayrıt türü': 'Тип краю',
  'Ayrıt rengi': 'Колір краю',
  'Yok': 'Немає',
  'Dış çizgi': 'Контур',
  'Gölge': 'Тінь',
  'Kabartma': 'Опуклий',
  'Oyma': 'Втиснутий',
  'Arka plan rengi': 'Колір фону',
  'Arka plan opaklığı': 'Непрозорість фону',
  'Arka plan yazının hemen arkasındaki dolgudur; pencere ise tüm altyazı bloğunun arkasındaki daha geniş yüzey.':
      'Фон — це заливка одразу за текстом, а вікно — ширша поверхня за всім блоком субтитрів.',
  'Pencere': 'Вікно',
  'Pencere rengi': 'Колір вікна',
  'Pencere opaklığı': 'Непрозорість вікна',
  'Altyazı görünümü sıfırlansın mı?': 'Скинути вигляд субтитрів?',
  'Renk, yazı tipi, boyut, ayrıt ve opaklık ayarları varsayılana döner.':
      'Налаштування кольору, шрифту, розміру, краю та непрозорості повернуться до типових значень.',
  'Altyazı görünümü varsayılana döndü': 'Вигляд субтитрів скинуто',
  '{} indiriliyor…': 'Завантаження {}…',
  'Seçince indirilir': 'Завантажиться після вибору',
  'Yazı tipi indirilemedi. Varsayılan yazı tipiyle gösteriliyor.':
      'Не вдалося завантажити шрифт. Показуємо типовим шрифтом.',
  'Beyaz': 'Білий',
  'Siyah': 'Чорний',
  'Kırmızı': 'Червоний',
  'Yeşil': 'Зелений',
  'Mavi': 'Синій',
  'Sarı': 'Жовтий',
  'Camgöbeği': 'Блакитний',
  'Macenta': 'Пурпуровий',
  'Yapım bütçesi': 'Бюджет виробництва',
  'Yapım bütçesi ve dünya çapında hasılat':
      'Бюджет виробництва та світові збори',
  'Dizinin yayın durumu': 'Статус виходу серіалу',
  'Devam ediyor': 'Триває',
  'Sona erdi': 'Завершився',
  'İptal edildi': 'Скасовано',
  'Yapımda': 'У виробництві',
  'Planlandı': 'Заплановано',
  'Pilot bölüm': 'Пілотна серія',
  'Ad': "Ім'я",
  'Adın': "Твоє ім'я",
  'Kullanıcı adı': "Ім'я користувача",
  'Kullanıcı adını değiştir': "Змінити ім'я користувача",
  'Kullanıcı adını 90 günde bir değiştirebilirsin':
      "Ім'я користувача можна змінювати раз на 90 днів",
  'Değiştir': 'Змінити',
  '{} gün sonra değiştirebilirsin': 'Змінити можна через {} дн.',
  'Kullanıcı adın: @{}': "Твоє ім'я користувача: @{}",
  'Bu kullanıcı adı zaten alınmış': "Це ім'я користувача вже зайняте",
  'Bu kullanıcı adı şu an başka bir hesaba ayrılmış':
      "Це ім'я користувача зараз закріплене за іншим акаунтом",
  'Bu zaten senin kullanıcı adın': "Це і є твоє ім'я користувача",
  'Kullanıcı adı 3-20 karakter; küçük harf, rakam, nokta, tire ve alt çizgi kullanılabilir':
      "Ім'я 3–20 символів; малі літери, цифри, крапки, дефіси й підкреслення",
  'Profilinde kullanıcı adının üstünde görünür. Boş bırakabilirsin.':
      'Показується у профілі над іменем користувача. Можна лишити порожнім.',
  'Değiştirdikten sonra 90 gün boyunca tekrar değiştiremezsin. Eski kullanıcı adın 90 gün boyunca sana ayrılır — istersen geri dönebilirsin, o sürede başkası alamaz. Eski adına giden bağlantılar artık profilini açmaz.':
      "Після зміни знову змінити вдасться лише через 90 днів. Старе ім'я користувача залишиться закріпленим за тобою на 90 днів — за бажання зможеш повернутися до нього, і ніхто інший його за цей час не займе. Посилання на старе ім'я більше не відкриють твій профіль.",
  'Etkinliğim': 'Активність',
  'Tercihler': 'Уподобання',
  'Gizlilik ve güvenlik': 'Приватність і безпека',
  'Destek': 'Підтримка',
  'Hesap': 'Акаунт',
  '{} dizileri': 'Серіали: {}',
  '{} filmleri': 'Фільми: {}',
  'En üste taşı': 'Перемістити на початок',
  'Sırayı sıfırla': 'Скинути порядок',
  'Listede ara': 'Пошук у списку',
  'Elle sıra sıfırlansın mı?': 'Скинути ручний порядок?',
  'Liste varsayılan sırasına (en son işaretlediğin önce) döner.':
      'Список повернеться до типового порядку (нещодавно позначені — першими).',
  'Sıra sıfırlanamadı': 'Не вдалося скинути порядок',
  'Listenin en üstüne taşındı': 'Переміщено на початок списку',
  'Adlar yükleniyor…': 'Завантаження назв…',
  'Afişe basılı tutup sürükle. Uzaktaki bir yapımı öne almak için "En üste taşı"yı kullan.':
      'Утримуй постер, щоб перетягнути його. Щоб підняти віддалену назву, скористайся «Перемістити на початок».',
  'Aramada sürükleme kapalı; "En üste taşı" ile öne al ({} sonuç).':
      'Перетягування в пошуку вимкнено; підніми через «Перемістити на початок» ({} результатів).',
  'Görünüm': 'Вигляд',
  'En çok izlediğin filmler': 'Найбільш переглянуті фільми',
  'Süreler tahmindir: bölüm ~{} dk, film ~{} dk sayılır':
      'Час приблизний: епізод рахується як ~{} хв, фільм — як ~{} хв',
  'Bu kullanıcı adı sistem tarafından ayrılmış':
      "Це ім'я користувача зарезервоване системою",
  'Ekran süresi': 'Екранний час',
  "Süreler TMDB'deki gerçek bölüm ve film süreleridir":
      'Час — це реальна тривалість епізодів і фільмів за даними TMDB',
  'Tüm zamanlar: {} bölüm · {} film · {}':
      'За весь час: {} серій · {} фільмів · {}',
  'Kabul et': 'Прийняти',
  'Reddedilenler': 'Відхилені',
  'İstekler': 'Запити',
  'Reddettiğin istek yok': 'Немає відхилених запитів',
  'Reddettiğin istekler burada durur; dilersen geri kabul edebilirsin.':
      'Відхилені запити залишаються тут; їх можна знову прийняти будь-коли.',
  'Ne izlesem?': 'Що подивитися?',
  'Karışık': 'Упереміш',
  'Çarkı çevir': 'Крутити колесо',
  'Tekrar çevir': 'Крутити ще раз',
  'Bu türde içerik yok': 'Немає контенту цього типу',
  'Bütçe': 'Бюджет',
  'dizi.jpg AI özeti': 'Підсумок ШІ dizi.jpg',
  'Daha fazlasını gör': 'Показати більше',
  'Yıldız sistemi': 'Система оцінок',
  '{} yıldız üzerinden': 'З {} зірок',
  'Önizleme': 'Попередній перегляд',
  'dokununca açılır': 'відкривається дотиком',
  'Bir azalt': 'Зменшити на одиницю',
  'Bir artır': 'Збільшити на одиницю',
  'Ölçek kaydedilemedi': 'Не вдалося зберегти шкалу',
  'Ölçeği değiştirmek puanlarını silmez; aynı puanlar yeni ölçekte gösterilir.':
      'Зміна шкали не видаляє ваші оцінки — ті самі оцінки показуються в новій шкалі.',
  'En aşağıya gönder': 'У кінець списку',
  'Listenin en altına taşındı': 'Переміщено в кінець списку',
  'Oturumun sonlandı': 'Ваш сеанс завершено',
  'Bu hesabın oturumları kapatıldı ya da şifresi değişti. Devam etmek için tekrar giriş yap.':
      'З цього облікового запису вийшли всюди або змінено пароль. Увійдіть знову, щоб продовжити.',
  'Son izleme: {}': 'Останній перегляд: {}',
  '{} tarihinde izledin': 'Переглянуто {}',
  'Geri bildirimine yanıt verdik': 'Ми відповіли на ваш відгук',
  'Bu yanıt artık görüntülenemiyor.': 'Цю відповідь більше не можна показати.',
  'Gönderdiğin geri bildirim': 'Надісланий вами відгук',
  'Dizi veya film seç (zorunlu)': 'Виберіть серіал або фільм (обовʼязково)',
  'Bölüm seç (isteğe bağlı)': 'Виберіть епізод (необовʼязково)',
  '{}. sezon {}. bölüm': 'Сезон {}, епізод {}',
  'Ne düşünüyorsun?': 'Що ви думаєте?',
  '{}/{} yükleniyor': 'Завантаження {}/{}',
  'Fotoğraf/video ekle': 'Додати фото або відео',
  'Paylaşmak için önce bir dizi veya film seç.':
      'Спочатку виберіть серіал або фільм для допису.',
  'Sezon seç': 'Виберіть сезон',
  '{}. sezon': 'Сезон {}',
  'Bölüm seçme (yalnız dizi hakkında)': 'Без епізоду (лише про серіал)',
  'Sezonlar yüklenemedi.': 'Не вдалося завантажити сезони.',
  'Bu dizide bölüm bilgisi yok.': 'Немає інформації про епізоди цього серіалу.',
  'Bu sezonda bölüm yok.': 'У цьому сезоні немає епізодів.',
  '{}. bölüm': 'Епізод {}',
  'Dizi veya film ara...': 'Пошук серіалів і фільмів...',
  'İzlediğin dizi ve film hakkında yorum paylaş':
      'Поділіться думкою про те, що дивилися',
  'Yapım seç (zorunlu)': 'Виберіть тайтл (обовʼязково)',
  'Paylaşmak için önce bir yapım seç.': 'Спочатку виберіть тайтл для допису.',
  'Kişi': 'Людина',
  'Yapım firması': 'Кінокомпанія',
  'Yapım ara...': 'Пошук тайтлів...',
  'Çarkı çevirmek için sürükle veya dokun':
      'Проведіть або торкніться, щоб крутити колесо',
  'Arşiv': 'Архів',
  'Yüklediklerim': 'Мої завантаження',
  'GIF ara...': 'Пошук GIF...',
  'GIF yükle': 'Завантажити GIF',
  'GIF etiketleri': 'Теги GIF',
  'Virgülle ayır. Aramada bu kelimelerle bulunur.':
      'Розділяйте комами. За цими словами його знайдуть.',
  'gülme, alkış, şaşkın': 'сміх, оплески, здивування',
  'Yükle': 'Завантажити',
  'Henüz GIF yok': 'Поки що немає GIF',
  'Bu aramaya uyan GIF yok': 'За цим запитом GIF не знайдено',
  'Arşiv yeni. İlk GIF i sen yükle — sen hemen kullanırsın, onaydan sonra herkes görür.':
      'Архів новий. Завантаж перший GIF — одразу зможеш ним користуватися, а всі побачать його після схвалення.',
  'GIF yüklendi. Sen hemen kullanabilirsin; herkese açık arşive onaydan sonra girer.':
      'GIF завантажено. Можеш користуватися ним одразу; до спільного архіву він потрапить після схвалення.',
  'Onay bekliyor': 'Очікує схвалення',
  'GIF listesi yüklenemedi': 'Не вдалося завантажити список GIF',
  'GIF yüklenemedi': 'Не вдалося завантажити GIF',
  'Bu GIF i şikayet et': 'Поскаржитися на цей GIF',
  'Uygunsuz içerik yöneticiye bildirilir ve arşivden kaldırılabilir.':
      'Про неприйнятний вміст повідомляють модератора, і його можуть вилучити з архіву.',
  'Şikayetin alındı': 'Скаргу отримано',
  'Şikayet gönderilemedi': 'Не вдалося надіслати скаргу',
  'Dizinin kendisi': 'Сам серіал',
  'Sezon ya da bölüm seçme': 'Без сезону чи серії',
  'Tüm {}. sezon': 'Увесь сезон {}',
  'Bölüm seçme': 'Без серії',
  'En fazla {} yapım ekleyebilirsin.': 'Можна додати щонайбільше {} назв.',
  'Bu zaten ekli.': 'Уже додано.',
  'Dizi, film, oyuncu ya da yönetmen etiketle — gönderin onların sayfasında da görünür.':
      'Познач серіал, фільм, актора або режисера — твій допис з’явиться і на їхній сторінці.',
  'Etiket ekle': 'Додати тег',
  'Dizi, film, oyuncu ya da yönetmen etiketle':
      'Познач серіал, фільм, актора або режисера',
  'Gönderiden vazgeç': 'Скасувати допис',
  'Yazdıkların kaydedilmeyecek.': 'Написане не буде збережено.',
  'Yazmaya devam et': 'Продовжити писати',
  'Bulunamadı. Adın yazılışını değiştirip dene.':
      'Не знайдено. Спробуй інше написання.',
  'Dizi, film, oyuncu, yönetmen ya da yapım firması ara.':
      'Шукай серіал, фільм, актора, режисера або студію.',
  'E-posta adresini değiştir': 'Змінити адресу електронної пошти',
  'E-posta bağlı değil': 'Пошту не прив’язано',
  'Geçerli bir e-posta adresi yaz': 'Введи дійсну адресу електронної пошти',
  'Bu zaten hesabının e-posta adresi':
      'Це вже адреса пошти твого облікового запису',
  'Kod gönder': 'Надіслати код',
  'E-posta adresin güncellendi': 'Адресу електронної пошти оновлено',
  'Bu e-posta başka bir hesapta kayıtlı':
      'Ця пошта зареєстрована в іншому обліковому записі',
  'Doğrulama kodu YENİ adrese gönderilir; adres ancak kodu girdikten sonra değişir. Şifre sıfırlama ve iki adımlı giriş kodları bundan sonra yeni adrese gider.':
      'Код підтвердження надсилається на НОВУ адресу; адреса змінюється лише після введення коду. Коди скидання пароля та двоетапного входу відтоді надходитимуть на нову адресу.',
  'Hesabındaki e-posta adresi geçerli değil, bu yüzden e-posta teslim edilemiyor. Doğru adresi bağlamak için destekle iletişime geç.':
      'Адреса електронної пошти у твоєму обліковому записі недійсна, тому лист неможливо доставити. Звернися до підтримки, щоб прив’язати правильну адресу.',
  'Bu adrese kod gönderilemedi, adresi kontrol et':
      'Не вдалося надіслати код на цю адресу, перевір адресу',
  'ya da': 'або',
  'Google ile doğrula': 'Підтвердити через Google',
  'Apple ile doğrula': 'Підтвердити через Apple',
  'Hesabını Google ya da Apple ile açtıysan şifren yoktur; kimliğini o hesapla doğrula.':
      'Якщо ви створили обліковий запис через Google або Apple, у вас немає пароля; підтвердьте особу цим обліковим записом.',
  'Hesabını Google ile açtıysan şifren yoktur; kimliğini Google ile doğrula.':
      'Якщо ти створив обліковий запис через Google, у тебе немає пароля; підтверди особу через Google.',
  'Google doğrulaması bu hesapla eşleşmedi':
      'Підтвердження Google не збіглося з цим обліковим записом',
  'Bu hesap Google ile açıldı; doğrulamak için Google ile giriş yap':
      'Цей обліковий запис створено через Google; увійди через Google, щоб підтвердити',
  // --- 31 Agu 2026: kitaplik paylasimi + reels ceviri + sohbet detayi ---
  'Bu liste gizli': 'Цей список приховано',
  'Bu kullanıcı izleme listelerini gizli tutmayı tercih ediyor.':
      'Цей користувач воліє тримати свої списки прихованими.',
  'Bu listede henüz içerik yok': 'У цьому списку поки немає тайтлів',
  'Otomatik çeviri': 'Автоматичний переклад',
  'Otomatik çeviri açıldı': 'Автопереклад увімкнено',
  'Otomatik çeviri kapatıldı': 'Автопереклад вимкнено',
  'Sohbet sessize alındı': 'Чат без звуку',
  'Sessize alma kaldırıldı': 'Звук увімкнено',
  'Tema özelleştir': 'Налаштувати тему',
  'Sohbette ara': 'Пошук у чаті',
  'Mesajlarda ara...': 'Пошук у повідомленнях...',
  'Sonuç yok': 'Немає результатів',
  'Profili gör': 'Переглянути профіль',
  'Medya ve dosyalar': 'Медіа та файли',
  'Henüz medya yok': 'Поки немає медіа',
  'Varsayılan': 'За замовчуванням',
  'Mor': 'Фіолетовий',
  'Pembe': 'Рожевий',
  'Turuncu': 'Помаранчевий',
  'Orijinal metin gösteriliyor': 'Показано оригінальний текст',
  'İnternet bağlantısı yok': 'Немає підключення до інтернету',
  'Sunucu yanıt vermedi': 'Сервер не відповів',
  'Güvenli bağlantı kurulamadı': 'Не вдалося встановити захищене з’єднання',
  'Sunucudan beklenmeyen bir yanıt geldi': 'Неочікувана відповідь сервера',
  'İzlediğim Diziler': 'Переглянуті серіали',
  'İzlediğim Filmler': 'Переглянуті фільми',
  'Satır görünümü': 'Списком',
  'Afiş görünümü': 'Постерами',
  'Satır görünümünde sürükleme kapalı; "En üste taşı" ile öne al.':
      'У вигляді списку перетягування вимкнено; скористайся «Перемістити на початок», щоб винести вперед.',
  'Spoiler uyarısını göster': 'Показувати попередження про спойлери',
  'Kapalıyken spoiler işaretli gönderiler perdesiz, doğrudan gösterilir.':
      'Якщо вимкнено, дописи з позначкою «спойлер» показуються одразу, без завіси.',
  'Etiketler': 'Позначки',
  'Galeri': 'Галерея',
  'Kamera': 'Камера',
  'Dosya': 'Файл',
  'GIF': 'GIF',
  'Dizi / Film': 'Серіал / Фільм',
  'Sesli mesaj (basılı tut)': 'Голосове повідомлення (утримуйте)',
  'Kaydetmek için basılı tut': 'Утримуйте для запису',
  'Kaydırarak iptal': 'Проведіть, щоб скасувати',
  'Kamera açılamadı': 'Не вдалося відкрити камеру',
  'Dosya seçilemedi': 'Не вдалося вибрати файл',
  'Dosya gönderilemedi': 'Не вдалося надіслати файл',
  'En yeni mesaja in': 'До останнього повідомлення',
  'Gönderilemedi · tekrar dene': 'Не надіслано · повторити',
  'Dosya açılamadı': 'Не вдалося відкрити файл',
  'Temizle': 'Очистити',
  'Oyuncu': 'Актор',
  'Böyle bir oda yok': 'Такої кімнати немає',
  'Bu oda kapandı': 'Цю кімнату закрито',
  'Bu odaya girmek için davet ya da oda kodu gerekli':
      'Потрібне запрошення або код кімнати',
  'Oda dolu': 'Кімната заповнена',
  'Bu odaya giremezsin': 'Ви не можете увійти в цю кімнату',
  'Bu odanın üyesi değilsin': 'Ви не учасник цієї кімнати',
  'Bunu yalnız oda sahibi yapabilir': 'Це може зробити лише господар кімнати',
  'Oda sahibi odayı kapatmalı': 'Кімнату має закрити господар',
  'Zaten açık bir odan var': 'У вас уже є відкрита кімната',
  'Oda kodu 6 karakter olmalı': 'Код кімнати має містити 6 символів',
  'Misafir hesaplar izleme odası açamaz. Hesap oluşturursan açabilirsin.':
      'Гостьові акаунти не можуть відкривати кімнати. Створіть акаунт, щоб відкрити.',
  'Yalnız karşılıklı takipleştiğin kişileri davet edebilirsin':
      'Запрошувати можна лише тих, хто підписаний на вас у відповідь',
  'Video en fazla {} GB olabilir': 'Відео не більше {} ГБ',
  'Yalnızca MP4 veya WebM izlenebilir': 'Відтворюються лише MP4 або WebM',
  '{} odaya katıldı': '{} приєднався до кімнати',
  '{} odadan ayrıldı': '{} вийшов із кімнати',
  '{} bir video yükledi': '{} завантажив відео',
  'bitti': 'закінчилося',
  '{} sn': '{} с',
  'Birlikte izle': 'Дивіться разом',
  'Bir video yükle, arkadaşlarını davet et, aynı anda izleyin.':
      'Завантажте відео, запросіть друзів і дивіться одночасно.',
  'Henüz bir odan ya da davetin yok.':
      'У вас поки немає ні кімнати, ні запрошення.',
  'Odaya katıl': 'Увійти в кімнату',
  'Oda kodu': 'Код кімнати',
  'Katıl': 'Увійти',
  'Oda oluştur': 'Створити кімнату',
  'Odalar 12 saat sonra kendiliğinden kapanır ve video silinir.':
      'Кімнати закриваються самі через 12 годин, а відео видаляється.',
  'Odam': 'Моя кімната',
  '@{} odası': 'Кімната @{}',
  'Seni davet etti': 'Запросив вас',
  '{} kişi': '{} осіб',
  'Video yok': 'Немає відео',
  '{} kaldı': 'Залишилося {}',
  'Davet': 'Запрошення',
  '@{} davet edildi': '@{} запрошено',
  'Oda kodu kopyalandı': 'Код кімнати скопійовано',
  'Odayı kapat': 'Закрити кімнату',
  'Odadan ayrıl': 'Вийти з кімнати',
  'Oda kapanacak ve video silinecek. Bu geri alınamaz.':
      'Кімнату буде закрито, а відео видалено. Це не можна скасувати.',
  'Odadan çıkacaksın. Kodla ya da davetle geri dönebilirsin.':
      'Ви вийдете з кімнати. Повернутися можна за кодом або запрошенням.',
  'Ayrıl': 'Вийти',
  'İzleme odası': 'Кімната перегляду',
  'Odalar 12 saat sonra kendiliğinden kapanır.':
      'Кімнати закриваються самі через 12 годин.',
  '{} kişi · {} kaldı': '{} осіб · залишилося {}',
  'Davet et': 'Запросити',
  'Bir video yükle, izlemeye başlayın': 'Завантажте відео й починайте дивитися',
  'Oda sahibi henüz video yüklemedi': 'Господар ще не завантажив відео',
  'Video yükle': 'Завантажити відео',
  'En fazla {} GB · MP4 veya WebM': 'До {} ГБ · MP4 або WebM',
  'Kaldığı yerden yükleniyor · {}/{} MB': 'Завантаження триває · {}/{} МБ',
  'Yükleniyor · {}/{} MB': 'Завантаження · {}/{} МБ',
  'Videoyu değiştir': 'Змінити відео',
  'Oda sahibiyle eşleniyor': 'Синхронізація з господарем',
  'Sohbet boş': 'Чат порожній',
  'İzlerken buradan konuşabilirsiniz.':
      'Тут можна спілкуватися під час перегляду.',
  'oda sahibi': 'господар',
  'davet bekliyor': 'очікує запрошення',
  'çevrimdışı': 'не в мережі',
  'Mesaj yaz...': 'Напишіть повідомлення...',
  'Oda kodunu kopyala': 'Скопіювати код кімнати',
  'Oda kodu {} — kopyala': 'Код кімнати {} — скопіювати',
  'Arkadaşını davet et': 'Запросіть друга',
  'kullanıcı adı': "ім'я користувача",
  'Yalnız karşılıklı takipleştiğin kişileri davet edebilirsin.':
      'Запрошувати можна лише тих, хто підписаний на вас у відповідь.',
  'Oda açılamadı': 'Не вдалося відкрити кімнату',
  'Tam ekrandan çık': 'Вийти з повноекранного режиму',
  'Sohbeti gizle': 'Сховати чат',
  'Sohbeti göster': 'Показати чат',
  'Fragman': 'Трейлер',
  'Tekrar oynat': 'Відтворити знову',
  '@{} seni izleme odasına davet etti': '@{} запросив вас до кімнати перегляду',
  '{} · {} davet bekliyor': '{} · очікують запрошень: {}',
  'Sırada bekliyor': 'У черзі',
  'Hazırlanıyor · %{}': 'Підготовка · {} %',
  'Video, her cihazda oynayacak biçime çevriliyor.':
      'Відео перетворюється, щоб відтворюватися на будь-якому пристрої.',
  'Başka video yükle': 'Завантажити інше відео',
  'Bu videoyu tarayıcı oynatamıyor. Telefondan aç.':
      'Браузер не може відтворити це відео. Відкрийте його на телефоні.',
  'Bu videoyu iPhone oynatamıyor.': 'iPhone не може відтворити це відео.',
  'Tarayıcı için hazırla': 'Підготувати для браузера',
  'Başlat': 'Почати',
  'Bu videonun görüntü biçimi oynatılamıyor. MP4 (H.264) olarak çevirip yeniden dene.':
      'Формат цього відео не відтворюється. Перетворіть його на MP4 (H.264) і спробуйте ще раз.',
  'Bu dosyada görüntü yok': 'У цьому файлі немає відео',
  'Video okunamadı, dosya bozuk olabilir':
      'Не вдалося прочитати відео, файл може бути пошкоджений',
  'Sunucuda yer kalmadı, biraz sonra tekrar dene':
      'На сервері закінчилося місце, спробуйте трохи згодом',
  'Video bulunamadı': 'Відео не знайдено',
  'Yetkili': 'Модератор',
  'Kontrolü ver': 'Передати керування',
  'Kontrolü al': 'Забрати керування',
  'Videoyu oynatıp durdurabilir ve değiştirebilir.':
      'Може вмикати, ставити на паузу й міняти відео.',
  'Artık videoyu yönetemez.': 'Більше не може керувати відео.',
  'Bunu oda sahibi ve yetki verdiği kişiler yapabilir':
      'Це можуть лише господар і ті, кому він довірив',
  'Kendi yetkini değiştiremezsin': 'Свої права змінити не можна',
  'Geçersiz yetki': 'Недійсне право',
  '{} artık videoyu yönetebilir': '{} тепер може керувати відео',
  '{} kontrolü bıraktı': '{} більше не керує',
  'Kişi ara...': 'Пошук людей...',
  'Karşılıklı takipleştiğin kimse yok':
      'Поки ніхто не підписаний на вас взаємно',
  'Böyle bir kullanıcı yok': 'Такого користувача немає',
  'Oda kodunu paylaşarak da davet edebilirsin.':
      'Запросити можна й поділившись кодом кімнати.',
  'Karşılıklı takipleşmiyorsunuz': 'Ви не підписані одне на одного',
  'Odada': 'У кімнаті',
  'Davet edildi': 'Запрошено',
  'Aşk': 'Кохання',
  'Arkadaşlar': 'Друзі',
  'Gece': 'Ніч',
  'Okyanus': 'Океан',
  'Gün batımı': 'Захід сонця',
  'Orman': 'Ліс',
  'Neon': 'Неон',
  'Temalar': 'Теми',
  'Renkler': 'Кольори',
  'Sık kullanılanlar': 'Часто вживані',
  'Klavye': 'Клавіатура',
  'Kullanıcı adını seç': 'Обери ім’я користувача',
  'Google ile giriş yaptığın için adını biz türettik. Profil bağlantın ve etiketlenmelerin bu adla görünür; şimdi kendin seç.':
      'Оскільки ти увійшов через Google, ми згенерували ім’я за тебе. Посилання на профіль і згадки використовують його — обери своє зараз.',
  'Kontrol ediliyor...': 'Перевіряємо...',
  '@{} müsait': '@{} вільне',
  'Bu adı seç': 'Обрати це ім’я',
  'Kullanıcı adın @{} oldu': 'Твоє ім’я користувача тепер @{}',
  'Sonradan Ayarlar\'dan değiştirebilirsin; her değişiklikten sonra 90 gün beklenir.':
      'Пізніше його можна змінити в Налаштуваннях; після кожної зміни потрібно чекати 90 днів.',
  'Değerlendirme': 'Оцінки',
  'Değerlendirmeler ({})': 'Оцінки ({})',
  'Henüz değerlendirme yok': 'Оцінок поки немає',
  'Puan verilen dizi ve filmler burada listelenir.':
      'Оцінені серіали та фільми з\'являться тут.',
  'Değerlendirmeler yüklenemedi': 'Не вдалося завантажити оцінки',
  'Ayarla': 'Налаштувати',
  'Filtre': 'Фільтр',
  'Çıkartma': 'Стікер',
  'Parlaklık': 'Яскравість',
  'Kontrast': 'Контраст',
  'Doygunluk': 'Насиченість',
  'Pozlama': 'Експозиція',
  'Sıcaklık': 'Температура',
  'Renk tonu': 'Відтінок',
  'Ton': 'Тонування',
  'Soluk': 'Вицвітання',
  'Orijinal': 'Оригінал',
  'Sıcak': 'Теплий',
  'Soğuk': 'Холодний',
  'Sinematik': 'Кіно',
  'Retro': 'Ретро',
  'Vintage': 'Вінтаж',
  'Siyah Beyaz': 'Чорно-білий',
  'Canlı': 'Яскравий',
  'Analog': 'Аналог',
  'Kes': 'Обрізати',
  'Hız': 'Швидкість',
  'Ses': 'Звук',
  'Ses seviyesi': 'Гучність',
  'Fotoğraf çek': 'Зробити фото',
  'Video çek': 'Зняти відео',
  'Kolaj': 'Колаж',
  'Düzen': 'Макет',
  'Boşluk': 'Відступ',
  'Köşe': 'Кути',
  'Kolaj oluşturulamadı': 'Не вдалося створити колаж',
  'Kolaj en fazla {} fotoğraf alır': 'У колаж входить до {} фото',
  'Yer değiştirmek için iki fotoğrafa dokun':
      'Торкніться двох фото, щоб поміняти їх',
  'Eleştirmen': 'Критики',
  'Seyirci': 'Глядачі',
  "2027'de Vizyona Girecek Filmler": "Фільми, що вийдуть у 2027",
  "2027'de Başlayacak Diziler": "Серіали, що стартують у 2027",
  'Gizli hesap': 'Закритий акаунт',
  'Açıkken profilini yalnız onayladığın takipçiler görür; yeni takipler sana istek olarak gelir':
      'Якщо ввімкнено, ваш профіль бачать лише схвалені підписники; нові підписки надходять як запити',
  'İstek Gönderildi': 'Запит надіслано',
  'Bu hesap gizli': 'Це закритий акаунт',
  'İsteğin bekliyor; onaylanınca izlediklerini, yorumlarını ve listelerini görebilirsin.':
      'Ваш запит очікує; після схвалення ви побачите перегляди, коментарі та списки.',
  'İzlediklerini, yorumlarını ve listelerini görmek için takip isteği gönder; onaylanınca profil açılır.':
      'Надішліть запит на підписку, щоб бачити перегляди, коментарі та списки; профіль відкриється після схвалення.',
  '@{} seni takip etmek istiyor': '@{} хоче підписатися на вас',
  '@{} takip isteğini kabul etti': '@{} прийняв(ла) ваш запит на підписку',
  'Onayla': 'Схвалити',
  'Yeni liste oluştur': 'Створити новий список',
  'Henüz listen yok.': 'У тебе ще немає списків.',
  'Telif hakkı onayı': 'Підтвердження авторських прав',
  'Yalnızca hakkına sahip olduğun ya da paylaşma izni aldığın videoları yükle. Telif hakkı ihlali içeren videolar kaldırılır ve hesap kapatılabilir.':
      'Завантажуй лише відео, права на які належать тобі або якими тобі дозволено ділитися. Відео, що порушують авторські права, видаляються, а обліковий запис може бути закрито.',
  'Onaylıyorum': 'Підтверджую',
  'Topluluk Kuralları': 'Правила спільноти',
  'Saldırgan, taciz edici, nefret içeren, cinsel, şiddet yüceltici ya da telif hakkı ihlali içeren içeriklere ve kötüye kullanıma sıfır tolerans gösterilir. Her yorum, gönderi, mesaj, liste ve kullanıcı uygulama içinden şikâyet edilebilir; istenmeyen kullanıcılar engellenebilir. Şikâyet edilen içerik 24 saat içinde incelenir; kural ihlalinde içerik kaldırılır ve hesap kapatılabilir.':
      'Нульова толерантність до образливого, цькувального, ненависницького, сексуального, такого, що прославляє насильство або порушує авторські права, контенту та до зловживання сервісом. На будь-який коментар, допис, повідомлення, список і користувача можна поскаржитися в застосунку; небажаних користувачів можна заблокувати. Контент зі скаргою перевіряється протягом 24 годин; у разі порушення правил його видаляють, а обліковий запис може бути закрито.',
  'Dosya verisi gerekli': 'Потрібні дані файлу',
  'Geçerli bir ZIP, CSV ya da JSON dosyası değil':
      'Це не коректний файл ZIP, CSV або JSON',
  'Dosya boş': 'Файл порожній',
  'Bu dosya tanınmadı: TV Time, Letterboxd ya da dizi.jpg dosyası yükle':
      'Файл не розпізнано: завантаж файл із TV Time, Letterboxd або dizi.jpg',
  'Bu dosya tek başına aktarılamıyor: arşivin tamamını (.zip) ya da dizijpg.json dosyasını yükle':
      'Цей файл не можна імпортувати окремо: завантаж увесь архів (.zip) або файл dizijpg.json',
  'Ort.': 'Сер.',
  'En iyi bölüm': 'Найкраща серія',
  'Sezonun bölümleri': 'Серії сезону',
  'Ekran görüntüsü aldın': 'Ви зробили знімок екрана',
  '{} ekran görüntüsü aldı': '{} зробив(ла) знімок екрана',
  'Gönderiliyor': 'Надсилання',
  'İzlediğin her şeyi tek yerde takip et':
      'Відстежуй усе, що дивишся, в одному місці',
  'Keşfet\'e göz at': 'Переглянути розділ «Огляд»',
  'Giriş yap': 'Увійти',
  'Neler yapabilirsin?': 'Що ти можеш робити?',
  'Bölüm bölüm takip': 'Відстеження серія за серією',
  'Kaldığın yeri asla kaybetme: sezonları ve bölümleri tek dokunuşla işaretle, ilerlemeni gör.':
      'Ніколи не губи місце: позначай сезони й серії одним дотиком і стеж за прогресом.',
  'Yeni bölümler ve vizyon tarihleri takviminde; hiçbir bölümü kaçırma.':
      'Нові серії та дати виходу у твоєму календарі; не пропусти жодної серії.',
  'Puanla ve yorumla': 'Оцінюй і пиши відгуки',
  'Ondalıklı puan ver, yorum yaz; IMDb, Rotten Tomatoes ve Metacritic puanlarını yan yana gör.':
      'Став дробові оцінки, пиши відгуки; порівнюй IMDb, Rotten Tomatoes і Metacritic поруч.',
  'Kendi listelerin': 'Твої списки',
  'Listeler oluştur, sırala ve paylaş; izleme listen her cihazında seninle.':
      'Створюй, сортуй і ділися списками; список «подивитися» з тобою на кожному пристрої.',
  'Arkadaşlarınla': 'З друзями',
  'Akışta arkadaşlarının ne izlediğini gör, yorumlarına tepki ver, mesajlaş.':
      'Дивись у стрічці, що дивляться друзі, реагуй на їхні відгуки, листуйся.',
  'İzleme odasında aynı anda izleyin ve sohbet edin.':
      'Дивіться одночасно в кімнаті перегляду й спілкуйтеся в чаті.',
  'Yapım verileri: TMDB': 'Дані про фільми й серіали: TMDB',
  'Galeriye kaydet': 'Зберегти в галерею',
  'Galeriye kaydedildi': 'Збережено в галерею',
  'Galeriye kaydediliyor...': 'Зберігаємо в галерею...',
  'Galeri izni verilmedi': 'Доступ до галереї не надано',
  'Kaydedilemedi': 'Не вдалося зберегти',
  '{} medyayı galeriye kaydet': 'Зберегти {} медіа в галерею',
  '{}/{} kaydedildi': 'Збережено {}/{}',
  'Hemen kaydol': 'Зареєструватися',
  'Dizilerini bölüm bölüm işaretle, film listeni tut, puanla ve yorumla; arkadaşlarının ne izlediğini gör. Web ve mobilde.':
      'Позначай серіали серія за серією, веди список фільмів, оцінюй і пиши відгуки, дивись, що дивляться друзі. У вебі та на мобільному.',
  '45 dilde. Web, Android ve iOS.': '45 мовами. Веб, Android та iOS.',
  'Takma ad': 'Псевдонім',
  'Bu adı yalnız sen görürsün': 'Це ім\'я бачиш лише ти',
  'Takma ad kaydedilemedi': 'Не вдалося зберегти псевдонім',
  'Tema karşı tarafa iletilemedi': 'Не вдалося надіслати тему співрозмовнику',
  'Seçtiğin tema karşı tarafta da görünür':
      'Обрана тема відображається і в співрозмовника',
  'Alıcı bir kez görebilir': 'Одержувач зможе переглянути один раз',
  'Açıldı': 'Відкрито',
  'Flaş': 'Спалах',
  'Fotoğraf çekilemedi': 'Не вдалося зробити фото',
  'Galeri izni gerekli': 'Потрібен доступ до галереї',
  'Galeride medya yok': 'У галереї немає медіа',
  'Galeriden seç': 'Вибрати з галереї',
  'Görmek için dokun': 'Торкнись, щоб переглянути',
  'Konum': 'Місцезнаходження',
  'Konum alınamadı': 'Не вдалося отримати місцезнаходження',
  'Konum izni gerekli': 'Потрібен дозвіл на місцезнаходження',
  'Konum servisi kapalı': 'Служби геолокації вимкнено',
  'Konumum': 'Моє місцезнаходження',
  'Mesaj ekle...': 'Додати повідомлення...',
  'Son fotoğrafların burada görünsün diye galerine erişim gerekir.':
      'Щоб показати тут останні фото, потрібен доступ до галереї.',
  'Tek kullanımlık': 'Одноразовий перегляд',
  'Tek kullanımlık fotoğraf': 'Одноразове фото',
  'Tek kullanımlık medya': 'Одноразове медіа',
  'Tek kullanımlık video': 'Одноразове відео',
  'Tek kullanımlık yalnız tek fotoğraf veya videoda':
      'Одноразовий перегляд працює лише з одним фото або відео',
  'Video kaydedilemedi': 'Не вдалося зберегти відео',
  'Video kaydı başlamadı': 'Запис відео не розпочався',
  'Çek': 'Зняти',
  'Çekmek için dokun, video için basılı tut':
      'Торкнись для фото, утримуй для відео',
  'İzin ver': 'Дозволити',
  'az önce': 'щойно',
  '{} dk önce': '{} хв тому',
  '{} dk önce~tekil': '{} хв тому',
  '{} saat önce': '{} год тому',
  '{} saat önce~tekil': '{} год тому',
  '{} gün önce': '{} дн. тому',
  '{} gün önce~tekil': '{} дн. тому',
  '{} hafta önce': '{} тиж. тому',
  '{} hafta önce~tekil': '{} тиж. тому',
  // Mesaj/sohbet silme + belge indirme (16 Eyl 2026)
  'Bu mesaj silindi': 'Це повідомлення видалено',
  'Benden sil': 'Видалити в мене',
  'Herkesten sil': 'Видалити в усіх',
  'Sohbeti sil': 'Видалити чат',
  'Karşı taraftan da sil': 'Видалити й у співрозмовника',
  'Yalnız senden silinir': 'Видаляється лише у вас',
  '{} için de silinir': 'Видаляється також у {}',
  'Bu sohbet senden silinecek. Geri alınamaz.':
      'Цей чат буде видалено у вас. Це неможливо скасувати.',
  'Bu sohbet iki taraftan da silinecek. Geri alınamaz.':
      'Цей чат буде видалено в обох сторін. Це неможливо скасувати.',
  'Sohbet silinemedi': 'Не вдалося видалити чат',
  'İndiriliyor': 'Завантаження',
  'İndirildi': 'Завантажено',
  'Dosya indirilemedi': 'Не вдалося завантажити файл',
  'Bu dosyayı açacak uygulama yok': 'Немає застосунку, який відкриє цей файл',
  'Dosyanın yalnız başı gösteriliyor': 'Показано лише початок файлу',
  // Kanon listeleri + çark seçeneği (kanon, 16 Eyl 2026)
  'Ölmeden İzlenmesi Gereken 100 Film': '100 фільмів, які варто побачити',
  'Ölmeden İzlenmesi Gereken 250 Film': '250 фільмів, які варто побачити',
  'Ölmeden İzlenmesi Gereken 500 Film': '500 фільмів, які варто побачити',
  'Ölmeden İzlenmesi Gereken 1000 Film': '1000 фільмів, які варто побачити',
  'Ölmeden İzlenmesi Gereken 10 Dizi': '10 серіалів, які варто побачити',
  'Ölmeden İzlenmesi Gereken 25 Dizi': '25 серіалів, які варто побачити',
  'Ölmeden İzlenmesi Gereken 50 Dizi': '50 серіалів, які варто побачити',
  'Ölmeden İzlenmesi Gereken 100 Dizi': '100 серіалів, які варто побачити',
  'İzlediklerimi gösterme': 'Сховати переглянуте',
  'Bu listede izlemediğin yapım kalmamış':
      'У цьому списку не лишилося непереглянутого',
  'Liste adı kaydedilemedi': 'Не вдалося зберегти назву списку',
  'Liste adı boş olamaz': 'Назва списку не може бути порожньою',
  "dizi.jpg'de Bu Hafta En Çok İzlenen 10 Film":
      "Топ-10 фільмів тижня на dizi.jpg",
  "dizi.jpg'de Bu Hafta En Çok İzlenen 10 Dizi":
      "Топ-10 серіалів тижня на dizi.jpg",
  "dizi.jpg'de Bu Ay En Çok İzlenen 10 Film":
      "Топ-10 фільмів місяця на dizi.jpg",
  "dizi.jpg'de Bu Ay En Çok İzlenen 10 Dizi":
      "Топ-10 серіалів місяця на dizi.jpg",
  "dizi.jpg'de 2026'da En Çok İzlenen 50 Film":
      "Топ-50 фільмів 2026 року на dizi.jpg",
  "dizi.jpg'de 2026'da En Çok İzlenen 50 Dizi":
      "Топ-50 серіалів 2026 року на dizi.jpg",
  "Bir süre gösterme": "Сховати на час",
  "Bu liste bir ay boyunca akışta görünmeyecek":
      "Цей список не з’являтиметься у стрічці місяць",
  "Liste gizlenemedi": "Не вдалося сховати список",
};
