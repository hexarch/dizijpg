...
/// Görsel editörünün AĞIR YARISI: `pro_image_editor`e dokunan her şey.
///
/// NEDEN AYRI DOSYA (19 Ağu 2026, "deferred imports" 1. tur): `pro_image_editor`
/// tüm uygulamada YALNIZ buradan kullanılıyor, ama tek parça `main.dart.js`
/// içinde HERKESE, HER AÇILIŞTA indiriliyordu. Ölçüldü: paket 12.414.444 bayt
/// ham JS ve içinde tek bir `.part.js` yok — yani hiçbir şey ertelenmiyordu.
/// Lighthouse aynı paketin %31,6'sının (686 KB) tipik bir oturumda HİÇ
/// çalışmadığını söylüyor; mobilde 1762 ms'lik ana iş parçacığı bloklaması
/// (TBT) büyük ölçüde buradan besleniyor. Oysa kullanıcıların çoğu "düzenle"
/// kalemine hiç dokunmuyor: herkesin ödediği bir maliyet, azınlığın kullandığı
/// bir özellik için.
///
/// Bu yüzden [gorselDuzenle] bu dosyayı `deferred as editor` ile import ediyor;
/// dart2js editörün TAMAMINI ayrı bir `.part.js`e koyuyor ve dosya ancak
/// kaleme dokunulduğunda ağdan çekiliyor.
///
/// SINIRIN KURALI — bunu bozmak sessizce erteleme özelliğini iptal eder:
/// * Buraya koyduğun bir sembolü ertelenmeyen taraftan SENKRON çağıramazsın;
///   çağıran her yol önce `loadLibrary()` beklemek zorunda.
/// * Bu dosyaya ertelenmemiş bir yoldan (`gorsel_duzenle.dart` DIŞINDAN,
///   normal `import` ile) ulaşan tek bir satır bile, dart2js'in tüm paketi
///   yeniden `main.dart.js`e taşımasına yeter. Yeni bir çağıran gerekiyorsa
///   o da [gorselDuzenle] üzerinden geçsin.
/// * Ters yön SERBEST: burası `gorsel_duzenle.dart`taki saf yardımcıları
///   ([gorselTuru], [duzenlenebilirMi], [ayniBaytlar]) normal import'la
///   kullanır — onlar zaten ana pakette, tekrar indirilmiyorlar.
///
/// BURAYA NE GİRER: `pro_image_editor` tiplerine dokunan her şey — editör
/// yapılandırması, i18n, ekran widget'ı, `ImageConverter` kullanan PNG
/// küçültme. NE GİRMEZ: pakete ihtiyacı olmayan saf bayt yardımcıları
/// (sihirli bayt, saydamlık taraması); onlar ertelenmeyen tarafta kalır ki
/// kalem düğmesinin ÇİZİLİP ÇİZİLMEYECEĞİNE karar vermek için 686 KB
/// indirmek gerekmesin.
...
/// SONRA çağrılıyor, yani araya en az bir asenkron boşluk girmiş oluyor ve o
/// boşlukta widget sökülmüş olabilir (`use_build_context_synchronously`).
/// Bu yüzden gezinme/mesaj kancaları ÇAĞIRAN tarafta, ilk `await`ten önce
/// alınıp buraya HAZIR geçiriliyor — burada `context`e hiç dokunulmuyor.
///
/// `null` dönüşün üç anlamı ve hata sözleşmesi için [gorselDuzenle] belgesine
/// bak; bu fonksiyon onun gövdesidir, ayrı bir sözleşmesi yoktur.
Future<Uint8List?> gorselDuzenleGovde(
  NavigatorState gezgin,
  ScaffoldMessengerState mesajci,
  Uint8List veri, {
  required bool saydam,
}) async {
...
/// Ertelenen tarafın kendi SnackBar yardımcısı.
///
/// NEDEN KOPYA: `gorsel_duzenle.dart`taki ikizi `private` ve ana pakette;
/// onu buraya açmak için `public` yapmak, ertelenmemiş yüzeyi tek satırlık
/// bir yardımcı uğruna genişletmek olurdu. İki satır kopya, sızdıran bir
/// sınırdan ucuzdur.
